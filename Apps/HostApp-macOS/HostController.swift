...
final class HostController {
    ...
    enum State: Equatable {
        ...
        /// socket) is in flight. The UI stays BUSY here so a fast Stop→Start cannot race the old
        /// listener and self-collide with EADDRINUSE on the same port.
        case stopping
        case failed(String)
    }
    ...
    /// The single ordered consumer of the live client-count stream. One loop applies counts in
    /// FIRING ORDER (no Task-per-event reordering) and drops any count from a server that is no
    /// longer the current one. Cancelled on stop/restart.
    ...
    func start(port: UInt16) {
        ...
        case .running,
            ...
            // is what prevents a new listener binding the port while the old one is still being
            // released; the UI also keeps Start disabled across `.stopping` via `isBusy`.
            return
        case .stopped,
             .failed:
            break
        ...
        // The hook fires off the main actor (from the server's lock-guarded spawn/remove paths AND
        // from a stopped server's drain), so it must hop to this actor. Funnel every count through
        // one AsyncStream consumed by a SINGLE MainActor loop:
        //  - ordering: updates apply in firing order, not via a Task-per-event firehose whose
        //    MainActor hops can reorder a quick 1→2→1 burst into a transiently wrong value;
        //  - identity: a count from a server that is no longer `self.server` is dropped, so an
        ...
        // Post-ready listener health: if the listener silently dies after binding (interface drop /
        // socket error), the host must NOT keep showing a healthy "running" badge while it accepts
        // nothing. Flip to `.failed` with the same operator-facing message — guarded by server
        ...
        // on the long-lived menu-bar host. Weak both: a replaced or deallocated server then fails the
        // identity guard (or can't fire) and nothing leaks.
    ...
    /// failure surfaces as ``SlopDeskTransportError/listenerFailed(_:)`` (NOT an `NSPOSIXErrorDomain`
    /// NSError, so matching on an `ns.code` would never fire), and is overwhelmingly a port collision
    /// for a known port. Name the concrete port so the operator can fix it (change the port / kill the
    /// holder).
    static func describe(_ error: Error, port: UInt16) -> String {
        if case let .listenerFailed(detail)? = error as? SlopDeskTransportError {
            // Delegate EADDRINUSE classification to the transport's pure classifier rather than a naive
            // `detail.contains("48")`, which would misclassify any error whose text merely embeds the
            // digits "48" (a port like 4843, errno 148, a size like 1048576) as a port collision. The
            // classifier matches the errno only as a standalone token plus the "in use" phrase.
