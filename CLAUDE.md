...
**A clean checkout builds with no prerequisite** — `swift build` compiles the whole package straight away (the only C is the in-tree `CSlopDeskSIMD` target, built from source by SwiftPM; there is no staticlib to pre-build, no FFI, no build ordering).
