...
| `scripts/check-macos.sh`, `check-video.sh` | GUI proof; needs unlocked Aqua + Screen Recording TCC (not over SSH). `check-video.sh --second-client` is the ONLY gate that puts two `SCStream`s + two `VTCompressionSession`s on one capture target — unit tests may not build any of the four. Client B is given the TERMINAL autoconnect only, so it has to learn the pane from the document; it asserts A's decode counter still CLIMBS after B joins (fan-out, not takeover) and shoots each instance separately, raised by PID (a name-matched raise photographs one client twice) |
