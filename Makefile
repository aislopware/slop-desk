...
# ---------------------------------------------------------------------------- #
# Full gate
.PHONY: check quick check-ios check-macos-apps check-ios-tests gui-macos gui-video gui-multiclient gui-launch-restore build test test-touched golden ffi ffi-test hook hook-test ctl ctl-test posix-test superd superd-test superd-install screend screend-test screend-install devtools devtools-test dropd dropd-test androidd androidd-test inspectord inspectord-test wire wire-test altscreen-test muxsession-test hostnet-test superclient-test screenclient-test hostpane-test hostsession-test hostserver-test clientsession-test fuzzy-test devicelog-test devicepanel-test clientctl-test superwire-test hookevent-test rowscan-test video video-test gfsimd-test apple-cgevent-test apple-cgwindow-test apple-cgdisplay-test apple-power-test apple-app-test apple-cursor-test apple-ax-test apple-text-test apple-vt-test apple-audio-test audio-out-test apple-sck-test apple-pasteboard-test apple-fsevents-test panecensus-test miri workspace workspace-test invariants-test ids ids-test tree tree-test settings settings-test config-schema codepanel codepanel-test agent agent-test terminal terminal-test cli cli-test hostlaunch-test sidecars-test codeseed codeseed-test probe probe-test git-test host host-restart host-status
...
# it is the one part of the port that owns file descriptors, so it links `socket2` — a dependency
...
test: ffi hook-test invariants-test devtools-test ctl-test probe-test posix-test ffi-test git-test superd-test screend-test dropd-test androidd-test inspectord-test wire-test altscreen-test muxsession-test hostnet-test superclient-test screenclient-test hostpane-test hostsession-test hostserver-test clientsession-test fuzzy-test devicelog-test devicepanel-test clientctl-test superwire-test hookevent-test rowscan-test video-test gfsimd-test apple-cgevent-test apple-cgwindow-test apple-cgdisplay-test apple-power-test apple-app-test apple-cursor-test apple-ax-test apple-text-test apple-vt-test apple-audio-test audio-out-test apple-sck-test apple-pasteboard-test apple-fsevents-test panecensus-test workspace-test ids-test tree-test settings-test codepanel-test agent-test terminal-test cli-test hostlaunch-test sidecars-test codeseed-test provision-test ctl superd screend dropd androidd inspectord ## cargo test (relay + agent CLI + metadata probe + the unsafe surface + the C ABI + the git engine + custodian + screen engine + file drop + android bridge + inspector + wire codec + alt-screen cut scanner + one pane session's decisions + hostd's PATH-1 listener + hostd's superd client + hostd's screend client + hostd's half of one pane + one pane's session + hostd's composition + one client session's decisions + fuzzy matcher + device console grammars + device panel decisions + the client control vocabulary + superd framing + hook bodies + row scans + FEC codec + SIMD kernels + CoreGraphics injection + the window and display lists + the two sleep assertions + the running-application reads + the cursor shape + the accessibility tree + the Core Text family name + the VideoToolbox session + the AudioToolbox codecs + client audio output + the capture stream + the pasteboard + the repo watch + one pane's process and port census + workspace rules + identity + the document tree + the settings catalogue + the code panel dressing + agent detection + terminal input + CLI core + hostd's launch + sidecar versions + code-server profile + the pinned-dependency provisioner + the operator tools) + swift test with the green-tree cache
