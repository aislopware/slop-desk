// OttyKit — small reusable otty chrome controls, ported from the reverse-engineering at
// /Volumes/Lacie/Workspace/oss/otty-reversed (`ReplicaKit.swift` + `OttyReplica.swift`) onto the polished
// `Otty` token layer (OttyDesign.swift). Two pieces:
//   • `PlateIconButton` — the hover-plate icon button (PanelToggleButton / HoverIconButton): a borderless
//     SF-Symbol button that grows a faint rounded hover plate, 0.12s small-fade. Used by the titlebar chrome.
//   • `TitlebarHoverCatcher` — otty's `TitlebarHoverView`: ONE tracking area over the whole titlebar strip
    ...
    var body: some View {
        Button(action: action) {
            Image(systemSymbol: symbol)
                ...
                .background(hovering ? Otty.State.hover : .clear, in: .rect(cornerRadius: Otty.Metric.radiusControl))
                .contentShape(.rect)
        }
        .buttonStyle(.plain)
        .onHover { hovering = $0 }
...
#if os(macOS)
/// otty's `TitlebarHoverView`: a click-through NSView that reports hover over the whole titlebar strip by
