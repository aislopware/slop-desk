...
// built by the PURE `CLIShellShim` in `AislopdeskWorkspaceCore`), and the **Allow Overwrite** toggle.
//
...
#if os(macOS)
import AislopdeskWorkspaceCore // CLIShellShim
import Darwin
import Defaults
import Foundation
import Observation
...
public final class CLIInstaller {
    ...
    /// Whether `/usr/local/bin/aislopdesk` is a symlink that points at OUR bundled CLI. Probes the live
    ...
    public func install() async -> Bool {
        guard let source = Self.bundledCLIPath() else {
            fail("Could not locate the bundled aislopdesk binary in the app bundle.")
            return false
        }
        phase = .working
        errorMessage = nil
    ...
    /// The bundled `aislopdesk` CLI inside the running app bundle. Tries the auxiliary-executable slot
    /// (`Contents/MacOS/aislopdesk`) first, then a few conventional sibling locations, returning the first
    ...
    static func bundledCLIPath() -> String? {
        if let aux = Bundle.main.url(forAuxiliaryExecutable: "aislopdesk")?.path,
        ...
        let candidates = [
            root.appendingPathComponent("Contents/MacOS/aislopdesk"),
            root.appendingPathComponent("Contents/Helpers/aislopdesk"),
            root.appendingPathComponent("Contents/Resources/aislopdesk"),
        ]
    ...
    /// `~/Library/Application Support/Aislopdesk/cli-shims.sh` — sibling of the workspace / frecency / control
    /// sockets, the conventional aislopdesk app-support home.
