// PaneMoveAffordance — the otty-style "grab the pane and move it" affordance (REBUILD-V2).
//
// otty.sh reveals a short drag handle (a `-` pill) near the TOP of a pane on hover; you grab it to move the
// pane. We mirror that, adapted to the remote-app rule: the drag mutates NOTHING in the store — it only
...
// (to reveal the pill) and owns the drag gesture. SYSTEM / otty token colours only.
...
struct PaneMoveHandle: View {
    ...
    private var strip: some View {
        ZStack {
            Capsule()
                .fill(isDragging ? Otty.State.accent : Otty.Text.tertiary)
        ...
        .onHover { hovering = $0 }
            ...
            .onTapGesture { onTap() }
            .animation(Otty.Anim.dividerHover, value: revealed)
    }
}
...
struct PaneMoveOverlay: View {
    ...
    private func swapWash(_ rect: CGRect) -> some View {
        RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
            .fill(Otty.State.accentMuted)
            .overlay(
                RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
                    .strokeBorder(Otty.State.accent, lineWidth: 2),
            )
    ...
    private func resplitSlab(in rect: CGRect, edge: PaneDropEdge) -> some View {
        ...
        return ZStack(alignment: .topLeading) {
            RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
                        .strokeBorder(Otty.State.accent.opacity(0.7), lineWidth: 1.5),
                )
    ...
    private func dockRail(edge: PaneDropEdge) -> some View {
        ...
        return RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
            .fill(Otty.State.accentMuted)
            .overlay(
                RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
                    .strokeBorder(Otty.State.accent, lineWidth: 2),
    ...
    private var sourceOutline: some View {
        Group {
            if let rect = frames[drag.source] {
                RoundedRectangle(cornerRadius: Otty.Metric.radiusCard)
                    .strokeBorder(
                        Otty.State.accent.opacity(0.55),
    ...
    private var ghostChip: some View {
        ...
        .background(
            Capsule(style: .continuous)
                .fill(Otty.Surface.card)
                .overlay(
                    Capsule(style: .continuous)
                        .strokeBorder(
                            drag.zone == .none ? Otty.Text.tertiary.opacity(0.4) : Otty.State.accent,
                            lineWidth: 1,
                        ),
                ),
        )
        .shadow(color: Otty.State.shadow, radius: 8, y: 2)
        .fixedSize()
    }
