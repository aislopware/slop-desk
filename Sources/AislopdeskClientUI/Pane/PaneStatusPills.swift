...
//
// otty renders these in the window TITLEBAR's top-right corner (`docs/otty-clone/screenshots/readonly-mode.png`
// and, later, `secure-input.png`). aislopdesk has NO persistent titlebar — the window chrome is a hover-reveal
// strip and the pane is a flush, window-level surface — so the FAITHFUL EQUIVALENT of otty's titlebar top-right
// is the pane's top-trailing overlay region (the same place the ⌘F find bar floats). This file is the home for
...
struct ReadOnlyPill: View {
    ...
    /// The `×` close glyph — LIGHTER than the label (per the screenshot), with the `OttyTabRow` close-button's
