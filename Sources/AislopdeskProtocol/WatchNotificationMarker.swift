...
// The private sentinel that lets `aislopdesk watch` (otty-clone E20, WI-7) route its finish banner to the
...
//
// Defined HERE in the shared low-level protocol module so the CLI emitter (`AislopdeskCLICore.WatchProgress`)
// and the client router (`AislopdeskWorkspaceCore.NotificationEvent`) agree on ONE source of truth.

public enum WatchNotificationMarker {
    /// The control-char-framed sentinel placed in the OSC-777 notification TITLE by `aislopdesk watch` on
    ...
    public static let title = "\u{1F}aislopdesk:watch-finish\u{1F}"
}
