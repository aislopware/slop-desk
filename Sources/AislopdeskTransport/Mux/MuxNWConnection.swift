...
/// data/control surface ``AislopdeskClient`` expects.
///
/// ### Lifecycle
...
public actor MuxNWConnection {
    ...
    /// data + control sub-channel pair for ``MuxClientTransport`` to wire into ``AislopdeskClient``.
    ///
    ...
    public func openChannel(
        ...
        guard !isDead else { throw AislopdeskTransportError.notConnected("mux connection closed") }
    ...
    /// Optional host hook (R5 rank 3): fired EXACTLY ONCE when the physical connection dies via a HARD
    /// link failure (TCP RST / NetBird flap / decode fault), so the host can drop it from its retention
