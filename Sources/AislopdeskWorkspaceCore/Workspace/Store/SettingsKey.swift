...
public enum SettingsKey {
    ...
    /// "Notify on Watch Finish" — fire when an `aislopdesk watch`-wrapped command finishes (default ON).
    ...
    /// "Badge while processing" — show the agent THINKING spinner (default OFF, per
    ...
    /// "Tab Badge — When Command Awaits Input" — show the awaiting-input hand for a plain command stopped
    ...
    // Controls / scroll / copy (Controls section). These are FIRE-TIME `Defaults.Keys` flags — they
    ...
    // E8 WI-1: the remaining Controls / Mouse / Scroll knobs. Same fire-time `Defaults.Keys`
    ...
    // E10 (Path/link detection — Settings → Controls → Open With / Link Schemes). Fire-time
    ...
    // `AISLOPDESK_FEC_M`). These guards exist because the WireGuard mesh, not app-layer auth, is the
    // security boundary here; this maps the toggles onto the host agent-control socket. See docs/DECISIONS.md.
    ...
    /// How the vertical sidebar BUCKETS tabs into sections (sort-hamburger "Group By", E6). Stored as
    ...
    // E19/A29 (`window-size` — user-interface__window-tab-split.md). A NEW `window.*` namespace, so a
    ...
    // one-time guided sheet; the three `shell.cli.*` keys are the "Install CLI" / "Omit Prefix" /
    ...
    /// Whether the `aislopdesk` CLI symlink (`/usr/local/bin/aislopdesk`) is installed ("Install CLI").
    ...
    /// "Omit `aislopdesk` Prefix" — expose `edit`/`view`/`watch`/`jump`/`learn` as bare shell functions in
    ...
    /// Recipes → Command Replay for INTERNALLY-saved recipes (the `.aislopdeskrecipe` ⌘S writes). Stored as the
    ...
    /// Recipes → Command Replay for EXTERNALLY-opened `.aislopdeskrecipe` FILES. Stored as the
    ...
    /// snippet at-prompt AUTO-EXPAND (E16 ES-E16-4) — whether typing a snippet's alias at the shell prompt
    ...
    // MARK: E20/WI-9 first-launch + Aislopdesk CLI (getting-started__first-launch.md)
    ...
    /// Whether the `aislopdesk` CLI symlink is installed ("Install CLI"), default OFF. macOS-only
    ...
    /// The replay mode for INTERNALLY-saved recipes (Command Replay, E16), default
    ...
    /// The replay mode for externally-opened `.aislopdeskrecipe` FILES (Command Replay, E16), default
    ...
    /// Whether typing a snippet alias at the shell prompt auto-expands it (E16 ES-E16-4), default OFF
...
public extension Defaults.Keys {
    ...
    // E14/K2: the "Auto Progress-Bar Commands" list. Fire-time `[String]` key (never folded into a
    ...
    // Controls / scroll / copy (Controls). FIRE-TIME flags only — never folded into a typed prefs model
...
/// persisted Command Replay setting (E16) round-trips compactly; `PreferRawRepresentable` selects the
...
/// Store the E8 Controls / Mouse / Scroll enums as their bare `String` rawValue (the aislopdesk
...
/// Store the E10 link-interaction enums as their bare `String` rawValue (the aislopdesk config
