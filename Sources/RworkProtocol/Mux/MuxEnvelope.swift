...
public enum MuxEnvelopeCodec {
    ...
    ///
    /// - Throws: ``RworkError/truncated`` if the body is shorter than the mux type
    ///   requires, ``RworkError/unknownMessageType(_:)`` for an unrecognized mux-type
    ///   byte, or ``RworkError/malformedBody(_:)`` for a right-length-but-invalid body
    ...
    public static func decode(inner: Data) throws -> MuxFrame {
        ...
        guard let type = MuxFrameType(rawValue: typeByte) else {
            throw RworkError.unknownMessageType(typeByte)
        }
        ...
        case .channelOpen:
            ...
            guard let sessionID = UUID(dataBytes: idBytes) else {
                throw RworkError.malformedBody("channelOpen: invalid sessionID bytes")
            }
            return .channelOpen(
                channelID: channelID,
                sessionID: sessionID,
