...
public actor MuxClientTransport: ClientTransporting {
    ...
    private func requireData() throws -> MuxSubChannel {
        guard let dataChannel else { throw RworkTransportError.invalidState("mux: not connected (data)") }
        return dataChannel
    }

    private func requireControl() throws -> MuxSubChannel {
        guard let controlChannel else { throw RworkTransportError.invalidState("mux: not connected (control)") }
