...
/// N independent ``RworkVideoHostSession``s — one per client video channel (per
...
/// ``RworkVideoHostSession`` + capture/encode) is GUI-only and HANGS in a test — so it
...
public actor VideoMuxSessionRegistry {
    ...
    private var sessions: [UInt32: RworkVideoHostSession] = [:]

    /// GUI-only factory: enumerate the window the hello names, build + start a
    /// ``RworkVideoHostSession`` bound to a ``VideoMuxChannelTransport`` for `channelID` (whose
    ...
    private let mintSession: @Sendable (_ channelID: UInt32, _ hello: VideoControlMessage) async throws -> RworkVideoHostSession
    ...
    public init(
        ...
        mintSession: @escaping @Sendable (UInt32, VideoControlMessage) async throws -> RworkVideoHostSession
    ) {
