...
/// ``SlopDeskClient`` — which presents the preserved `sessionID` + `highestContiguousSeq`
...
/// byte-exact resume — see the reconnect note on ``SlopDeskClient`` for the full story (and the
...
///
/// ### Policy
/// - **Trigger:** a `SlopDeskClient.Event.disconnected` (transport FIN/failure) that is not
///   the result of a deliberate ``SlopDeskClient/pause()``/``close()``.
...
/// ``SlopDeskClient``; this type stores only immutable `let`s, so it is `Sendable`.
public final class ReconnectManager: Sendable {
    ...
    private let client: SlopDeskClient
