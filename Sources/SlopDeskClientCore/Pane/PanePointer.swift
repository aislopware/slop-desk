...
//
// The defect the value prevents is worse than the gate count. The pane divider's pointer
// (``SlopDeskMacUI/MacPaneDivider`` / ``SlopDeskPhoneUI/PaneDividerView``) carries a
...
package enum PanePointer: Equatable {
    ...
    /// travel — see ``SlopDeskMacUI/MacPaneDivider`` / ``SlopDeskPhoneUI/PaneDividerView`` — which the
    /// handle's pair weights answer, so the glyph can never
