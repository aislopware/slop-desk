...
package struct RailStructureKey: Equatable {
    ...
    /// fingerprint AND the window-title read (``WorkspaceChromePolicy/windowTitle(for:)``, which omits
    /// `projectKey` — no section header there, the folder name stays its title) so every title site
    /// registers the volatile process dict as an
