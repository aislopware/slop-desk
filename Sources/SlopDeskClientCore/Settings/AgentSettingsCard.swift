...
// (`AgentSettingsCardWiringTests` in `SlopDeskClientUITests`, plus the pure fallback pin in
...
// both static funcs return a plain enum / `Bool` — so it descended here from `SlopDeskClientUI` in
// batch 2 of the draining-floor split. What stayed behind is the `@Entry` environment slot
// (`EnvironmentValues.agentHooksController` / `View.agentHooksController(_:)`): an environment key IS
// SwiftUI, so the reader-and-writer-of-one-key pairing lives in `SlopDeskClientUI/Settings/AgentSettingsCard.swift`
// beside the views that inject and read it.

import SlopDeskWorkspaceCore

/// `package`: read from `SlopDeskClientUI` (`SettingsPages.swift`, and the environment slot's own file)
