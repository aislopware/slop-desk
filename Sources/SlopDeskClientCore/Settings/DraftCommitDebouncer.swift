...
@preconcurrency
@MainActor
package final class DraftCommitDebouncer {
    ...
    package init(idle: Duration = .milliseconds(500)) {
        self.idle = idle
    }
    ...
    package func schedule(_ commit: @escaping @MainActor () -> Void) {
        pending?.cancel()
    ...
    package func flush(_ commit: @MainActor () -> Void) {
        cancel()
        commit()
    }
