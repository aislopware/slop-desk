...
// from the old shared SwiftUI target in batch 2 of the draining-floor split (`SettingsSectionHeaderTests` moved with
...
// `SlopDeskPhoneUI/Settings/SettingsSectionHeader.swift`, which now carries only that half and says so.
/// `package`: read from `SlopDeskPhoneUI/Settings/SettingsSectionHeader.swift`'s `slateFormSection`
