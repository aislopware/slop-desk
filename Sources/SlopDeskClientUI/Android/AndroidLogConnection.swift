...
@MainActor
protocol AndroidLogStreaming: AnyObject {
    func connect(host: String, port: UInt16, serial: String, level: AndroidLogLevel)
    func disconnect()
}

@MainActor
final class AndroidLogConnection: AndroidLogStreaming {
    ...
    static let lineLimit = 1 << 16

    init(sink: @escaping (AndroidLogEvent) -> Void) {
        self.sink = sink
    }
