...
struct DeviceLogLine: Identifiable, Equatable {
    ...
    var name = ""
    var message = ""
    var severity: DeviceLogSeverity = .plain

    /// One `logcat -v time` line.
    static func logcat(_ text: String) -> Self {
        parse(text, through: slopdesk_logcat_parse)
    }

    /// One `log stream --style compact` line.
    static func unified(_ text: String) -> Self {
