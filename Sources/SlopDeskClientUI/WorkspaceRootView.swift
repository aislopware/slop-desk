...
public struct WorkspaceRootView: View {
    ...
    /// The iOS ``sidebarColumnVisibility`` SETTER side — the mapping from the split's visibility to
    /// the shared collapsed flag. The DECISION (recording a manual override so the auto-hide policy
    /// stops fighting the user) is ``WorkspaceChromePolicy/applySidebarCollapsed(_:chrome:)``; what is
    /// left here is the two-column shell's reading of the visibility itself: `.detailOnly` is the only
    /// "sidebar hidden" value, since `.doubleColumn` shows BOTH columns of a two-column split.
