...
// `SlopDeskSplitViewController` (an `NSSplitViewController` with sidebar | content items,
...
public struct WorkspaceRootView: View {
    ...
    /// representable). OWNED BY THE APP (`SlopDeskClientApp` `@State`) and passed in — NOT view-local
    ...
    // (`SlopDeskClientApp`), and `chrome` is the internal `WorkspaceChromeState`. Narrowing the init keeps
