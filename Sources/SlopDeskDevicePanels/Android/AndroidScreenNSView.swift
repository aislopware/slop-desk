...
// touch.
//
// It was the macOS half of what is now `Sources/SlopDeskPhoneUI/Panel/Android/PhoneAndroidScreenView.swift`, until docs/56 stage D
...
// its only reason to be up there was the `NSViewRepresentable` wrapper that mounted it. The AppKit
// panel mounts the view directly, so the wrapper is gone and this file DELETES an import edge rather
// than adding one — it used to `import SlopDeskDevicePanels`, and it is inside it now.
//
...
// mount and is fed from the socket directly; no view framework is in the path on either platform.
//
...
import CoreMedia
import Foundation
import SlopDeskVideoProtocol // InputModifiers — the vocabulary the key maps already speak

/// A plain `NSView` hosting the display layer, plus the mouse/scroll/key handling — a gesture
/// recognizer cannot express "report every intermediate point of a drag at the rate they arrive",
/// which is precisely what a touch sequence is.
@MainActor
package final class AndroidScreenNSView: NSView, AndroidFrameRenderer {
    /// Where a control message goes. Nil until the mount sets it, so an early click is dropped rather
    /// than queued against a device that may not be the one finally selected.
    package var send: ((Data) -> Void)?
    ...
    package var videoSize: CGSize = .zero {
        didSet {
    ...
    override package func rightMouseDown(with event: NSEvent) {
    ...
    private let scroll = AndroidScrollGesture()
