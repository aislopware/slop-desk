...
/// `just provision` run fixes it mid-session), `.starting` is a server the host is
/// booting or probing, and verb 21 is the round that answers.
package typealias SimulatorSidebarPhase = DevicePanelPhase

@MainActor
@Observable
package final class SimulatorSidebarModel {
    // MARK: Observable state

    package private(set) var phase: SimulatorSidebarPhase = .starting
    package private(set) var devices: [SimulatorDevice] = []
    /// The UDID whose stream is live, or `nil` for the device list.
    package private(set) var selection: String?
    ...
    package let frames = SimulatorFrameSink()
    ...
    package private(set) var chrome: SimulatorChromeBundle?
    ...
    package private(set) var orientation: SimulatorOrientation = .portrait
    /// Whether the demo status bar (9:41, full bars, full battery) is in force.
    package private(set) var isStatusBarOverridden = false
    ...
    package private(set) var isSendingFile = false
    ...
    /// nobody has checked.
    package private(set) var resolution: CGSize?
    ...
    package private(set) var pinnedLocation: SimulatorCoordinate?
    ...
    package private(set) var isAwaitingStream = false
    ...
    /// subscribed.
    package private(set) var isConsoleOpen = false
    package private(set) var logLevel: SimulatorLogLevel = .info
    package private(set) var logLines: [DeviceLogLine] = []
    ...
    package private(set) var isLogStarted = false
    ...
    private var chromeCache: [String: SimulatorChromeBundle] = [:]
    ...
    package static let firstFrameDeadline: Duration = .seconds(5)
    ...
    package func boot(_ udid: String) async {
        await act(udid) { try await control.boot(host: $0, port: $1, udid: udid) }
    }

    package func shutdown(_ udid: String) async {
    ...
    package func select(_ udid: String?) {
    ...
    package func rotate(_ direction: SimulatorOrientation.Turn) async {
    ...
    package func copyScreenshot(of udid: String? = nil) async {
        ...
        do {
            ...
            // success.
            guard ClientPasteboard.writeImage(jpeg) else {
    ...
    package func thumbnail(for udid: String) async -> Data? {
    ...
    package static let thumbnailCadence: Duration = .seconds(2)
    ...
    package func send(file url: URL, contents: Data) async {
    ...
    package func pin(_ coordinate: SimulatorCoordinate?) async {
        guard case let .ready(host, port) = phase, let udid = selection else { return }
        do {
    ...
    package func observed(resolution size: CGSize) {
    ...
    /// throw away.
    package func setLogLevel(_ level: SimulatorLogLevel) {
    ...
    /// target.
    package func report(_ text: String) {
        notice = nil
        noticeClear.cancel()
        failure = text
    }
    ...
    private func loadChrome(for udid: String) {
        ...
        chromeLoad = Task { [weak self] in
            let assets = await SimulatorChromeBundle.load(
