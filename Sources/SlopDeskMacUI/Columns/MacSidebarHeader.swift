...
final class MacSidebarHeaderView: NSView {
    ...
    func refresh() {
        headerFollow = ObservationFollow.arm(self, replacing: headerFollow) { header in
            ...
            header.git.isHidden = header.git.isEmpty
    ...
    override func menu(for _: NSEvent) -> NSMenu? {
        ...
        let menu = NSMenu()
        let item = NSMenuItem(
            title: NavigatorChromeCopy.refreshGitStatus,
            action: #selector(refreshGit),
            keyEquivalent: "",
        )
        item.target = self
...
///
/// The measuring is ``GitLineLadder``'s, one floor down and shared with the phone's
/// ``SlopDeskPhoneUI/SidebarGitLineView``: the ladder is `NSAttributedString` arithmetic, which both
/// platforms spell identically, so a second copy of it here would be forty-four lines of the same
/// sums (docs/56 §3, docs/62 stage H). What is left is what AppKit alone asks for — a flipped
/// coordinate space, `needsDisplay`, and the intrinsic height the ladder measured.
@MainActor
final class MacGitLineView: NSView {
    ...
    override var intrinsicContentSize: NSSize {
        NSSize(width: NSView.noIntrinsicMetric, height: ladder?.height ?? 0)
    }
