...
struct GlobalSearchView: View {
    ...
    var body: some View {
        ...
        // Esc for the iPad's hardware keyboard. This was written inline, on the reasoning that
        // ``View/slateCancelKey(perform:)`` "exists to carry the macOS responder-chain half" and this
        // card is the phone's — but increment 63 deleted that half with the rest of the AppKit arms,
        // and the modifier is now exactly the four lines that were being spelled here. The Mac's
        // counterpart is a `MacOverlayPanel` taking Esc through `cancelOperation(_:)`; that is a
        // different framework's mechanism, not a second arm of this one.
        .slateCancelKey { coordinator.closeGlobalSearch() }
    }
