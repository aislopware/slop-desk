...
struct SplitContainer: View {
    ...
    private func moveLayer(
        ...
        if leaves.count > 1 || paneDrag != nil {
            ...
            // so it never shadows Escape for anything else at rest. It used to be mounted
            // unconditionally because ``PaneMoveEscapeMonitor`` owned a platform gate of its own —
            // an `NSViewRepresentable` on one arm, a first-responder `View` on the other. Increment 63
            // took the AppKit arm with the rest of them, so there is one declaration now and nothing
            // left to restate. Escape and a gesture cancel are the SAME bail-out, so both are the
            // controller's one `interrupted()` rather than two spellings.
