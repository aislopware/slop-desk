// PhoneAndroidScreenView — the device's live frame, and the surface that turns a finger into a touch.
//
// THE WRAPPER IS WHAT LEFT. This was already a `UIView` doing its own drawing, its own touch handling
// and its own key handling — docs/62 §2 kind (c), "90%+ already UIKit" — under a
// `UIViewRepresentable` whose whole job was to hand it a send closure and a video size. The stage sets
// both directly now, and `dismantleUIView`'s two lines are ``unmount()``.
//
...
// the simulator panel and for the same reason — this needs one rectangle at panel size, so the simpler
// API is not a shortcut, it is the whole feature.
//
// FRAMES DO NOT ARRIVE AS STATE. The view registers itself with the model's ``AndroidFrameSink`` on
// mount and is fed from the socket directly; nothing observable is in the path. That was true under
// SwiftUI and it is why this half needed no rewrite: the reason it was written this way — 25.3 frames
// a second under a drag, each one otherwise rebuilding header, toolbar, body and drawer — is a fact
// about the frame rate, not about the framework.
//
// THE INPUT HALF IS THE PHONE'S, not a port of the Mac's. A finger on the mirror IS the finger, so the
// three machines the Mac needs to SYNTHESIZE one — the scroll gesture, the classic wheel's idle timer,
// and the magnify-gesture-to-two-contacts translation — have nothing to do here and are absent rather
// than reimplemented. What survives is everything that is about the DEVICE rather than about the
// pointer: the clamped drag, the pinch's rate limit, and the pointer-index discipline.
//
// ⚠️ A POSITIONAL MESSAGE CARRIES THE DEVICE'S OWN PIXEL GRID, paired with the exact size the server
// says it is encoding — `scrcpy`'s `PositionMapper` DROPS a mismatched pair rather than rescaling it
// (`docs/48`). Every touch here goes through ``AndroidScreenLayout/Surface``, which is the one place
// that pairing is made, and which resolves the arithmetic through the FFI door rather than in Swift.
//
...
@MainActor
final class PhoneAndroidScreenView: UIView, AndroidFrameRenderer {
    /// Where a control message goes. Nil until the stage sets it, so an early touch is dropped rather
    /// than queued against a device that may not be the one finally selected.
    var send: ((Data) -> Void)?

    /// Drawing geometry, from the decoder's reading of the bitstream. Kept apart from ``videoSize``
    /// because they can legitimately differ: the decoder reads the bitstream's own dimensions, while
    /// the session packet states what the server BELIEVES it is encoding, and a positional message may
    /// only ever be paired with the latter.
    ...
    /// ⚠️ THE LAYER IS PLACED BY FRAME, which is docs/62 §3.3's one exception and this view is it: the
    /// fitted rectangle is a function of two sizes neither of which Auto Layout can see, and a
    /// constraint pair recomputed per rotation would be the same arithmetic with a solver in front of
    /// it.
    ...
    ///
    /// The send closure goes with them, which is the second half of what the deleted representable's
    /// `dismantleUIView` did. A view kept alive for one animation beat past its device must not be able
    /// to write into the next device's socket.
    func unmount() {
        isPinching = false
        send = nil
    }
...
extension PhoneAndroidScreenView: DeviceSoftKeyboardHost {}
#endif
