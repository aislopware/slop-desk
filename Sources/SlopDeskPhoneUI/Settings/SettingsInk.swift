...
// Settings is a modal sheet full of native `Form` / `Toggle` / `Picker` / `Stepper` controls: those draw
// themselves from the OS appearance and the system tint no matter what, so a theme-tinted label beside a
// system-blue switch reads as two apps in one sheet. A settings surface is OS chrome, not product
// surface — it should look like the Settings app.
//
// So every ink here comes from UIKit semantic colours (which already track light/dark and increased
// contrast), and every font is a Dynamic-Type text style rather than a fixed point size.
// `Slate.Metric.*` (the 8pt grid, radii, the height ladder) is theme-free geometry and stays.
//
...
enum SettingsInk {
    ...
    /// The system tint — the SAME blue the native `Toggle` and `Picker` in the next row are already
    /// using (the sheet resets its tint, so this is the stock one, not the terminal theme's accent).
    ...
    static var inset: Color { Color(uiColor: .secondarySystemBackground) }
    ...
    static var face: Color { Color(uiColor: .systemBackground) }
...
enum SettingsMetric {
    ...
    static var resolvedBodyPointSize: CGFloat { UIFont.preferredFont(forTextStyle: .body).pointSize }
}
#endif
