...
struct SettingsSheet: View {
    ...
    /// A local selected-section state ONLY to satisfy the All-Settings ✎ jump binding. In a pushed
    /// navigation stack a jump cannot repoint a `TabView` (there is none) — the cross-section
    /// jump/highlight is deferred (a known follow-up), so writing this is a harmless no-op.
