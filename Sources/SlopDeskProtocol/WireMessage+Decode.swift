...
extension WireMessage {
    ...
    ///
    /// - Throws: ``SlopDeskError/truncated`` if the body is shorter than the type
    ///   requires, ``SlopDeskError/unknownMessageType(_:)`` for an unrecognized type
    ///   byte, or ``SlopDeskError/malformedBody(_:)`` for a right-length-but-invalid
    ///   body (e.g. bad UTF-8).
    ///
    ...
    /// UTF-8 (an invalid sequence throws ``SlopDeskError/malformedBody(_:)``, never a
    ...
    static func decode(payload: Data) throws -> WireMessage {
        ...
        case 10: // hello
            ...
            guard let sessionID = UUID(dataBytes: idBytes) else {
                throw SlopDeskError.malformedBody("hello: invalid sessionID bytes")
            }
        ...
        case 20: // helloAck
            ...
            guard let sessionID = UUID(dataBytes: idBytes) else {
                throw SlopDeskError.malformedBody("helloAck: invalid sessionID bytes")
            }
            return .helloAck(
                sessionID: sessionID,
        ...
        case 21: // title
            ...
            guard let string = String(data: bytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("title: invalid UTF-8")
            }
            return .title(string)
        ...
        case 23: // commandStatus
            ...
            default:
                throw SlopDeskError.malformedBody("commandStatus: invalid tag \(tag)")
            }
        ...
        case 25: // notification
            ...
            guard let title = String(data: titleBytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("notification: invalid title UTF-8")
            }
            guard let body = String(data: bodyBytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("notification: invalid body UTF-8")
            }
        ...
        case 26: // foregroundProcess
            ...
            guard let name = String(data: bytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("foregroundProcess: invalid UTF-8")
            }
        ...
        case 27: // claudeStatus
            ...
            guard let label = String(data: labelBytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("claudeStatus: invalid label UTF-8")
            }
        ...
        case 28: // commandBlock
            ...
            guard let commandText = String(data: cmdBytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("commandBlock: invalid commandText UTF-8")
            }
            return .commandBlock(
                index: index,
        ...
        case 33: // cwd
            ...
            guard let path = String(data: bytes, encoding: .utf8) else {
                throw SlopDeskError.malformedBody("cwd: invalid UTF-8")
            }
            return .cwd(path)
