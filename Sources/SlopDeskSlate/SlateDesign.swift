...
//
// A THIN, headless token layer — `SlateNativeColor`/`SlateNativeFont`/`CGFloat`/`SlateCurve` constants
// and nothing that draws. It compiled into `SlopDeskClientUI` for as long as there was ONE UI target to
...
//
// SwiftUI is gone from this floor entirely (user-directed, whole-codebase SwiftUI removal): every
// token that used to carry a `Color`/`Font`/`Animation` spelling ALONGSIDE its `SlateNativeColor`/
// `SlateNativeFont`/`SlateCurve` one now carries only the native spelling — the AppKit half never
// stopped being the one both frameworks were converging on, and there is only one framework left to
// read it. A handful of scalars (``Slate/Anim/ackSpeed`` and its neighbours) were never SwiftUI in the
// first place — a `Double`/`CGFloat` has no framework in it — and those survive untouched, read
// directly by AppKit's `CABasicAnimation` timings and UIKit's `symbolEffect` calls.
//
...
/// The platform's own colour type — what a token IS.
///
/// Every colour in this file is built out of one of these (a semantic system colour, a dynamic
/// light/dark pair, or a hex). Naming the wrapped type lets the AppKit half of the client (docs/56
/// stage D — an `NSView` fills with an `NSColor`, never a view-framework wrapper) read the SAME
/// constant the token layer declares, instead of a second palette that drifts.
...
package struct SlateTheme: Equatable, Sendable {
    ...
    /// (and the two derivations beside it) rather than stored as a resolved ``SlateNativeColor`` pair,
    /// so a profile stays one set of hexes with every derived rung read out of it on demand — see
    /// ``Slate/Native``.
    package let glass: GlassSet

    // The glass surfaces — read as ``SlateNativeColor`` via ``Slate/Native/Terminal``, which pulls
    // straight off ``glass`` (face/ink/ink2/edge/accent) and off ``terminalRimHex``/``terminalOkHex``/
    // ``terminalErrHex`` below; there is no stored `terminal` rung here because `glass.face` already
    // IS it.
    /// The RIM around a lifted plate ON the glass — one step BRIGHTER than the plate, never equal to
    /// it. Its own token because ``glass``'s `edge` is a SEAM (a line drawn between two things that
    ...
    /// be LIGHTER than it — the inverse of the light side's rule (``Slate/Native/Line/overlayRim``),
    /// which is darker than its cream — so this is derived by lifting the plate toward the profile's
    /// own comment ink rather than by inventing a hex.
...
package enum Slate {
    ...
    /// The two appearance polarities this app's chrome ever chooses between — the native replacement
    /// for SwiftUI's `ColorScheme` now that SwiftUI is gone from this floor. Spelled as its own type
    /// (rather than inlining `.light`/`.dark` at each call site) for the reason ``glassColorScheme``
    /// and ``chromeColorScheme`` already existed as named rungs instead of bare booleans: a polarity is
    /// a DECISION, and every place that reads one should say so in a token. `NSAppearance.Name` and
    /// `UIUserInterfaceStyle` are each already dynamic-pair mechanisms of their own
    /// (``SlateNativeColor/slateDynamic(light:dark:lightAlpha:darkAlpha:)``); this type is only the
    /// coarse "which one" a whole subtree is pinned to.
    package enum Polarity: Sendable {
        case light
        case dark
    }
    ...
    @MainActor package static var glassColorScheme: Polarity { .dark }
    ...
    ///
    /// There is exactly one such subtree: the pane canvas's transient notice capsule. It is cream —
    /// ``Native/Surface/field``, a FIXED tone that does not follow any appearance — but it is mounted
    /// on the pane canvas, INSIDE the island subtree that ``glassColorScheme`` has forced dark. Its
    /// ink comes from ``Native/Overlay``, which derives from `labelColor`/`secondaryLabelColor` (or
    /// their UIKit equivalents), so without this it resolved for the dark well and drew WHITE ON
    /// CREAM: a capsule with the right surface and no readable text (caught in the
    /// `testRenderIslandChips` snapshot, 2026-08-11, before it ever shipped).
    ///
    ...
    @MainActor package static var chromeColorScheme: Polarity { .light }

    /// THE TOKEN VALUES, in the platform's own colour type — ``SlateNativeColor``, `NSColor` on the
    /// Mac and `UIColor` on the phone.
    ///
    /// This namespace name is a leftover of the SwiftUI era, kept rather than flattened into `Slate`
    /// directly: every AppKit call site across the client already reads `Slate.Native.Surface.field`,
    /// `Slate.Native.Text.primary` and so on, and collapsing the indirection now would be a rename
    /// sweeping ~400 call sites for a cosmetic win. `Native` no longer means "the non-SwiftUI half of
    /// a pair" — there is no other half left — it just means "the token layer", and every rung below
    /// (``Surface``, ``Text``, ``Line``, ``State``, ``Status``, ``StatusInk``, ``Terminal``) is the
    /// ONLY spelling of that token now, not one of two.
    ///
    ...
    /// SOURCES that only ever composites through ``ProjectTint/bed(at:)``.
    @MainActor
    package enum Native {
        /// The brand accent (Dracula purple): Alucard's `#644AC9` on light appearances, the Pro
        /// `#9580FF` on dark. The ONLY chrome colour that is not the system's — user-directed
        /// 2026-08-07 (fixed brand accent over the user-configurable system accent; purple replaced
        /// the Ember teal in the round-8 Dracula verdict). The two hexes are spelled ONCE, here.
        ...
        /// `void` (aux-window backdrops) → `ground` (sidebar housing; on macOS the real sidebar
        /// material sits BEHIND the column and this is its fallback) → `face` (the window content
        /// ground) → `raised`/`lift` (the system fill ladder) → `terminal` (the island glass —
        /// profile-driven).
        ///
        ...
        package enum Surface {
            ...
            /// ⚠️ NOT the app's ground — see the ladder's note above; you almost certainly want
            /// ``field``.
            ...
            #endif
            /// THE GROUND — the one sunken tone every column paints: the navigator, the code panel,
            /// the top band and the island's moat (law 1: they SINK, they are not islands). Kept under
            /// its old name because the eight column call sites mean exactly this; ``island`` is its
            ...
            /// THE ISLAND — the terminal canvas, the one lifted surface (law 1). Equal to ``terminal``
            /// by construction; spelled separately so the island's own geometry reads by intent.
        ...
        package enum Terminal {
            ...
            /// The RIM of a floating plate ON the glass (the connection chip) — LIGHTER than the
            /// plate, unlike ``edge``, which is the same tone as it. See ``SlateTheme/terminalRimHex``:
            /// a border that matches its own fill is not a border.
            ...
            /// The status pair ON the glass — the profile's own green / red (``SlateTheme/terminalOkHex``,
            /// ``SlateTheme/terminalErrHex``). Anything drawn inside the island that has to say "clean"
            /// or "failed" reads THESE, never ``Status`` — that set is the system's, tuned for the OS
        ...
        /// The text tiers — a ladder MEASURED against this app's own ground rather than inherited
        /// whole from the system's.
        ///
        /// Apple's label alphas are tuned for a sidebar drawn on a VIBRANT material, which lends its
        /// labels contrast the flat ground here cannot: this chrome paints one opaque cream (there is
        /// no `NSVisualEffectView` anywhere under the columns), so the tiers landed on the ground at
        /// 14.5 : 3.9 : 1.9 — the second rung under the 4.5 reading floor, the third under even the
        /// 3.0 floor for non-text, while carrying real data (the process label, the branch name, a
        /// command's duration, and the host line exactly when it says DISCONNECTED). Legibility,
        /// user-reported 2026-08-08. Measured on white as a control: 3.95 : 1.88, so the cream was
        /// never the cause.
        ///
        ...
        ///
        /// ⚠️ The LIGHT rungs are flat hexes; the dark side is NOT a free system fallback. Two
        /// subtrees flip to the glass polarity (the selected row's plate in ``MacSidebarRowView``, the
        /// pane chrome inside the terminal island), and a light-pinned hex would draw dark-on-dark
        /// there — which is why `secondary` still defers to the system tier on that side
        /// (`secondaryLabel` composites to 5.76 on the glass face and needs no help).
        ///
        ...
        package enum Text {
            ...
            // NO `onAccent`/`onWarn` rung — see the ``Native`` header above: ink on a saturated,
            // appearance-INDEPENDENT fill (the secure/sync pills, the amber hazard plate) is pinned
            // white/black, so a caller reaches ``SlateNativeColor/white``/`.black` directly instead of
            // a token that would only ever hold one of two constants.
        }
        ...
        package enum Line {
            ...
            /// A FLOATING SURFACE's rim — the toast card, every summoned paper card, the sheet. Its
            /// own token, and NOT ``divider``: the system separator measures ~1.25 : 1 on this cream,
            /// which is a rule between two visible things, not the edge of an object that has to read
            /// as LIFTED off the ground it covers. A notification card is exactly that object, and it
            /// was reported as having no visible border at all (2026-08-10).
            ///
            ...
            /// (``Terminal/rim`` lifts the dark plate toward its ink). One idea, spelled once per
        ...
        /// The FLOATING (overlay) family's palette: system-semantic, neutral, theme-INDEPENDENT — the
        /// switcher, the palette, Open Quickly, global search, the cheat sheet, Connect, peek-reply
        /// and the toast stack. Every value derives from the platform label colour or the system
        /// accent, so it is a true grey on both appearances and repoints itself when the appearance
        /// changes — without ever reaching into `Slate.theme`, which is the terminal's filter and
        /// belongs to the workspace.
        ///
        /// Its four derived rungs are alphas over the platform LABEL colour, so they resolve against
        /// whichever polarity the card stands in without a call site changing. Both platforms read
        /// this table directly now: the Mac's cheat sheet is an `NSView` (docs/56 stage D) and the
        /// phone's own view reads the same values, and the ladder they read is ONE value, once.
        @MainActor
        ...
        package enum State {
            ...
            #endif
            /// The brand accent, natively — see ``Native/accent``. Its own rung here (rather than
            /// leaving every AppKit caller to climb back out to the enclosing namespace) because
            /// ``selected``/``accentMuted`` already derive from it in this same enum.
            package static let accent: SlateNativeColor = Native.accent
            ...
            /// The SUMMONED card's cast shadow — twice ``shadow``, and its own rung because it does
            /// twice the work. A panel that floats over the dark island is separated by tone alone; a
            /// paper card is the ground's own cream lifted off the ground, so nothing but the cast
            /// tells the two apart at the card's edges. Compared side by side at true size, `shadow`
            /// read as a halo and this reads as lift.
        ...
        /// The status FILLS — marks and words take ``StatusInk`` instead.
        @MainActor
        package enum Status {
            #if canImport(AppKit)
            ...
            /// FIXED security-blue — appearance-INDEPENDENT: the secure-input pill must read as the
            /// SAME vivid royal-blue everywhere so it can never be confused with the accent. Pinned to
            ...
            /// FIXED sync-amber — same rationale as ``secureInput``: the `⚠ SYNC INPUT` pill flags a
            /// MODE where every keystroke fans into multiple shells, so it must read as the same
            /// unmistakable amber everywhere. Never re-route this through a theme or the system
            /// palette.
        ...
        ///
        /// `Status.ok`/`warn`/`err` are the system palette, and the system palette is tuned for dark
        /// UI and for filled controls. Measured as ink on this chrome's cream (`#FFFBEB`) they land at
        /// **2.05** (systemGreen) and **2.12** (systemOrange) — under even the 3.0 non-text floor,
        /// while ``Text/tertiary``, the rung whose whole job is to be ignorable, measures **5.16**.
        /// The rail was spending its loudest vocabulary on its faintest ink: a `+3` staged count was
        /// two and a half times quieter than the `zsh` label beside it (user-reported 2026-08-10).
        ///
        /// Two more faults died with it. `Status.warn` and the retired `Chroma.orange` were BOTH
        /// `systemOrange`, so the git line's documented green→yellow→orange→red ramp rendered
        /// `!modified` and `?untracked` in one identical colour — a four-rung ramp with three rungs.
        /// And `info` (the accent) sat 12.6° from `Chroma.purple` in Lab hue, so `↑↓` and `$` were
        /// near-indistinguishable too.
        ///
        /// ## How this set is built
        ///
        /// Six hue angles, solved ISO-LIGHTNESS on each side — one L\*, maximum in-gamut chroma at
        /// that L\* for every angle. Iso-lightness is the whole point: equal contrast BY CONSTRUCTION,
        /// so no run can shout over another by accident and hue is left to do the only job it is good
        /// at, which is naming WHICH state this is.
        ///
        ...
        /// vivid system orange behind black ink, and darkening it would only muddy the plate — a
        /// colour tuned for a fill is the wrong colour for a mark, and the reverse.
        ///
        /// ⚠️ NOT for anything inside the terminal island — `ok`/`err` there are the profile's own
        /// ANSI pair (``Terminal/ok``, ``Terminal/err``), because a surface that ships its own palette
        /// must answer in it.
        ///
        /// Re-solve BOTH sides if the cream, the deepest bed (so: ``Slate/Opacity/bed``) or the glass
        /// face moves — each side is pinned to its own ground, and neither is a free system fallback.
        @MainActor
        ...
        /// `ViewThatFits` where AppKit measures the ladder by hand. There is no `Slate.gitInk(_:)`
        /// twin beside `attentionInk`/`agentInk` — every caller, on either platform, reads this rung
        /// directly; a second entry point is worth minting when a second SHAPE of value is needed, and
        /// none has ever been.
        ///
        ...
        /// One ATTENTION role, as ink — the role is ``TabBadgeReading``'s (which badge means what, and
        /// which of them outranks which); the hue is this ladder's.
        ///
        /// ⚠️ A ring 10 pt across is the thinnest thing in the rail that carries state, so it is
        /// exactly where a hue tuned for filled controls fails worst (systemGreen measures 2.05 on
        /// this cream) — hence ``StatusInk``'s solved angles rather than ``Status``'s system fills.
        /// The system palette was tried for the MARK COLUMN ALONE on 2026-08-11 (user-directed) and
        /// REVERTED the same day on hardware; see `docs/DECISIONS.md`. Do not re-propose it without a
        /// different ground.
        ...
        /// One AGENT's state, as ink — and the reason ``AgentInk`` exists as a value at all. The
        /// mapping from a `ClaudeStatus` to a MEANING is ``AgentReadout/ink(_:)``, one floor down and
        /// framework-free; this is the rung that meaning lands on.
        ///
        /// ⚠️ `thinking` and `awaiting` land on the SAME warm rung deliberately (see
        /// ``StatusPresentation/thinkingMark`` for the measurement that put them there). What
        /// separates them is the silhouette and the motion — a still hand against a block of dots
        /// with a hole running round it — never the hue.
        ...
        /// primary. NO hue: a row of metric digits has nothing to hang a palette on, and an instrument
        /// that lights a different colour per fault asks the eye to learn one before it can read a
        /// number.
        ...
        /// The connection alarm's weight — the second channel carrying the same rungs. A brightness
        /// step alone is easy to lose beside a hostname; the weight step is what makes a raised reading
        /// findable without looking for it.
        ///
        /// Its own switch rather than a shared body with anything else: ``SlateNativeFont/Weight`` is
        /// the only font-weight type left on this floor, the same split ``Slate/PaneStatusPillArt``
        /// leaves a pill's glyph weight in (one name below).
        ...
        /// The pane status pill's fill. The vivid tones are theme-INDEPENDENT on purpose (the shipped
        /// themes have `info == accent`, so a palette-derived security badge would be invisible against
        /// the accent — `secure-input.png` is the green-accent Paper theme yet the pill is the same
        /// royal blue), and only a NAME can say that — ``PaneStatusPillInk`` is that name, one floor
        /// down and colour-free.
        ...
        /// One TOAST rung, as ink. `.warn` is AMBER, not the theme accent (see ``ToastMarkRung/warn``):
        /// the rail already fixed "amber = a question waiting", and every FOUNDRY seed sets
        /// `info == accent`, so the accent would have drawn needs-input in the same cyan as a routine
        /// notice. `.neutral` stays the family's own reading ink — status hues keep their meaning, and
        /// a routine notice stays NEUTRAL.
        ...
        /// The project BEDS — the five register sources composited at ``Opacity/bed``, reached by the
        /// index a ``Slate/ProjectTint/Deal`` dealt.
        ///
    ...
    package enum ProjectTint {
        ...
        /// The five sources as HEXES — spelled ONCE, because both platforms read them through the
        /// same ``Slate/Native/ProjectTint/bed(at:)``, and a bed dealt to one project may not be a
        /// different colour on the two devices.
        package static let registerHexes: [UInt32] = [0x00A68F, 0x0075F7, 0x514AF8, 0xF414F7, 0xF854A4]

        /// The keyless "Other" bucket's bed source. It is ``Slate/Native/Text/secondary``'s light pin
        ...
        package struct Deal {
            ...
            /// Reads straight through ``Slate/Native/ProjectTint/bed(at:)``, which already carries
            /// this exact guard — there is no second copy of it here.
            @MainActor
            package subscript(position: Int) -> SlateNativeColor {
                Slate.Native.ProjectTint.bed(at: indices.indices.contains(position) ? indices[position] : nil)
            }
        }

        /// The register size, readable without `@MainActor` (``Deal`` runs the arithmetic off the
        /// colour values). Pinned by `SlateProjectTintTests` to match ``registerHexes``'s own count.
    ...
    package enum Metric {
        ...
        // Renderers pick a rung, never a raw height literal (the `design-token-leaks` gate enforces it).
    ...
    package enum Typeface {
        ...
        /// the PROPORTIONAL system face silently (no mono at all), so ``instrumentNative(_:weight:)``
        /// degrades to `.monospacedSystemFont` instead. Checked once: fonts don't appear mid-session.
        ...
        /// The instrument voice, natively — the same family, the same installed check, the same SF
        /// Mono fallback, for every caller now that this floor has one framework's font type left.
        ///
    ...
    /// floats, ``State/overlayShadow`` for a summoned card) via `CALayer.slateShadow(_:color:in:)`.
    ///
    ...
    /// THE MOTION VALUES — a curve and a duration, framework-neutral: ``SlateCurve/timingFunction``
    /// is the CoreAnimation view of a rung, for a `CAAnimation`'s `timingFunction` or an
    /// `NSAnimationContext`'s (`NSSplitViewItem.animator()` needs exactly this). ``Anim`` below reads
    /// the same rungs' raw `duration`s (and its own few scalars that were never curves at all —
    /// ``Anim/ackSpeed`` and its neighbours) directly, since there is no animation framework left on
    /// this floor to build a second, framework-typed view for.
    package enum Motion {
        ...
        /// demonstrates blinking (the cursor preview). Autoreversing forever is each renderer's own
        /// job (`CABasicAnimation.autoreverses`/`repeatCount` on AppKit, the UIKit equivalent on the
        /// phone) — this rung is only the curve and the duration of one half-cycle.
        package static let pulse = SlateCurve(0.42, 0, 0.58, 1, duration: 0.55)
    }

    /// The scalar tuning knobs beside ``Motion``'s curves — never `Animation`-typed even before this
    /// floor dropped SwiftUI, because a `Double`/`CGFloat` has no framework in it. What USED to live
    /// here alongside them was a same-named `Animation` per ``Motion`` rung (`Anim.standard`,
    /// `Anim.reveal`, and so on) for SwiftUI callers to spend directly; every one of those is gone
    /// now, and every remaining caller reads ``Motion``'s curves/durations directly instead
    /// (`Slate.Motion.promptFlash.timingFunction` on a `CABasicAnimation`, `Slate.Motion.columnSlide
    /// .duration` for a delay). What is left below never had an `Animation` spelling to lose.
    package enum Anim {
        /// How fast THE acknowledgement plays — a symbol bounce runs long by default and a click has
        /// to feel answered, not performed. Read by `SlateControls.swift` and `SlateKit.swift`
        /// (`glyph.addSymbolEffect(.bounce.down, options: .speed(Slate.Anim.ackSpeed))`), and lives
        /// here rather than at either call site because the effect is the app's, not any one
        /// button's.
        package static let ackSpeed: Double = 1.4
        /// The opacity the prompt-jump landing flash CUTS ON at, before it decays to zero (see
        /// ``Motion/promptFlash``). Loud enough to catch a saccade, quiet enough to read as light
        /// rather than as a selection band.
        ///
        ...
        /// How long the prompt-jump landing flash's rects stay MOUNTED — the fade, PLUS the beat that
        /// unmounts it.
        ///
        ...
        /// different clocks — a `CABasicAnimation`'s and a `Task.sleep`'s — which will not agree to
        /// the frame.
        ///
        ...
        /// the emphasized curve (``Motion/columnSlide``) and a beat more than ``Motion/stackReflow``
        /// to keep the terminal's re-wrap from reading as a snap. Anything that has to LAND with the
        /// column (the titlebar strip arriving as the sidebar leaves) delays by this much.
...
/// A NAMED MOTION — a cubic Bézier and a duration, framework-neutral: what CoreAnimation actually
/// takes is control points plus a `duration` on the context, which is exactly the value below.
/// Spelling the rung once and deriving ``timingFunction`` from it is the same move ``Slate/Native``
/// makes for colour, and for the same reason — every renderer must animate on the app's curve
/// without re-typing its numbers.
...
package extension SlateNativeColor {
    ...
    /// here so every reader gets the same object rather than a separately-built copy of the same two
    /// hexes.
    ...
    /// ``Slate/Native/Text`` for why the light rungs left the system ladder in the first place.
    ...
    /// The same colour at a FRACTION of its own alpha — a MULTIPLY on the existing alpha, and NOT
    /// what `withAlphaComponent(_:)` does (that one REPLACES the alpha, which turns the system
    /// separator — already a low-alpha black — into a solid rule). Stays dynamic: the scale is
    /// applied to whatever the appearance resolves, not to one snapshot of it.
