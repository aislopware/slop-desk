...
/// *not* a ``SlopDeskProtocol/MuxFrame`` — it is raw bytes the transport peels off first.
///
/// ```
...
private extension UUID {
    /// The UUID's 16 raw bytes in canonical order (same layout `SlopDeskProtocol` uses).
