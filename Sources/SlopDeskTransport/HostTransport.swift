...
public actor HostTransport {
    ...
    public func start(
        ...
        let resolvedPort: UInt16 = try await withCheckedThrowingContinuation { continuation in
            ...
            let timeoutTask = Task {
                ...
                box.tryResume {
                    continuation.resume(throwing: SlopDeskTransportError.timedOut("listener readiness"))
                }
            }
            listener.stateUpdateHandler = { state in
                ...
                case .cancelled:
                    ...
                    timeoutTask.cancel()
                    box.tryResume {
                        continuation.resume(throwing: SlopDeskTransportError.listenerFailed("cancelled during start"))
                    }
            ...
            do {
                try await withThrowingTaskGroup(of: Void.self) { group in
                    ...
                    group.addTask {
                        ...
                        throw SlopDeskTransportError.timedOut("host handshake")
                    }
    ...
    private func handshake(_ connection: NWConnection) async throws {
        let connQueue = DispatchQueue(label: "slopdesk.host.conn")
        ...
        case ChannelAssociation.muxControlTag:
            ...
            guard let connectionID = UUID(dataBytesForAssociation: idBytes) else {
                throw SlopDeskTransportError.handshakeFailed("mux control preamble: bad connectionID")
            }
            associateMux(connection, connectionID: connectionID, isControl: true)
        case ChannelAssociation.muxDataTag:
            let idBytes = try await connection.receiveExactly(ChannelAssociation.sessionIDByteCount)
            guard let connectionID = UUID(dataBytesForAssociation: idBytes) else {
                throw SlopDeskTransportError.handshakeFailed("mux data preamble: bad connectionID")
            }
            associateMux(connection, connectionID: connectionID, isControl: false)
        default:
            throw SlopDeskTransportError.handshakeFailed("unknown association tag \(String(describing: tag))")
        }
    }
