...
public final class NWMuxByteLink: MuxByteLink, @unchecked Sendable {
    ...
    public init(connection: NWConnection, label: String) {
        ...
        queue = DispatchQueue(label: "slopdesk.mux.link.\(label)")
    ...
    public func send(_ data: Data) async throws {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            connection.send(content: data, completion: .contentProcessed { error in
                if let error {
                    continuation.resume(throwing: SlopDeskTransportError.sendFailed(String(describing: error)))
                } else {
                    continuation.resume()
                }
            })
    ...
    public func sendPipelined(_ data: Data) {
        connection.send(content: data, completion: .contentProcessed { [chunkContinuation, connection] error in
            ...
            chunkContinuation.finish(throwing: SlopDeskTransportError.sendFailed(String(describing: error)))
            connection.cancel()
        })
    }
...
public enum LiveMuxConnectionFactory {
    ...
    /// the client's default `handshakeTimeout` (`SlopDeskClient.connect`, `.seconds(10)`).
    ...
    /// timeout is clean and the dead-host case surfaces a thrown ``SlopDeskTransportError/timedOut(_:)``
    /// instead of hanging. This is the "transport-level handshake deadline" the client UI asks for; it
    /// does NOT wrap `SlopDeskClient.connect` in a task group (which deadlocked the cooperative pool — see
    ...
    static func makeConnection(host: String, port: UInt16, timeout: Duration) async throws -> MuxNWConnection {
        try await withMuxConnectTimeout(timeout, host: host, port: port) {
            ...
            do {
                try await controlConn.startAndWaitReady(on: DispatchQueue(label: "slopdesk.mux.control.ready"))
                ...
                dataConn = data
                try await data.startAndWaitReady(on: DispatchQueue(label: "slopdesk.mux.data.ready"))
...
/// CANCELLED. On timeout, throws ``SlopDeskTransportError/timedOut(_:)``; the cancelled `body`'s
...
/// task group) — NOT the higher-level `SlopDeskClient.connect` — so it does not reintroduce the
...
private func withMuxConnectTimeout(
    ...
    try await withThrowingTaskGroup(of: MuxNWConnection?.self) { group in
        ...
        while let result = try await group.next() {
            ...
            throw SlopDeskTransportError.timedOut("connect to \(host):\(port) exceeded \(timeout)")
        }
