...
extension NWConnection {
    ...
    /// Throws ``SlopDeskTransportError/connectionFailed(_:)`` if it fails/cancels first.
    ///
            ...
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                stateUpdateHandler = { newState in
                    ...
                    case let .failed(error):
                        box.tryResume {
                            continuation
                                .resume(throwing: SlopDeskTransportError.connectionFailed(String(describing: error)))
                        }
                    case .cancelled:
                        box.tryResume {
                            continuation.resume(throwing: SlopDeskTransportError.connectionFailed("cancelled"))
                        }
                    default:
                        break
                    }
    ...
    func receiveExactly(_ count: Int) async throws -> Data {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Data, Error>) in
            receive(minimumIncompleteLength: count, maximumLength: count) { data, _, isComplete, error in
                if let error {
                    continuation.resume(throwing: SlopDeskTransportError.receiveFailed(String(describing: error)))
                    return
                }
                ...
                if isComplete {
                    continuation
                        .resume(throwing: SlopDeskTransportError.connectionFailed("peer closed during preamble"))
                    return
                }
                // Short read without completion shouldn't happen given min==count, but be safe.
                continuation.resume(throwing: SlopDeskTransportError.receiveFailed("short preamble read"))
            }
        }
    }
}
