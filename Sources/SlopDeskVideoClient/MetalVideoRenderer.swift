...
public final class MetalVideoRenderer {
    ...
    public init?(metalLayer: CAMetalLayer) {
        ...
        guard let library = try? device.makeLibrary(source: Self.shaderSource, options: nil),
              let vertexFunction = library.makeFunction(name: "slopdesk_video_vertex"),
              let fragmentFunction = library.makeFunction(name: "slopdesk_video_fragment")
        else {
            return nil
        }
        ...
        if dw > 0, dh > 0, width > 0, height > 0 {
            ...
            if Self.renderDiag, renderDiagCount == 1 || renderDiagCount % 120 == 1 {
                FileHandle.standardError
                    .write(
                        Data(
                            "SlopDesk[video.client]:   fit=\(fit.x)x\(fit.y) (rect=\(Int(r.size.width))x\(Int(r.size.height)) in dw×dh=\(Int(dw))x\(Int(dh)))\n"
                                .utf8,
                        ),
                    )
            }
    ...
    vertex VertexOut slopdesk_video_vertex(uint vid [[vertex_id]],
    ...
    fragment float4 slopdesk_video_fragment(VertexOut in [[stage_in]],
