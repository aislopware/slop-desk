...
public final class NWVideoMuxClientFlow: @unchecked Sendable {
    ...
    private let log = Logger(subsystem: "slopdesk.video.client", category: "NWVideoMuxClientFlow")
                    ...
                    if Self.dbgGapEnabled, channel == .video {
                        ...
                        if dbgLastVideoRxAt > 0, now - dbgLastVideoRxAt > 0.028 {
                            FileHandle.standardError
                                .write(
                                    Data("SlopDesk[video.client]: rx gap \(Int((now - dbgLastVideoRxAt) * 1000))ms\n"
                                        .utf8),
                                )
                        }
