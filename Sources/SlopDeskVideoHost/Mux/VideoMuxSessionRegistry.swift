...
/// N independent ``SlopDeskVideoHostSession``s — one per client video channel (per
...
/// ``SlopDeskVideoHostSession`` + capture/encode) is GUI-only and HANGS in a test — so it
...
public actor VideoMuxSessionRegistry {
    ...
    private var sessions: [UInt32: SlopDeskVideoHostSession] = [:]

    /// GUI-only factory: enumerate the window the hello names, build + start a
    /// ``SlopDeskVideoHostSession`` bound to a ``VideoMuxChannelTransport`` for `channelID` (whose
