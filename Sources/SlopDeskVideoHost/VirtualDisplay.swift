...
public struct VirtualDisplayGeometry: Equatable, Sendable {
    ...
    public func sizeInMillimeters(targetPPI: Double = 163) -> CGSize {
        // `max(1, targetPPI)` — the free `max`'s `y >= x ? y : x` with x = 1.0, i.e. NaN → 1.0
        // (ternary, NOT `(1.0).maximum(targetPPI)`, which would propagate NaN). Matches the core.
        let ppi = max(1, targetPPI)
        return CGSize(
            // keep mul+add separate — FMA breaks bit-exact parity
            // (here `/ ppi * 25.4` left-to-right; no add, but pin op order for parity).
...
public enum VirtualDisplayPlanner {
    ...
    public static func chipPixelLimit(cpuBrand: String) -> Int {
        // BRANCH ORDER is load-bearing: pro/max/ultra is tested BEFORE the bare "apple m" prefix, so
        // e.g. "Apple M1 Max" → 7680 (NOT 6144). `.contains` is substring matching. Matches the core.
