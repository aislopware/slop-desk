...
///
/// Default = all-`nil` ⇒ NO behaviour change: the theme stays the compile-time default (Foundry Ember)
...
public struct AppearancePreferences: Codable, Sendable, Equatable {
    ...
    /// `nil` ⇒ unset ⇒ the dark slot resolves to the compile-time default Foundry Ember.
...
/// The user-selectable theme. `.system` follows the OS appearance (→ Foundry Ember dark /
/// Foundry Ember Light); every other case pins one FOUNDRY seed. A
...
/// stored theme (fresh install / reset) resolves to the compile-time default Foundry Ember. The old
/// Monokai Pro raw values decode-fail the blob to its default — deliberate (no backcompat).
public enum ThemeChoice: String, Codable, Sendable, Equatable, CaseIterable {
    case system
    // FOUNDRY seeds — generated by the OKLCH theme engine (see DESIGN.md, the Seeded-Engine Rule).
    case foundryEmber // warm graphite (dark), the DEFAULT
    case foundryEmberLight // warm paper (light)
    case foundryDusk // cool mauve (dark)
    case foundryGraphite // near-neutral (dark)
}
