...
/// leaf ``SlopDeskVideoProtocol`` module so ``TerminalConfigBuilder`` (and its headless test) can emit the
/// control config lines WITHOUT importing the higher-level `SlopDeskWorkspaceCore.TerminalControls` value
/// type — which carries the `Defaults`-backed `from(defaults:)` factory and the multi-state enums
/// (`ClipboardAccess`, `MouseShiftCapture`, …). The module graph is one-way (`SlopDeskWorkspaceCore` →
/// `SlopDeskVideoProtocol`, never the reverse — VideoProtocol stays the pure wire/settings leaf with no
/// `Defaults` dependency), so the builder's input MUST live in the leaf; the embedder maps
/// `SlopDeskWorkspaceCore.TerminalControls` → this struct at the `PreferencesStore.applyTerminal()`
