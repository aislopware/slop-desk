...
// Tiny big-endian read/write helpers, local to SlopDeskVideoProtocol so this target
// stays a leaf with ZERO dependency (it must compile for macOS + iOS and be unit
// testable in isolation, exactly like SlopDeskProtocol). All multi-byte integers on
...
///
/// Mirrors `SlopDeskProtocol.BigEndianReader` but lives here so the target stays a
