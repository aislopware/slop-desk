...
public enum ConnectionPresenter {
    ...
    public static func friendlyFailure(_ raw: String) -> String {
        ...
        if lower.contains("refused") {
            return "Connection refused — is slopdesk-hostd running on the host?"
        }
        ...
        if lower.contains("reset") {
            return "Connection reset — the host daemon may have crashed. Restart slopdesk-hostd."
        }
        // The TCP connected but the slopdesk handshake didn't complete — wrong daemon, a version
        // mismatch, or a bad mux preamble (SlopDeskTransportError.handshakeFailed's errorDescription
        // carries the word "handshake"). Distinct from "refused": something IS listening, it just isn't
        // a compatible slopdesk-hostd.
        if lower.contains("handshake") {
            return "The host answered but isn't a compatible slopdesk host — check it's running "
                + "slopdesk-hostd and that the versions match."
        }
        ...
        if lower == "connection failed" {
            return "Couldn't reach the host — check the address and port, and that slopdesk-hostd is running."
        }
        return raw
    }
