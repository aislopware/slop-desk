...
/// paste-protection pre-check (``PastePrecheck``) — rather than letting the configured `right-click-action`
/// dispatch directly.
///
/// The hole this closes, and the gates that close it, are `slopdesk_terminal::surface`'s
/// `right_click_intercepts_as_paste`: the deleted fork's libghostty owned the bare-right-click dispatch end
/// to end and its own paste gate only flagged a newline or a bracketed-paste end, so the four-danger
/// analysis ⌘V runs was unreachable from a right-click unless the embedder took the click first. The rule
/// outlived the fork it was written against (see the ⚠️ on ``RightClickAction``).
///
