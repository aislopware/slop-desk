...
public final class WorkspaceStore {
    ...
    public internal(set) var isInteractiveResizeActive = false
    ...
    var lastSolvedLayout: SolvedLayout?
    ...
    var lastContainerBounds: CGRect?
    ...
    /// re-renders as results land; `internal(set)` so only ``runGlobalSearch(query:caseSensitive:isRegex:)``
    /// mutates it — it moved to `WorkspaceStore+Search.swift`, which is why the setter is module-wide
    /// rather than file-private. Reopening ⇧⌘F shows the last results until the query is re-run.
    public internal(set) var globalSearch: GlobalSearchResults?
    ...
    public internal(set) var globalSearchQuery: String = ""
    public internal(set) var globalSearchCaseSensitive = false
    public internal(set) var globalSearchRegex = false
    ...
    @ObservationIgnored var globalSearchSourceCache: [GlobalSearchSource]?
    ...
    var projectGitInFlight: Set<String> = []
