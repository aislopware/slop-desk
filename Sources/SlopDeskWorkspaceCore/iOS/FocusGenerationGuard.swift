...
///
/// On iPad-regular, more than one `TerminalInputHostView` is mounted at once, so switching the
/// focused pane means *resigning* one and *making another* the first responder. UIKit's
/// `becomeFirstResponder` is honoured asynchronously (a runloop hop later), so two rapid focus
/// changes (A→B→C, or a claim already scheduled onto the main queue by a mount that has since been
/// superseded) can land **out of order**: a late callback for an already-superseded pane steals
/// focus back, leaving the wrong terminal receiving keystrokes — the classic "I typed into the pane
/// I just left" bug. `TerminalInputHostView` carries no generation counter of its own.
///
