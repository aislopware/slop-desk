// aislopdesk-fake-client — minimal UDP trigger that makes the REAL video host start capturing a
...
//
// Usage: aislopdesk-fake-client --window-id N --width W --height H [--host 127.0.0.1] [--port 9000]
//        [--seconds 20] [--self-scroll PID]
// The host logs `capture gap`/`send gap` (AISLOPDESK_VIDEO_DEBUG=1); grep those to measure.
...
exit(0)
#else
fatalError("aislopdesk-fake-client is macOS-only")
#endif
