...
while let a = it.next() {
    switch a {
    case "--key": if let v = it
        .next() { keyCode = v.hasPrefix("0x") ? (UInt16(v.dropFirst(2), radix: 16) ?? keyCode) : (
            UInt16(v) ?? keyCode
        )
        }
...
guard let client = VirtualHIDKeyboardClient() else { eprint("could not open UDP socket to the bridge")
    exit(1)
}

eprint(
    "typing key 0x\(String(keyCode, radix: 16)) every \(intervalMs)ms via VirtualHIDKeyboardClient → bridge (127.0.0.1:9100). Ctrl-C to stop.",
)
