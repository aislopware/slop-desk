...
//   slopdesk-videohostd --window-id 12345               # serve that window (default ports 9000/9001)
//   slopdesk-videohostd --window-title Safari           # serve the first window whose title matches
//   slopdesk-videohostd --window-id 12345 --media-port 9000 --cursor-port 9001
//
// The client enters the printed windowID + ports in the SlopDesk app's Remote-window panel.
    ...
    static func usage(_ program: String) -> String {
        ...
        [--media-port N] [--cursor-port N]
          ...
          --virtual-display  create a HiDPI 2× virtual display and move each remoted window onto it so it
                             ...
                             on a 1× host. DEFAULT OFF. Also via SLOPDESK_VD=1.
          --no-virtual-display  (default) capture the real display directly — no synthetic display, no
                             window parking; 1× capture on a 1× host. Also via SLOPDESK_VD=0.
...
if !appliedVideoPrefs.isEmpty, ProcessInfo.processInfo.environment["SLOPDESK_VIDEO_DEBUG"] != nil {
    FileHandle.standardError.write(
        Data("slopdesk-videohostd: applied video-prefs.json overlay → \(appliedVideoPrefs.sorted())\n".utf8),
    )
}
...
func resolvePaneCapture(
    ...
    // unless SLOPDESK_CAPTURE_SCALE forces a lower one: capturing at 1× DOWNSCALES the 2× VD render
    // (supersampled — sharper text than a native-1× render) for a ~4× cheaper encode that sustains
    // 60fps. The client re-sharpens (SLOPDESK_SHARPEN). Trades pixel-perfect text for smoothness.
...
/// Optional capture-scale override (`SLOPDESK_CAPTURE_SCALE`), clamped to `[1, vdScale]`. Unset ⇒
...
Task {
    do {
        ...
        log("client: open the SlopDesk app → Remote window; each pane's hello picks its window")
    } catch {
...
FileHandle.standardError.write(Data(
    "slopdesk-videohostd: the GUI video path host is macOS-only (ScreenCaptureKit + VideoToolbox).\n".utf8,
))
exit(1)
