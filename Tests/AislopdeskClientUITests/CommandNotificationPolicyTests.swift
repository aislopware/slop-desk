...
final class CommandNotificationPolicyTests: XCTestCase {
    ...
    func testThresholdIsTenSeconds() {
        XCTAssertEqual(CommandNotificationPolicy.longRunningThresholdMS, 10_000)
    }
    ...
    func testQuickCommandsDoNotNotify() {
        ...
        XCTAssertFalse(CommandNotificationPolicy.shouldNotify(durationMS: 9_999)) // just under
    }

    func testLongCommandsNotify() {
        XCTAssertTrue(CommandNotificationPolicy.shouldNotify(durationMS: 10_000)) // exactly at
        XCTAssertTrue(CommandNotificationPolicy.shouldNotify(durationMS: 12_000)) // `sleep 12`
