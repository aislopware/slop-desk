...
final class SnapToNativeTests: XCTestCase {
    ...
    private func remoteStore() -> (WorkspaceStore, PaneID) {
        let id = PaneID()
        let item = CanvasItem(id: id, spec: PaneSpec(kind: .remoteGUI, title: "win",
                              video: VideoEndpoint(windowID: 1, title: "win")),
                              frame: CGRect(x: 50, y: 60, width: 800, height: 600), z: 0)
        let store = WorkspaceStore(restoring: Workspace(canvas: Canvas(items: [item]), focusedPane: id),
    ...
    func testSnapCachesNativeFrameAndResizeRestoresIt() {
        let (store, id) = remoteStore()
        ...
        store.snapPaneToContentSize(id, target: CGSize(width: 1280, height: 720),
                                    current: CGSize(width: 760, height: 560))
        XCTAssertTrue(store.hasNativeSize(id))
        let nativeFrame = store.workspace.canvas.frame(of: id)!.size
