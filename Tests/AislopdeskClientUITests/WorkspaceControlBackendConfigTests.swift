...
// `EnvConfig.overlay[key]` plus a dead `aislopdesk.cli.config.*` UserDefaults namespace and ALWAYS
...
final class WorkspaceControlBackendConfigTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        // Wire the GUI apply hook exactly as `AislopdeskClientUI` does at launch, so mutating the live
