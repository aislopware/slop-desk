...
final class MuxChannelSessionBackpressureTests: XCTestCase {
    ...
    private final class PauseRec: @unchecked Sendable {
        ...
        func apply(_ p: Bool) { lock.lock(); current = p; lock.unlock() }
    ...
    func testExitGateReleasesWhenEOFArrivesDuringWait() async {
        ...
        XCTAssertLessThan(start.duration(to: ContinuousClock.now), .milliseconds(200),
    ...
    func testShutdownDeletesTheShimDirectory() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("aislopdesk-zdotdir-\(UUID().uuidString)", isDirectory: true)
