...
final class MetadataWireMessageTests: XCTestCase {
    ...
    func testMetadataRequestTruncatedBodyDrops() throws {
        ...
        for payload in truncations {
            XCTAssertThrowsError(try decodePayload(payload)) { error in
                XCTAssertEqual(error as? AislopdeskError, .truncated, "\(payload)")
            }
        }
    }
    ...
    func testUnknownTypeDropsNotTraps() throws {
        // A peer that does not know 16/30 DROPS the frame via unknownMessageType, never traps. (17 and
        // 31 are the new "next free" bytes after this epic; 99 is arbitrary.)
        for unknown: UInt8 in [17, 31, 99] {
