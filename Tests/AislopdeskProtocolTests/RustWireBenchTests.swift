...
final class RustWireBenchTests: XCTestCase {
    ...
    func testEncodeDecodePerfIsNotRegressed() throws {
        ...
        let scenarios: [(String, WireMessage, Int)] = [
            ...
            ("output 4 KiB", .output(seq: 1, bytes: Data(repeating: 0xAB, count: 4 * 1024)), 80000),
            ("output 8 KiB", .output(seq: 1, bytes: Data(repeating: 0xAB, count: 8 * 1024)), 60000),
            ("output 16 KiB", .output(seq: 1, bytes: Data(repeating: 0xAB, count: 16 * 1024)), 40000),
        ...
        for (name, msg, iters) in scenarios {
            ...
            let native = nsPerOp(iters) {
                sink &+= ((try? WireMessage.decodeNative(payload: payload))?.messageType).map(Int.init) ?? 0
            }
