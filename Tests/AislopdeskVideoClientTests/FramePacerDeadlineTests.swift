...
final class FramePacerDeadlineTests: XCTestCase {
    ...
    func testFirstFrameSchedulesArrivalPlusPlayout() {
        XCTAssertEqual(
            FramePacer.deadlineForArrival(arrival: 10.0, lastDeadline: 0, interval: interval, playoutDelay: playout),
            10.020,
            accuracy: 1e-9,
        )
    }
    ...
    func testBunchedPairKeepsFullSpacing() {
        ...
        let d1 = FramePacer.deadlineForArrival(
            arrival: 5.000,
            lastDeadline: 4.990,
            interval: interval,
            playoutDelay: playout,
        )
        let d2 = FramePacer.deadlineForArrival(
            arrival: 5.002,
            lastDeadline: d1,
            interval: interval,
            playoutDelay: playout,
    ...
    func testStallReanchorsInsteadOfFastForwarding() {
        ...
        XCTAssertEqual(
            FramePacer.deadlineForArrival(
                arrival: arrivalAfterStall,
                lastDeadline: last,
                interval: interval,
                playoutDelay: playout,
            ),
            arrivalAfterStall + playout,
            accuracy: 1e-9,
        )
        // Just-one-interval late is NOT a stall — the rhythm holds.
        XCTAssertEqual(
            FramePacer.deadlineForArrival(
                arrival: last + interval * 1.5,
                lastDeadline: last,
                interval: interval,
                playoutDelay: playout,
            ),
    ...
    func testDeadlineDueHalfTickLookahead() {
        ...
        XCTAssertTrue(
            FramePacer.deadlineDue(deadline: 1.003, now: 1.000, halfTick: halfTick),
            "within lookahead → present this tick",
        )
        XCTAssertFalse(
            FramePacer.deadlineDue(deadline: 1.006, now: 1.000, halfTick: halfTick),
            "beyond lookahead → wait",
        )
    }
    ...
    func testDeadlineModeEndToEndThroughTicks() throws {
        ...
        try pacer.submit(XCTUnwrap(pb))
        let start = FramePacer.currentHostTimeSeconds()
        var t = start
        for _ in 0..<10 { t += 1.0 / 120.0
