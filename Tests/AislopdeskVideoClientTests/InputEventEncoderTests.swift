...
final class InputEventEncoderTests: XCTestCase {
    ...
    func testNormalizeCentreOfLayer() {
        let n = InputEventEncoder.normalize(
            viewPoint: VideoPoint(x: 400, y: 300),
            layerSize: VideoSize(width: 800, height: 600),
            videoNativeSize: VideoSize(width: 800, height: 600),
        )
    ...
    func testNormalizeClampsOutOfBounds() {
        let over = InputEventEncoder.normalize(
            viewPoint: VideoPoint(x: 1000, y: -50),
            layerSize: VideoSize(width: 800, height: 600),
            videoNativeSize: VideoSize(width: 800, height: 600),
        )
    ...
    func testNormalizeZeroLayerIsSafe() {
        let n = InputEventEncoder.normalize(
            viewPoint: VideoPoint(x: 10, y: 10),
            layerSize: VideoSize(width: 0, height: 0),
            videoNativeSize: VideoSize(width: 800, height: 600),
        )
    ...
    func testTagsAreMonotonic() {
        ...
        let e2 = enc.mouseDown(
            button: .left,
            viewPoint: VideoPoint(x: 50, y: 50),
            layerSize: layer,
    ...
    func testMouseDownNormalisesAndRoundTrips() throws {
        var enc = InputEventEncoder()
        let event = enc.mouseDown(
            button: .right,
            viewPoint: VideoPoint(x: 800, y: 300),
            layerSize: VideoSize(width: 800, height: 600),
            videoNativeSize: VideoSize(width: 800, height: 600),
            clickCount: 2,
