...
final class VideoScaleMathTests: XCTestCase {
    ...
    func testOneToOneScale() {
        let scale = VideoScaleMath.videoScale(layerSize: VideoSize(width: 800, height: 600), decodedSize: VideoSize(width: 800, height: 600))
    ...
    func testHalfSizeLayerHalvesScale() {
        ...
        let scale = VideoScaleMath.videoScale(layerSize: VideoSize(width: 800, height: 600), decodedSize: VideoSize(width: 1600, height: 1200))
        XCTAssertEqual(scale, 0.5, accuracy: 1e-9)
    }

    func testUpscaledLayerGivesScaleAboveOne() {
        let scale = VideoScaleMath.videoScale(layerSize: VideoSize(width: 1600, height: 1200), decodedSize: VideoSize(width: 800, height: 600))
        XCTAssertEqual(scale, 2.0, accuracy: 1e-9)
    }

    func testZeroDecodedWidthFallsBackToUnity() {
        let scale = VideoScaleMath.videoScale(layerSize: VideoSize(width: 800, height: 600), decodedSize: VideoSize(width: 0, height: 0))
        XCTAssertEqual(scale, 1.0, accuracy: 1e-9)
    }
    ...
    func testCursorPlacementAtUnityScaleSubtractsHotspot() {
        let update = CursorUpdate(position: VideoPoint(x: 100, y: 200), shapeID: 1, hotspot: VideoPoint(x: 4, y: 6), visible: true)
        let frame = ClientCursorCompositor.layerFrame(for: update, videoScale: 1.0, cursorSize: VideoSize(width: 16, height: 16))
    ...
    func testCursorPlacementScalesPositionByVideoScale() {
        ...
        let update = CursorUpdate(position: VideoPoint(x: 100, y: 200), shapeID: 1, hotspot: VideoPoint(x: 2, y: 2), visible: true)
        let frame = ClientCursorCompositor.layerFrame(for: update, videoScale: 0.5, cursorSize: VideoSize(width: 24, height: 24))
    ...
    func testScaleMathAndCursorPlacementCompose() {
        ...
        let scale = VideoScaleMath.videoScale(layerSize: VideoSize(width: 800, height: 600), decodedSize: VideoSize(width: 1600, height: 1200))
        let update = CursorUpdate(position: VideoPoint(x: 400, y: 400), shapeID: 1, hotspot: VideoPoint(x: 0, y: 0), visible: true)
        let frame = ClientCursorCompositor.layerFrame(for: update, videoScale: scale, cursorSize: VideoSize(width: 16, height: 16))
