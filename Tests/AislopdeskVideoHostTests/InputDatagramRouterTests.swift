...
final class InputDatagramRouterTests: XCTestCase {
    ...
    func testArmedLatchRaisesKeyButExemptsScroll() {
        ...
        guard case .inject(_, let keyRaises) = router.route(datagram: key.encode(), mediaFlowing: true, needsRaise: true) else {
        ...
        guard case .inject(_, let scrollRaises) = router.route(datagram: scroll.encode(), mediaFlowing: true, needsRaise: true) else {
    ...
    func testScrollIsTheOnlyLatchExemptEvent() {
        ...
        XCTAssertFalse(InputDatagramRouter.latchExemptFromRaise(.mouseDown(button: .left, normalized: n, clickCount: 1, modifiers: [], tag: 0)))
        XCTAssertFalse(InputDatagramRouter.latchExemptFromRaise(.mouseUp(button: .left, normalized: n, clickCount: 1, modifiers: [], tag: 0)))
    ...
    func testPostClickScrollIsExemptButLeavesLatchForFollowingKey() {
        ...
        XCTAssertTrue(step(.mouseDown(button: .left, normalized: n, clickCount: 1, modifiers: [], tag: 0)), "click raises (button-down)")
        XCTAssertFalse(step(.mouseUp(button: .left, normalized: n, clickCount: 1, modifiers: [], tag: 0)), "the up re-arms the latch")
        XCTAssertFalse(step(.scroll(dx: 0, dy: -5, normalized: n, tag: 0)), "the post-click scroll is latch-exempt → NO AX raise (the fix)")
        XCTAssertTrue(step(.key(keyCode: 0x24, down: true, modifiers: [], tag: 0)), "a key after the scroll still raises (the exempt scroll did not consume the armed latch)")
    }
}
