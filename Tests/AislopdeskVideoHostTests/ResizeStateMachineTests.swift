...
final class ResizeStateMachineTests: XCTestCase {
    ...
    private func streamingMachine(nextStreamID: UInt32 = 7) -> VideoSessionStateMachine {
        ...
        _ = sm.start()
        let hello = VideoControlMessage.hello(protocolVersion: AislopdeskVideoProtocol.version,
    ...
    func testResizeWhileStreamingEmitsResizeCaptureClampedWithEpoch() {
        ...
        let effects = sm.handleControl(req, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize)
    ...
    func testResizeClampsBelowMinAndAboveMax() {
        ...
        XCTAssertEqual(sm.handleControl(tooSmall, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize),
        ...
        XCTAssertEqual(sm.handleControl(tooBig, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize),
    ...
    func testResizeDoesNotMintNewStreamID() {
        ...
        let req = VideoControlMessage.resizeRequest(desired: VideoSize(width: 1280, height: 800), epoch: 1)
        _ = sm.handleControl(req, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize)
        ...
        let hello = VideoControlMessage.hello(protocolVersion: AislopdeskVideoProtocol.version, requestedWindowID: windowID, viewport: VideoSize(width: 800, height: 600))
    ...
    func testStaleEpochIgnored() {
        ...
        XCTAssertEqual(sm.handleControl(first, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize),
        ...
        XCTAssertTrue(sm.handleControl(stale, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize).isEmpty)
        let dup = VideoControlMessage.resizeRequest(desired: VideoSize(width: 640, height: 480), epoch: 5)
        XCTAssertTrue(sm.handleControl(dup, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize).isEmpty)
        ...
        XCTAssertEqual(sm.handleControl(fresh, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize),
    ...
    func testResizeIgnoredWhileListening() {
        ...
        let req = VideoControlMessage.resizeRequest(desired: VideoSize(width: 1280, height: 800), epoch: 1)
        XCTAssertTrue(sm.handleControl(req, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: resolveResize).isEmpty)
        XCTAssertEqual(sm.state, .listening)
    ...
    func testResizeForWrongWindowRejectedByResolver() {
        ...
        let req = VideoControlMessage.resizeRequest(desired: VideoSize(width: 1280, height: 800), epoch: 1)
        XCTAssertTrue(sm.handleControl(req, windowBoundsCG: bounds, resolveCaptureSize: acceptAll, resolveResizeSize: rejectResolver).isEmpty)
        XCTAssertEqual(sm.state, .streaming)
        XCTAssertEqual(sm.lastResizeEpoch, 0, "rejected resize does NOT advance the epoch (a later valid request still wins)")
    ...
    func testResizeResolverReceivesSessionWindowID() {
        ...
        let spy: (UInt32, VideoSize) -> (UInt16, UInt16)? = { wid, _ in seenWindowID = wid; return (1024, 768) }
