...
final class StaticIDRDeciderTests: XCTestCase {
    ...
    func testNoBufferNeverFires() {
        ...
        for now: TimeInterval in [0, 1, 5, 100] {
            for forced in [false, true] {
                XCTAssertFalse(d.shouldReencode(now: now, forcedLatched: forced, hasRetainedBuffer: false),
    ...
    func testHeartbeatBoundaryInclusive() {
        ...
        XCTAssertFalse(d.shouldReencode(now: 10.999, forcedLatched: false, hasRetainedBuffer: true),
    ...
    func testQuietWindowSuppressesForced() {
        var d = make()
        d.onCompleteFrame(now: 10)
        XCTAssertFalse(d.shouldReencode(now: 10.5, forcedLatched: true, hasRetainedBuffer: true),
    ...
    func testQuietWindowExactBoundary() {
        ...
        XCTAssertFalse(d.shouldReencode(now: 10.3, forcedLatched: false, hasRetainedBuffer: true),
    ...
    func testForcedArmedFires() {
        let d = make()
        XCTAssertTrue(d.shouldReencode(now: 0.5, forcedLatched: true, hasRetainedBuffer: true),
    ...
    func testSyntheticReAnchorsCadence() {
        ...
        XCTAssertFalse(d.shouldReencode(now: 11.5, forcedLatched: false, hasRetainedBuffer: true),
                       "0.5s after the synthetic ⇒ no fire")
        XCTAssertTrue(d.shouldReencode(now: 12.0, forcedLatched: false, hasRetainedBuffer: true),
    ...
    func testLastEmissionPicksSyntheticWhenNoRealFrame() {
        ...
        XCTAssertFalse(d.shouldReencode(now: 5.5, forcedLatched: false, hasRetainedBuffer: true),
                       "0.5s after the only (synthetic) emission, no real frame ⇒ no fire")
        XCTAssertTrue(d.shouldReencode(now: 6.0, forcedLatched: false, hasRetainedBuffer: true),
    ...
    func testEquatable() {
        ...
        var b = make()
        XCTAssertEqual(a, b)
        a.onCompleteFrame(now: 10); b.onCompleteFrame(now: 10)
        a.recordSynthetic(now: 11); b.recordSynthetic(now: 11)
        XCTAssertEqual(a, b)
