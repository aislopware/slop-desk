...
final class ResizeControlCodecTests: XCTestCase {
    ...
    func testResizeRequestRoundTrip() throws {
        let cases: [VideoControlMessage] = [
            ...
            .resizeRequest(desired: VideoSize(width: 1920.5, height: 1080.25), epoch: 0xDEAD_BEEF),
    ...
    func testResizeAckRoundTrip() throws {
        let cases: [VideoControlMessage] = [
            ...
            .resizeAck(captureWidth: .max, captureHeight: .max, epoch: 0xCAFE_BABE),
    ...
    func testResizeMessageTypeBytes() {
        XCTAssertEqual(
            VideoControlMessage.resizeRequest(desired: VideoSize(width: 1, height: 1), epoch: 0).messageType,
            4,
        )
        ...
        XCTAssertEqual(
            VideoControlMessage
                .hello(protocolVersion: 1, requestedWindowID: 0, viewport: VideoSize(width: 0, height: 0)).messageType,
            1,
        )
        XCTAssertEqual(
            VideoControlMessage
                .helloAck(accepted: true, streamID: 0, captureWidth: 0, captureHeight: 0, windowBoundsCG: VideoRect(
                    x: 0,
                    y: 0,
                    width: 0,
                    height: 0,
                ), fullRange: false).messageType,
            2,
        )
