...
/// its two `chord: nil` `.view` registry bindings (Info / Files; the old Outline tab is merged into
/// Info's Commands section, and the old Git tab into Info's git-summary row + popup) — that switch the
/// right-hand Details panel to a specific
...
final class DetailsTabRoutingTests: XCTestCase {
    ...
    /// `.selectDetailsTab(tab)` forwards EACH of the tabs to the supplied `selectDetailsTab` closure
    ...
    /// commands are never DEAD chords, just inert until the view installs the closure).
    ...
    func testSelectDetailsTabRoutesOnCanvasPath() {
        ...
        XCTAssertEqual(captured, .files, "the canvas path also forwards the selected tab to the closure")
    }

    // MARK: - Registry: the two bindings exist, are `.view`, and are chord-less

    /// Each of the two `Details: *` commands has a registered binding with the documented id, the `.view`
    /// category, and NO default chord (`chord: nil` — unbound, palette/menu-only). FAILS on the un-fixed code
    /// (the bindings don't exist) and on a category / chord regression. Revert-to-confirm-fail: removing
    /// either registry row drops its `binding(for:)` to nil here.
    ...
    /// The two commands surface in the cheat-sheet / palette display set (the View group of
    ...
    /// nor `view.detailsGit` (its tab merged into Info's git-summary row + popup).
    func testDetailsBindingsSurfaceInTheViewDisplayGroup() {
        ...
        XCTAssertTrue(
            ids.isSuperset(of: ["view.detailsInfo", "view.detailsFiles"]),
            "the two Details: * jump commands surface in the View display group (palette / cheat sheet)",
        )
        XCTAssertFalse(ids.contains("view.detailsOutline"), "the merged-away Outline jump command is gone")
        XCTAssertFalse(ids.contains("view.detailsGit"), "the merged-away Git jump command is gone")
    }
