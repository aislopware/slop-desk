...
@testable import RworkTransport   // reach `MuxSubChannel.deliver(payload:)` (the demux inbound seam)
...
final class PTYProcessTests: XCTestCase {
    ...
    func testInteractiveZshControllingTTYAndSigwinch() throws {
        ...
        var env = curatedEnv()
        env["ZDOTDIR"] = "/nonexistent-rwork-test"   // belt-and-suspenders: no stray rc.
        try pty.spawn(
            zsh,
            arguments: ["-f", "-i"],
            environment: env,
        ...
        Thread.sleep(forTimeInterval: 0.3)
        PTYProcessTests.write(pty.masterFD, "print -r -- RWORK_COLS=$COLUMNS\n")
        let colsOut = readUntil(fd: pty.masterFD, needle: "RWORK_COLS=132", timeout: 5.0)
        XCTAssertTrue(
            colsOut.contains("RWORK_COLS=132"),
