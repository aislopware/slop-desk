...
final class ResizeAdoptionTests: XCTestCase {
    ...
    func testDegenerateZeroDimsRejected() {
        let pending = VideoSize(width: 400, height: 300)
        XCTAssertFalse(ResizeAdoption.shouldAdopt(pending: pending, decoded: VideoSize(width: 0, height: 600), previousDecoded: nil))
        XCTAssertFalse(ResizeAdoption.shouldAdopt(pending: VideoSize(width: 0, height: 300), decoded: pending, previousDecoded: nil))
    }
