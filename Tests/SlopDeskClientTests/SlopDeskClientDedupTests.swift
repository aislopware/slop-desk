...
/// (`SlopDeskClient.deliverOutput`'s `guard seq > highestSeqFed`).
///
...
final class SlopDeskClientDedupTests: XCTestCase {
    ...
    /// output restarts at seq 1 (no per-channel resume). `SlopDeskClient.connect` must RESET the dedup
    ...
    /// (mint a UUID on a new resume; `returningClient = resume != newSessionID`) so `SlopDeskClient.connect`
    ...
    private func waitUntil(timeout: Duration, _ condition: @Sendable () -> Bool) async throws {
        ...
        if !condition() { throw SlopDeskDedupTestError.timedOut }
    }

    private enum SlopDeskDedupTestError: Error { case timedOut }
}
