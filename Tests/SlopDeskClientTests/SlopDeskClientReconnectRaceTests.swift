...
/// close()/pause()/resume()/concurrent-connect variants). `SlopDeskClient` is an `actor`, so it is
...
final class SlopDeskClientReconnectRaceTests: XCTestCase {
    ...
    func testCloseDuringInflightConnectTearsDownNewTransportNoZombie() async {
        for _ in 0..<200 {
            ...
            let client = SlopDeskClient(makeTransport: { rec.make() })
