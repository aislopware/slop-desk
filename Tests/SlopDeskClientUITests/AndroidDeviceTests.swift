...
final class AndroidDeviceListSectionTests: XCTestCase {
    ...
    func testEverythingWithATransportGoesInTheTopGroup() {
        ...
        let sections = AndroidDeviceList.sections(for: [unauthorized, device("cold")])
        XCTAssertEqual(sections.first?.title, AndroidDeviceList.runningTitle)
    ...
    func testTheIdleDevicesAreCutByFamilyInRankOrder() {
        let sections = AndroidDeviceList.sections(for: [
    ...
    func testAVersionEveryMemberSharesIsLiftedIntoTheHeading() {
        ...
        let sections = AndroidDeviceList.sections(for: [device("a"), device("b")])
    ...
    func testDisagreementLeavesTheVersionOnTheRows() {
        let sections = AndroidDeviceList.sections(for: [device("a"), device("b", release: "15")])
    ...
    func testAnAbsentVersionIsNotLiftedAsIfItWereAFact() {
        ...
        XCTAssertNil(AndroidDeviceList.sharedVersion(of: [device("a", release: nil)]))
    }
    ...
    func testARowIdentityIsQualifiedByItsSection() {
        ...
        let cold = AndroidDeviceList.sections(for: [device("k")])
        let hot = AndroidDeviceList.sections(for: [device("k", serial: "emulator-5554")])
        XCTAssertNotEqual(cold.first?.rowIdentities, hot.first?.rowIdentities)
    }

    func testAnEmptyDeviceSetHasNoSections() {
        XCTAssertTrue(AndroidDeviceList.sections(for: []).isEmpty)
    }
}
#endif
