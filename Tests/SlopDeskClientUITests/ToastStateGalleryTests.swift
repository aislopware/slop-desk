...
// CPU rasterisation. The asserts are crash-free-render smoke; the VALUE is the opt-in PNG dump:
//
...
final class ToastStateGalleryTests: XCTestCase {
    ...
    func testGalleryLightTheme() throws {
        ThemeStore.shared.active = .monokaiProClassicLight
    ...
    private func write(_ name: String, view: some View) throws {
        ...
        renderer.scale = 2
        let image = try XCTUnwrap(renderer.nsImage, "\(name) renders without crashing")
