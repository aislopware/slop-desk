...
/// (`ingestPTYChunkForTesting` → the live ``HostOutputSniffer``), the REAL hook ingest, and the
/// REAL foreground fold, so the seam under test is the same code the daemon runs.
final class MuxChannelSessionTitleRetirementTests: XCTestCase {
    ...
    /// Every control message queued so far, from BOTH producers: the sniffer's messages ride the
    ...
    func testForegroundAbsenceAlsoClearsTheTitle() {
        ...
        session.ingestPTYChunkForTesting(osc("0;⠂ Say hi in one word"))
    ...
    /// The coalescing reset, proven through the session rather than the sniffer alone: the NEXT
    /// claude run opens on the SAME `✳ Claude Code` string that was just retired, and it must
    /// still reach the client.
    func testNextAgentRunReTitlesWithTheIdenticalString() {
        let session = makeSession()
        session.foldForegroundSampleForTesting(name: "claude", at: 0)
        session.ingestPTYChunkForTesting(osc("0;✳ Claude Code"))
        session.foldForegroundSampleForTesting(name: "zsh", at: 1)
        ...
        session.ingestPTYChunkForTesting(osc("0;✳ Claude Code"))
        XCTAssertEqual(
            titles(drainControl(session)), ["✳ Claude Code"],
            "the retired title is no longer the coalescing anchor",
        )
    }
    ...
    func testAForeignTitleSurvivesTheAgentLeaving() {
        let session = makeSession()
        session.foldForegroundSampleForTesting(name: "claude", at: 0)
        session.ingestPTYChunkForTesting(osc("0;nvim — README.md"))
        _ = drainControl(session)
