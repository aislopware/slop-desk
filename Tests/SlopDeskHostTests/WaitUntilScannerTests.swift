...
final class WaitUntilScannerTests: XCTestCase {
    private func makeScanner(
        ...
        try XCTUnwrap(WaitUntilScanner(pattern: pattern, bufferCap: cap), "the pattern compiles")
    }

    func testMarkerWithinASingleChunkMatches() throws {
        let s = try makeScanner("BUILD COMPLETE")
    ...
    func testOscSplitAcrossBoundaryStripsCleanAndMarkerMatches() throws {
        ...
        let s = try makeScanner("^READY>$", cap: AgentControlHandler.waitBufferCap)
    ...
    func testMultibyteUTF8SplitAcrossBoundaryDecodes() throws {
        ...
        let s = try makeScanner("réussite")
    ...
    func testStrippedAccumulatorKeepsCapAndTrimsOldestHalf() throws {
        let s = try makeScanner("NEVER MATCHES ANYTHING", cap: 1024)
        for _ in 0..<64 {
