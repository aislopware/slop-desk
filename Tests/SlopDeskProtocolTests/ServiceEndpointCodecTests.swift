...
/// The ``MetadataCodec/ServiceEndpoint`` wire codec (`ensureCodeServer` = 18): the 3-byte
...
final class ServiceEndpointCodecTests: XCTestCase {
    func testRoundTripEveryState() throws {
        for state in MetadataCodec.ServiceState.allCases {
            let endpoint = MetadataCodec.ServiceEndpoint(state: state, port: 62636)
            let decoded = try MetadataCodec.decodeServiceEndpoint(
                MetadataCodec.encodeServiceEndpoint(endpoint),
            )
    ...
    func testEncodedShapeIsPinned() {
        ...
        let data = MetadataCodec.encodeServiceEndpoint(
            MetadataCodec.ServiceEndpoint(state: .ready, port: 0x1F90),
        )
        XCTAssertEqual(Array(data), [1, 0x1F, 0x90])
    }

    func testTruncatedThrows() {
        XCTAssertThrowsError(try MetadataCodec.decodeServiceEndpoint(Data()))
        XCTAssertThrowsError(try MetadataCodec.decodeServiceEndpoint(Data([1])))
        XCTAssertThrowsError(try MetadataCodec.decodeServiceEndpoint(Data([1, 0])))
    }

    func testTrailerTolerated() throws {
        // A future field appended after the port must not break this reader.
        let decoded = try MetadataCodec.decodeServiceEndpoint(Data([1, 0x1F, 0x90, 0xAB, 0xCD]))
        XCTAssertEqual(decoded, MetadataCodec.ServiceEndpoint(state: .ready, port: 0x1F90))
    }
    ...
    func testUnknownStateByteReadsStarting() throws {
        ...
        let decoded = try MetadataCodec.decodeServiceEndpoint(Data([7, 0x00, 0x50]))
