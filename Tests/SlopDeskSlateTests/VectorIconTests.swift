...
// can go wrong is the transcription. These are headless VALUE assertions on the resulting `Path`
...
final class VectorIconTests: XCTestCase {
    ...
    func testRelativeAndAbsoluteAgree() {
        let absolute = SVGPath.path("M2 2 L12 2 L12 12 L2 12 Z")
        let relative = SVGPath.path("m2 2 l10 0 l0 10 l-10 0 z")
        XCTAssertEqual(absolute.boundingRect, CGRect(x: 2, y: 2, width: 10, height: 10))
        XCTAssertEqual(relative.boundingRect, absolute.boundingRect)
    }
    ...
    func testShorthandAxesAndImplicitRepeats() {
        XCTAssertEqual(
            SVGPath.path("M0 0H10V10H0Z").boundingRect,
            CGRect(x: 0, y: 0, width: 10, height: 10),
        )
        // "M0 0 10 0 10 10" — one moveto, then two implicit LINES.
        let implicit = SVGPath.path("M0 0 10 0 10 10")
        let explicit = SVGPath.path("M0 0 L10 0 L10 10")
    ...
    func testCloseReturnsToTheSubpathStart() {
        ...
        let path = SVGPath.path("M5 5H10V10H5Z l10 0")
        XCTAssertEqual(path.boundingRect.maxX, 15, accuracy: 1e-9)
    }
    ...
    func testArcsBuildATrueCircle() {
        let circle = SVGPath.path("M10 0A10 10 0 0 1 -10 0A10 10 0 0 1 10 0")
        let box = circle.boundingRect
    ...
    func testArcFlagsMayBePackedAgainstTheNextNumber() {
        let spaced = SVGPath.path("M0 0a10 10 0 0 1 10 10")
        let packed = SVGPath.path("M0 0a10 10 0 0110 10")
        XCTAssertFalse(spaced.isEmpty)
        XCTAssertEqual(packed.boundingRect, spaced.boundingRect)
    }
    ...
    func testDegenerateArcsDegradeToLines() {
        XCTAssertEqual(
            SVGPath.path("M0 0A0 0 0 0 1 10 10").boundingRect,
            CGRect(x: 0, y: 0, width: 10, height: 10),
        )
        ...
        let stationary = SVGPath.path("M4 4A2 2 0 0 1 4 4").boundingRect
    ...
    func testSmoothCurvesReflectTheirControlPoint() {
        let smooth = SVGPath.path("M0 0 C0 5 5 10 10 10 S20 5 20 0")
        let spelled = SVGPath.path("M0 0 C0 5 5 10 10 10 C15 10 20 5 20 0")
        XCTAssertEqual(smooth.boundingRect.maxX, spelled.boundingRect.maxX, accuracy: 1e-9)
        XCTAssertEqual(smooth.boundingRect.maxY, spelled.boundingRect.maxY, accuracy: 1e-9)
    }
    ...
    func testNumbersParseWithoutSeparators() {
        XCTAssertEqual(SVGPath.path("M0 0h1e1").boundingRect.maxX, 10, accuracy: 1e-9)
        XCTAssertEqual(SVGPath.path("M0 0h.5").boundingRect.maxX, 0.5, accuracy: 1e-9)
        XCTAssertEqual(SVGPath.path("M0 0h-.5").boundingRect.minX, -0.5, accuracy: 1e-9)
        XCTAssertEqual(SVGPath.path("M0,0 L10,0").boundingRect.maxX, 10, accuracy: 1e-9)
    }
    ...
    func testMalformedDataTerminates() {
        XCTAssertTrue(SVGPath.path("").isEmpty)
        XCTAssertTrue(SVGPath.path("?!@").isEmpty)
        XCTAssertFalse(SVGPath.path("M0 0 L10 10 X 5 5").isEmpty)
    }
    ...
    func testTheHandFillsItsViewBoxAndNoMore() {
        ...
        for outline in OttyIcon.hand.outlines {
            let path = SVGPath.path(outline)
            XCTAssertFalse(path.isEmpty, "every stroke of the hand must parse")
            union = union.union(path.boundingRect)
        }
    ...
    func testTheCupsDuotoneLayersOverlap() {
        let boxes = OttyIcon.coffee.fills.map { SVGPath.path($0.data).boundingRect }
    ...
    func testFittingIsUniformAndCentred() {
        ...
        XCTAssertEqual(
            SVGPath.path(data, viewBox: 24, in: square).boundingRect,
            CGRect(x: 0, y: 0, width: 48, height: 48),
        )
        let fitted = SVGPath.path(data, viewBox: 24, in: wide).boundingRect
