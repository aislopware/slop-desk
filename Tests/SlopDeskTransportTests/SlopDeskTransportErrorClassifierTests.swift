...
final class SlopDeskTransportErrorClassifierTests: XCTestCase {
    func testCanonicalAddressInUsePhraseMatches() {
        ...
        XCTAssertTrue(SlopDeskTransportError
    ...
    func testNumericErrnoRenderingMatchesAsStandaloneToken() {
        XCTAssertTrue(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("posix(48)"))
        XCTAssertTrue(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("NWError errno 48"))
        XCTAssertTrue(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("48"))
        XCTAssertTrue(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("error 48: bind"))
    }

    func testEmbedded48DoesNotMisclassify() {
        // None of these are EADDRINUSE — "48" is part of a longer number, not a standalone errno.
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("could not bind port 4843"))
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("posix errno 148: network down"))
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("buffer size 1048576 exceeded"))
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("port 8048 refused"))
    }

    func testUnrelatedErrorsDoNotMatch() {
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("Network is down"))
        XCTAssertFalse(SlopDeskTransportError.listenerDetailIndicatesAddressInUse("Connection refused"))
...
final class WaitingBindConflictClassifierTests: XCTestCase {
    func testEADDRINUSEIsFatalInWaiting() {
        XCTAssertTrue(
            SlopDeskTransportError.waitingErrnoIsFatalBindConflict(EADDRINUSE),
            "a stuck .waiting on EADDRINUSE never auto-recovers → surface it immediately",
        )
        XCTAssertTrue(
            SlopDeskTransportError.waitingErrnoIsFatalBindConflict(48),
    ...
    func testTransientNetworkErrnosKeepWaiting() {
        ...
        for errno in [ENETDOWN, ENETUNREACH, ETIMEDOUT, EAGAIN, ECONNREFUSED, EHOSTUNREACH, ENOTCONN] {
            XCTAssertFalse(
                SlopDeskTransportError.waitingErrnoIsFatalBindConflict(errno),
                "transient waiting errno \(errno) must keep waiting, not fail",
            )
        }
        XCTAssertFalse(
            SlopDeskTransportError.waitingErrnoIsFatalBindConflict(0),
