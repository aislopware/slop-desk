...
final class InputKeyRedundancyTests: XCTestCase {
    ...
    func testModifierKeyUpIsSentRedundantly() {
        for keyCode: UInt16 in [54, 55, 56, 58, 59, 60, 61, 62, 63] {
            XCTAssertEqual(
                SlopDeskVideoClientSession.keySendCount(keyCode: keyCode, down: false), 3,
    ...
    func testModifierKeyDownIsSentOnce() {
        for keyCode: UInt16 in [54, 55, 56, 58, 59, 60, 61, 62, 63] {
            XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: keyCode, down: true), 1)
        }
    }

    /// Ordinary keys are never duplicated on either edge.
    func testOrdinaryKeysAreSentOnce() {
        XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: 0, down: false), 1) // 'a' up
        XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: 0, down: true), 1)
        XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: 36, down: false), 1) // return up
    }
    ...
    func testCapsLockIsNeverDuplicated() {
        XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: 57, down: false), 1)
        XCTAssertEqual(SlopDeskVideoClientSession.keySendCount(keyCode: 57, down: true), 1)
