...
final class OwdLateDetectorTests: XCTestCase {
    ...
    func testEnvConfigClamps() {
        let c = OwdLateDetector.Config.fromEnvironment([
            "SLOPDESK_OWD_LATE_FLOOR_MS": "0.2", // below min → clamp to 1
            "SLOPDESK_OWD_LATE_FRAC_PCT": "9999", // above max → clamp to 400%
            "SLOPDESK_OWD_LATE_WARMUP": "0", // below min → clamp to 1
        ])
