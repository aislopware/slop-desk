...
/// `SLOPDESK_DISPLAY_CAPTURE` A/B seam and the VD-parked default (docs: the tooltip
...
final class CaptureModeResolutionTests: XCTestCase {
    ...
    /// `SLOPDESK_DISPLAY_CAPTURE` through ``EnvConfig`` (overlay → env). This asserts the integrated
    ...
    func testOverlayReachesDisplayCaptureSelection() throws {
        try XCTSkipIf(
            ProcessInfo.processInfo.environment["SLOPDESK_DISPLAY_CAPTURE"] != nil,
        ...
        XCTAssertEqual(
            WindowCapturer.resolveCaptureMode(
                envValue: EnvConfig.string("SLOPDESK_DISPLAY_CAPTURE"), preferDisplayAnchored: true,
            ),
            .displayIncluding,
        )
        // A settings override forces the window-composite mode even on a VD-parked window.
        EnvConfig.overlay["SLOPDESK_DISPLAY_CAPTURE"] = "window"
        XCTAssertEqual(
            WindowCapturer.resolveCaptureMode(
                envValue: EnvConfig.string("SLOPDESK_DISPLAY_CAPTURE"), preferDisplayAnchored: true,
            ),
            .window,
            "overlay SLOPDESK_DISPLAY_CAPTURE=window forces .window",
        )
    }

    /// REACHES-CONSUMER (P1): the `VideoEncoder.qpDecouple` consumer resolves `SLOPDESK_QP_DECOUPLE`
    ...
    func testOverlayReachesQPDecoupleResolution() throws {
        try XCTSkipIf(
            ProcessInfo.processInfo.environment["SLOPDESK_QP_DECOUPLE"] != nil,
            "a real env var would win over the overlay (decision #16)",
        )
        EnvConfig.overlay = [:]
        XCTAssertTrue(EnvConfig.boolDefaultOn("SLOPDESK_QP_DECOUPLE"), "empty overlay ⇒ decouple ON (default)")
        EnvConfig.overlay["SLOPDESK_QP_DECOUPLE"] = "0"
        XCTAssertFalse(EnvConfig.boolDefaultOn("SLOPDESK_QP_DECOUPLE"), "overlay \"0\" ⇒ decouple OFF")
    }
