...
final class AdaptiveFECPolicyTests: XCTestCase {
    ...
    /// The escape hatch (`SLOPDESK_FEC_ALLOW_OFF=1` → `allowOff: true`) restores the old walk all
    ...
    func testAllowOffEnvResolution() {
        ...
        XCTAssertFalse(AdaptiveFECPolicy.allowOffTier(env: ["SLOPDESK_FEC_ALLOW_OFF": "0"]))
        XCTAssertFalse(AdaptiveFECPolicy.allowOffTier(env: ["SLOPDESK_FEC_ALLOW_OFF": "true"]))
        XCTAssertTrue(AdaptiveFECPolicy.allowOffTier(env: ["SLOPDESK_FEC_ALLOW_OFF": "1"]))
    }
    ...
    func testAdaptiveMGateRequiresFlagAndMultiLossCodec() {
        ...
        XCTAssertTrue(AdaptiveFECPolicy.adaptiveMFromEnv([
            "SLOPDESK_ADAPTIVE_FEC_M": "1", "SLOPDESK_FEC_M": "3",
        ]))
        XCTAssertFalse(
            AdaptiveFECPolicy.adaptiveMFromEnv(["SLOPDESK_ADAPTIVE_FEC_M": "1"]),
            "no FEC_M ⇒ single-parity codec ⇒ tier→m table inert ⇒ gate OFF",
        )
        XCTAssertFalse(
            AdaptiveFECPolicy.adaptiveMFromEnv(["SLOPDESK_FEC_M": "3"]),
            "flag absent ⇒ gate OFF",
        )
        XCTAssertFalse(AdaptiveFECPolicy.adaptiveMFromEnv([
            "SLOPDESK_ADAPTIVE_FEC_M": "1", "SLOPDESK_FEC_M": "1",
