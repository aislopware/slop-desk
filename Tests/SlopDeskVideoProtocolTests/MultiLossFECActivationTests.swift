...
/// LOAD-BEARING PROOF for the gated multi-loss Reed-Solomon FEC (`SLOPDESK_FEC_M >= 2`).
///
...
///
/// Plus the env-gate resolution (parse/clamp of `SLOPDESK_FEC_M` / `SLOPDESK_FEC_K`) and the
...
final class MultiLossFECActivationTests: XCTestCase {
    ...
    func testEnvGateDefaultsToM1() {
        ...
        XCTAssertFalse(
            AdaptiveFECPolicy.MultiLossFEC.resolveParityCount(env: ["SLOPDESK_FEC_M": "1"]) >= 2,
            "m=1 is not active",
        )
    }
    ...
    func testEnvGateParsesAndClamps() {
        ...
        XCTAssertEqual(mlf.resolveParityCount(env: ["SLOPDESK_FEC_M": "2"]), 2)
        XCTAssertEqual(mlf.resolveGroupSize(env: ["SLOPDESK_FEC_M": "2", "SLOPDESK_FEC_K": "8"]), 8)
        // m clamps to [1, 8].
        XCTAssertEqual(mlf.resolveParityCount(env: ["SLOPDESK_FEC_M": "0"]), 1, "m floored to 1")
        XCTAssertEqual(mlf.resolveParityCount(env: ["SLOPDESK_FEC_M": "99"]), 8, "m capped to 8")
        XCTAssertEqual(mlf.resolveParityCount(env: ["SLOPDESK_FEC_M": "garbage"]), 1, "non-numeric ⇒ m=1")
        // k clamps to [2, 64].
        XCTAssertEqual(
            mlf.resolveGroupSize(env: ["SLOPDESK_FEC_M": "2", "SLOPDESK_FEC_K": "1"]),
            2,
            "k floored to 2",
        )
        XCTAssertEqual(
            mlf.resolveGroupSize(env: ["SLOPDESK_FEC_M": "2", "SLOPDESK_FEC_K": "999"]),
            64,
            "k capped to 64",
        )
        // Joint GF(2^8) bound k + m <= 255 (degenerate huge-m, but proves the cap is applied).
        XCTAssertEqual(mlf.resolveParityCount(env: ["SLOPDESK_FEC_M": "8"]), 8)
        XCTAssertLessThanOrEqual(
            mlf.resolveGroupSize(env: ["SLOPDESK_FEC_M": "8", "SLOPDESK_FEC_K": "64"]) + 8,
            255,
    ...
    // MARK: W12 — the settings overlay REACHES the FEC resolution (SLOPDESK_FEC_M / _FEC_K)
    ...
    /// toggle of `SLOPDESK_FEC_M` / `_FEC_K` actually moves the parity/group the codec is built with.
    ...
    func testOverlayReachesFECResolution() {
        ...
        EnvConfig.overlay["SLOPDESK_FEC_M"] = "3"
        EnvConfig.overlay["SLOPDESK_FEC_K"] = "8"
        XCTAssertEqual(mlf.configEnv["SLOPDESK_FEC_M"], "3", "overlay value flows into configEnv")
        XCTAssertEqual(mlf.resolveParityCount(env: mlf.configEnv), 3, "overlay SLOPDESK_FEC_M=3 ⇒ m=3")
        XCTAssertEqual(mlf.resolveGroupSize(env: mlf.configEnv), 8, "overlay SLOPDESK_FEC_K=8 ⇒ k=8")
    }
