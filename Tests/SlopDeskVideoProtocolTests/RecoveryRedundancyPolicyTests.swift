...
final class RecoveryRedundancyPolicyTests: XCTestCase {
    ...
    /// `SLOPDESK_ESCALATION_FLOOR_MS` env resolution: default 60 ms, clamp 20...500,
    ...
    func testEscalationFloorEnvResolution() {
        ...
        XCTAssertEqual(RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "100"]), 0.1)
        XCTAssertEqual(RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "20"]), 0.02)
        XCTAssertEqual(RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "500"]), 0.5)
        XCTAssertEqual(
            RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "19"]),
            0.06,
            "below the clamp → default",
        )
        XCTAssertEqual(
            RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "501"]),
            0.06,
            "above the clamp → default",
        )
        XCTAssertEqual(RecoveryPolicy.escalationFloorSeconds(env: ["SLOPDESK_ESCALATION_FLOOR_MS": "garbage"]), 0.06)
    }
