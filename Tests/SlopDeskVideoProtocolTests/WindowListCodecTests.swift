...
final class WindowListCodecTests: XCTestCase {
    ...
    func testWindowListRoundTripPreservesEveryField() throws {
        let windows = [
            ...
            WindowSummary(windowID: 464, appName: "Ghostty", title: "/Users/dev", width: 1408, height: 889),
    ...
    func testNonASCIITitleRoundTrips() throws {
        ...
        XCTAssertEqual(
            try VideoControlMessage.decode(VideoControlMessage.windowList([w]).encode()),
            .windowList([w]),
            "UTF-8 app/title survive the length-prefixed round-trip",
        )
    }
    ...
    func testRealisticListStaysModestlySized() {
        ...
        let windows = (0..<40).map {
            WindowSummary(
                ...
                title: "A reasonably long window title \($0)",
                width: 1920,
                height: 1080,
            )
        }
