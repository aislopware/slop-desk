...
///
/// It prints a µs/op table and asserts loose ceilings against the BEST of three timed passes, so a
...
final class TerminalBlockStoreBenchTests: XCTestCase {
    ...
    func testBlockStoreCostIsMeasuredNotAssumed() {
        ...
        var grow = UInt32(0)
        let upsertInPlace = usPerOp(20000) {
            grow &+= 1
            model.upsert(
                index: 63,
        ...
        let upsertEvicting = usPerOp(20000) {
            newIndex &+= 1
            model.upsert(
                index: newIndex,
        ...
        let filtered = usPerOp(20000) { sink &+= model.blocks(filter: .failed).count }
        let navigator = usPerOp(20000) { sink &+= model.navigatorBlocks.count }
        let statusRead = usPerOp(20000) {
            for block in model.blocks { sink &+= block.isFailed ? 1 : 0 }
        }
        let labelRead = usPerOp(20000) {
            for block in model.blocks { sink &+= block.durationLabel?.count ?? 0 }
        }
        let rows = model.navigatorBlocks
        let jump = usPerOp(20000) {
