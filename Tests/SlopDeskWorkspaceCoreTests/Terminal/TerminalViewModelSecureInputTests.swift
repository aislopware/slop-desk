...
/// directly (no leaf / SwiftUI render — the hang-safety rule) by stating `controls.auto-secure-input` as the
/// running configuration, then assert the pill mirror reconciles. ``stateSetting(_:_:)`` registers its own
/// teardown, so the process-global reading never leaks into another test.
@MainActor
final class TerminalViewModelSecureInputTests: XCTestCase {
    ...
    func testReconcileHidesPillWhenAutoTurnedOffLive() {
        stateSetting("controls.auto-secure-input", true)
        ...
        stateSetting("controls.auto-secure-input", false) // the user turns the setting OFF in Settings
        XCTAssertTrue(
    ...
    func testReconcileLeavesManuallyHeldPillLit() {
        ...
        stateSetting("controls.auto-secure-input", false)
        model.reconcileSecureInputSetting()
