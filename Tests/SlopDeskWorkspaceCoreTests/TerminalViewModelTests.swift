...
/// `SlopDeskClient.Event`s + `output` chunks into observable connection / title / byte-count /
...
final class TerminalViewModelTests: XCTestCase {
    ...
    /// The begin/end bracket is called defensively (e.g. `SlopDeskSplitViewController.viewWillDisappear`),
    ...
    func testAwaitingReflowClearsOnDisconnectAndExit() {
        for drop in [SlopDeskClient.Event.disconnected(reason: "drop"), .exit(code: 1)] {
            let model = TerminalViewModel()
            model.resizeSink = { _, _ in }
            model.sendResize(cols: 80, rows: 24)
