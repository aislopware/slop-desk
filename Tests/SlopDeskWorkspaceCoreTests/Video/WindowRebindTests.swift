...
final class WindowRebindTests: XCTestCase {
    ...
    func testLiveIDSameAppKeeps() {
        let list = [win(58, "Code", "main.swift — slopdesk"), win(215, "Cua Driver", "")]
        XCTAssertEqual(WindowRebind.resolve(
            windowID: 58,
            appName: "Code",
            title: "main.swift — slopdesk",
            in: list,
        ), .keep)
    }
    ...
    func testRecycledIDRebindsByApp() {
        let list = [win(215, "Cua Driver", ""), win(58, "Code", "main.swift — slopdesk")]
        XCTAssertEqual(
            WindowRebind.resolve(
                windowID: 215,
                appName: "Code",
                title: "old.swift — slopdesk",
                in: list,
            ),
            .rebind(list[1]),
        )
