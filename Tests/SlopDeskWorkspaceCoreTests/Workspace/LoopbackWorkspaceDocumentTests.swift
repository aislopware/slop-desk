...
final class LoopbackWorkspaceDocumentTests: XCTestCase {
    ...
    /// The loopback and the host answer the SAME intent script with the same bytes.
    ///
    /// The decision function is shared — both call ``WorkspaceIntentApplier`` — but the versioning
    /// around it is not, and drift there is invisible to a suite that only ever runs one of the two.
    func testTheLoopbackAndTheHostDocumentAgreeByteForByte() async {
        let s = seed()
        var opening = HostWorkspaceState()
        opening.write(topology: WorkspaceTopology(tree: s.workspace))

        let box = WorkspaceMirrorBox()
        let loopback = LoopbackWorkspaceDocument(box: box)
        loopback.install(opening, pristine: true)
        let host = HostWorkspaceDocument()
        ...
        let script: [(WorkspaceIntentOp, Data)] = [
            ...
            (.renameSession, WorkspaceIntentArgs.encode(id: s.session.raw, name: "slop")),
            (.setSyncInput, WorkspaceIntentArgs.encode(id: s.tab.raw, flag: true)),
            (.renameTab, WorkspaceIntentArgs.encode(id: s.tab.raw, name: "build")), // a no-op
            (.setZoom, WorkspaceIntentArgs.encode(id: s.pane.raw, flag: true)),
            (.focusTab, WorkspaceIntentArgs.encode(tab: s.other)),
            (.closeTab, WorkspaceIntentArgs.encode(tab: s.other)),
            (.renamePane, WorkspaceIntentArgs.encode(id: UUID(), name: "ghost")), // a refusal
        ]
        for (op, args) in script {
            let intent = WorkspaceIntent(intentID: UUID(), op: op.rawValue, args: args)
            let mine = loopback.serve(intent)
            let theirs = await host.apply(intent: intent.op, args: intent.args)
            XCTAssertEqual(mine, theirs, "op \(op)")
        }

        let hostState = await host.snapshot
        let hostStateNum = await host.stateNum
        let hostIsPristine = await host.isPristine
        XCTAssertEqual(
            WorkspaceStateCodec.encodeSnapshot(loopback.snapshot),
            WorkspaceStateCodec.encodeSnapshot(hostState),
        )
        XCTAssertEqual(loopback.stateNum, hostStateNum)
        XCTAssertEqual(loopback.isPristine, hostIsPristine)
    }
