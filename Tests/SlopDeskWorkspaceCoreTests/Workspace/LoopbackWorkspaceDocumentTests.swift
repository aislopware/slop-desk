...
final class LoopbackWorkspaceDocumentTests: XCTestCase {
    ...
    //
    // `testTheLoopbackAndTheHostDocumentAgreeByteForByte` used to live here: it ran one fixed intent
    // script through BOTH documents and compared the encoded snapshots, because the decision function
    // is shared (`WorkspaceIntentApplier`) but the versioning around it was written twice.
    //
    // `docs/60` F.9 deleted the Swift host, so the second document is `rust/slopdesk-hostserver`'s
    // `workspace.rs` and no Swift test can reach it. The pin is NOT dead — it is now a CROSS-LANGUAGE
    // one, and the repo already has the mechanism for those: `golden/golden_vectors.json`. The script
    // above and the snapshot it produces belong there, generated by `Sources/slopdesk-corevectors` and
    // verified by `slopdesk-devtools`'s golden gate, the same way every other two-ended wire fact is.
    // Until that group exists, the loopback's versioning is pinned only by the tests above.
