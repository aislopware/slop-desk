...
**The system calls are Rust's too**, which is the part this section used to get wrong: capture is `slopdesk-apple-sck`, encode and decode are `slopdesk-apple-vt`, input injection is `slopdesk-apple-cgevent`, the display list is `slopdesk-apple-cgdisplay`, the accessibility tree is `slopdesk-apple-ax`, and the PTY is `slopdesk-superd` over `slopdesk-posix`. The GUI video HOST that drives all of those is `slopdesk-videohostd` — a Rust daemon since [61](61-videohost-deletion.md) deleted the `SlopDeskVideoHost` target and the Swift entry point beside it, so PATH 2's two ends are no longer the same language. What is genuinely left to Swift is Metal/CAMetalLayer rendering, Network.framework on the CLIENT, and the two view layers.
