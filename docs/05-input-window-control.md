...
| Swipe between pages (horizontal swipe, any speed) | same scroll stream | **Translated on HOST**: `SwipeNavRecognizer` on the injected stream → ⌘[ / ⌘] when the receiving app is in `SwipeNavPolicy` (browsers — all channels — + Finder). `SLOPDESK_SWIPE_NAV` default-ON; `SLOPDESK_SWIPE_NAV_APPS` extends. Three decision points: at LIFT (≤ 450 ms began→ended, ≥ 80 pt on-glass, ≥ 3× horizontal dominance); by MOMENTUM CONFIRMATION — a sharp flick spends most of its displacement in the momentum tail, so a dominant quick lift ≥ 24 pt arms a 250 ms coast window that fires at ≥ 120 pt combined; or via the SLOW tier — natively a page-swipe works at any speed (drag, even hold, release), so past 450 ms the lift fires on COMMITMENT: a graduated SURFACE (`slowRequiredTravel`), not steps — the band's cheap-end anchor eases along the seam (450 → 700 ms: dominance 3× → 4×, travel 80 → 160 pt); at/above the anchor the requirement is the anchor's travel, below it interpolates toward the fixed 2× floor @ 240 pt; below 2× nothing fires at any travel. ONE joint interpolation, continuous in both axes (field-tuned 2026-07-17: both step cliffs ate real swipes; a first cut that `min`-combined two independent interpolations folded a NEW cliff along their crossing — property-test-pinned smooth now); no upper duration bound (`SLOPDESK_SWIPE_NAV_SLOW=0` restores the flick-only gate, grace included). Momentum can only confirm what a flick lift armed — diagonal/modest content pans keep scrolling only. UDP-loss tolerant: a lost `began` is synthesised from the first continuous `changed`, a lost `ended` from the first momentum event. WINDOW-scoped panes additionally gate the chord on the pane's app being frontmost AT FIRE TIME (the HID-tap post lands in the OS key-focus holder — a mismatch suppresses and raises the target instead of outdenting whoever holds focus), and the status push mirrors the same gate so the chip goes dark while the pane's app isn't frontmost (≤2 s stale, like display eligibility). `SLOPDESK_SWIPE_NAV_TRAVEL` scales the threshold family (default 80); `SLOPDESK_SWIPE_NAV_TRACE` logs per-gesture verdicts (travel/duration/dominance) plus a per-injector REGIME banner naming the threshold family, so logs spanning deploys self-describe. |
| — swipe-peel FEEDBACK (client, macOS) | same scroll stream, mirrored locally | **The piece key translation can't give**: something reacting WHILE the fingers are on the glass. The client runs its own `SwipeNavRecognizer` (in `SwipePeelPlanner`) over the very stream it forwards and, from ≥ 24 pt decisively-horizontal travel, shows a chevron-in-circle chip at the pane edge — the WHOLE visible feedback. The streamed image never moves (v6 HW verdict: a remote pane is a window onto a desktop, so translating it reads as dragging the pane, not peeling a page — the v5 1:1 page-follow + commit snapshot choreography were removed, see DECISIONS). To still live with the finger, the chip EMERGES from its edge as the fill grows (tucked ~12 pt at the arm line, fully out at commit); its ring fills toward the live tier's commit threshold, turning solid + one trackpad haptic tap the instant "release now navigates". Commit ⇒ a confirm pulse (scale up + dim, ~150 ms) settling into a DIM HOLD (~35 % opacity) until the ~520 ms clear fades it out — pulse + hold genuinely span the 150–400 ms beat where the host's ⌘[/⌘] lands and the post-navigation page streams in (fading to zero would end the visible acknowledgement at ~150 ms). Reject/coast-expiry/reroute ⇒ the chip fades out. Reduce Motion ⇒ no tuck emergence, no scale pulses — fades only (ring fill, solid state and the haptic stay: information, not motion). Gated + configured by the host's `SwipeNavStatusMessage` push (cursor socket type 3, doc 20 §9.6): no push or `eligible=false` ⇒ no chip — the affordance never lies in an app where ⌘[ would edit text, and a host-side `SLOPDESK_SWIPE_NAV_TRAVEL`/`_SLOW` retune re-tunes the mirror. The push also carries the target's LIVE canGoBack/canGoForward (`slopdesk-videohostd`'s `navhistory`: AX read of the ⌘[/⌘] menu pair, or Safari's `BackButton`/`ForwardButton` toolbar identifiers — its lazily-validated menus go stale; ~250 ms change-poll): a swipe toward a DEAD direction (Back/Forward greyed out) shows no chip at all and a mid-gesture flip retracts it (a CONFIRMING chip is exempt — the fired back-nav itself flips canGoBack within one poll and must not cut its own 520 ms acknowledgement hold). Unknown history (non-browser target, AX failure, `SLOPDESK_SWIPE_NAV_HISTORY=0`) fails OPEN to pre-gate behavior. The HOST still fires the chord regardless — it is a validated-menu no-op when dead, so a stale read can only cost feedback, never a navigation. |
...
| Smart zoom (double-tap) | `smartMagnify(with:)` | **Translated on CLIENT**: ⌘0 (actual size) — pairs with the pinch ladder. Same flag. Skipped where ⌘0 is NOT a zoom reset (`PinchZeroPolicy` denylist — Xcode toggles its Navigator; `SLOPDESK_PINCH_ZERO_UNSAFE_APPS` extends by app display name; desktop panes fail open — the frontmost remote app is unknowable client-side). ⌘=/⌘− stay ungated. |
