...
- **Read-only avoids the cost of an SDK-driven interactive pane:** no reimplementing slash/model/permissions (the TUI handles input), no duplicate prompts, no `--bare`/`settingSources` concerns. We **observe** the transcript, never **drive** the agent — no inspector→`claude` input path, so read-only by construction.
- **Same session as the TUI** (not a second SDK session): reads the transcript of the very `claude` process running.
- **Better than Happy/Happier** (structured only, losing TUI fidelity): we keep **full TUI + inspector**.

## Data source: tail the JSONL transcript (+ supplementary hooks)

`claude` writes append-only JSONL at `~/.claude/projects/<encoded-cwd>/<session-uuid>.jsonl` (override via `CLAUDE_CONFIG_DIR`). **Take the path from the `transcript_path` field in the hook payload** (`SessionStart`) — do not reconstruct it.
...
| **CoT/thinking** | ⚠️ mostly EMPTY on Opus 4.x | `{type:thinking,thinking:"",signature}` block always structurally present but text empty by default — see below |
...
> ⚠️ **CoT/thinking — heaviest caveat:** on **Claude 4 (Opus 4.5/4.7/4.8 = our stack)** the `thinking` field is **EMPTY** by default (`display:"omitted"` for Opus 4.7+; `showThinkingSummaries` not wired to `thinking.display`). Workaround: the **undocumented** flag `claude --thinking-display summarized` populates the text (may break on updates). `redacted_thinking` (encrypted) is Claude 3.7 only, not Claude 4. → The inspector must be defensive: `thinking===""` + a `signature` present → render placeholder "Thinking (not persisted)". **Load-bearing decision (see end).**

## "Workflow" = Dynamic Workflows (new feature)

CC ≥ **v2.1.154** (released 2026-05-28, **research preview**). A **JS orchestration script** Claude writes itself, running in a runtime **separate** from the conversation, spawning ≤16 concurrent / 1000 total agents, intermediate results in **script variables** (outside the context). Triggers: keyword `workflow`, `/effort ultracode`, `/deep-research`. Saved in `.claude/workflows/`. UI: `/workflows`. Disable: `CLAUDE_CODE_DISABLE_WORKFLOWS=1`.
- **Observing:** NO dedicated JSONL event, NO WorkflowStart/Stop hook. Indirect via **SubagentStart/Stop hooks** + sidechain files. ⚠️ **Gap:** main JSONL is **sparse/silent** throughout a long workflow run (results live in script vars) → the inspector must show "workflow running" (via Subagent hooks) instead of appearing hung.
...
**Data flow:** host spawns `claude` (PTY) + registers a `SessionStart` hook (POST to a local listener) → the inspector daemon opens `transcript_path`, FSEvents-watches it + `subagents/` → parses into typed events → a **second length-prefixed TCP/NWConnection** (JSON frames), separate from the PTY byte stream → a client-side Swift actor keeps an ordered store → SwiftUI read-only views. (Optional: PostToolUse/SubagentStop HTTP hooks for low-latency push, backfilled by JSONL later.)
...
**Sync/dedup:** a `processedMessageKeys` Set (keyed by `uuid` / `sidechain:<id>:<uuid>` / ...); files are append-only → re-read the tail (cap ~1MB), `JSON.parse` each line in try/catch (a line may be mid-write); `.passthrough()` to tolerate unknown fields (schema stable only at the discriminated-union level); skip internal types (`file-history-snapshot`,`queue-operation`,`rate_limit_event`); reconnect = host assigns a monotonic `seq`, client sends `lastSeq` → replay.
...
## Phasing
- **P1 (MVP, 80% of value):** tail main session JSONL + SessionStart hook; tool-call cards + timeline + todos.
- **P2:** subagent tree (watch `subagents/`, SubagentStop hook).
- **P3:** workflow panel + agent teams inbox (experimental, defer).
- **CoT:** placeholder-only in every phase (settled — no "CoT text" phase).

## Settled decisions
1. **CoT/thinking = PLACEHOLDER ONLY** ✅ — render "Thinking (not persisted)" + a signature fingerprint when `thinking===""`. Do **NOT** pursue the undocumented `--thinking-display summarized` flag (fragile). → drop the P4 "CoT text" phase; if Anthropic later persists thinking by default it displays naturally (the render slot already exists).
...
## Confidence (honest)
- ✅ High: JSONL is the right source; tool input/output/todos complete; hooks supplement it; `agent_transcript_path` is real; Workflow = Dynamic Workflows v2.1.154+.
- ⚠️ Uncertain (load-bearing): thinking empty on Claude 4 (model+flag dependent; corpus has conflicting verdicts); subagent/team paths from Happier source + observation, **not official** → verify on the target CC version; schema stable only at the union level → `.passthrough()`.
