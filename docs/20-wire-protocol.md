...
- **`foregroundProcess` (26) and `claudeStatus` (27) carry per-pane Claude-Code agent status**
  ...
  - **`claudeStatus`** is the rich hook path: the host folds Claude Code `Notification`/`Stop`/
    `SessionEnd` hooks (via `slopdesk_agent_detector_hook`) into `[UInt8 state][UInt8 kind][UInt16
    ...
    which says this transition is BOOKKEEPING and must raise no attention (no toast/banner/sound/unread;
    the dots still move). `4` rides a `/compact` boundary's `idle`, whose `working → idle` shape is
    otherwise the hook-less completion edge. `label` = the (often empty, length-prefixed)
    Stop/Notification message, capped at the UInt16 length field. The wire carries the RAW `state`/`kind` bytes —
...
- **`metadataRequest` (16) and `metadataResponse` (30) are the generic host-metadata RPC** (E4,
  ...
  - **`ensureSimulatorServer` (21) is the embedded-simulators verb** (2026-08-04 — the client's
    ...
    **empty** — simulators are a MACHINE resource, not a project one, so unlike 18 there is nothing
    ...
    `baguette` binary on the host → the tab shows the `just provision` hint; the version is pinned in `ThirdParty/tools/tools.lock`). `port` is meaningful ONLY when ready (the child
  ...
  - **`ensureAndroidBridge` (22) is the embedded-Android verb** (2026-08-04 — the client's
    ...
    `2` unavailable (no `adb` on the host → the tab shows the `just provision` hint; `adb` is pinned
