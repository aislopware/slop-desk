...
|-------|--------|-------------|
| GUI **capture + HW encode** + input injection + UDP transport | `AislopdeskVideoHost` (`WindowCapturer`, `VideoEncoder` 2-session, `InputInjector`, `CursorSampler`, `NWVideoDatagramTransport`, `AislopdeskVideoHostSession` orchestrator) | `SCStream` capture **and** `VTCompressionSession` HW encode **HANG** without a window-server + Screen-Recording TCC session — measured in [`research/spikes/vtbench/RESULTS.md`](research/spikes/vtbench/RESULTS.md) ("the HW VideoToolbox encode harness **HANGS when run over SSH**"). Only the host **pure logic** (session state machine, datagram routers, send scheduler) is tested; no test imports ScreenCaptureKit/VideoToolbox/Metal (verified by grep of `Tests/`). |
