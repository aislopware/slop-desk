...
> **SlopDesk** (WF-1 … WF-9, consolidated in WF-10). It states what is COMPLETE + tested
...
>   `SlopDeskClient.sendInput`/`sendResize`. So the GUI terminal is interactive (typing + resize),
>   not just a read-only render. (Was the documented "remaining seam".)
> - **PATH 2 now has BOTH ends**: a new host daemon **`slopdesk-videohostd`** (enumerate windows,
>   serve one via `SlopDeskVideoHostSession`) + a client **Remote-window panel** (endpoint form →
...
| **Reconnect hardening** — fixed the resume-rebind race: dead-channel send is `ECANCELED`→`notConnected`→**retain-not-throw** (bytes re-sent from replay buffer), gate on new-channel readiness, client clean-FIN→reconnect. Verified `HandshakeReconnectTests` **100/100** consecutive. | `SlopDeskTransport` | 96 (cumulative) | `4288535` |
...
|-------|--------|-------------|
| GUI **capture + HW encode** + input injection + UDP transport | `SlopDeskVideoHost` (`WindowCapturer`, `VideoEncoder` 2-session, `InputInjector`, `CursorSampler`, `NWVideoDatagramTransport`, `SlopDeskVideoHostSession` orchestrator) | `SCStream` capture **and** `VTCompressionSession` HW encode **HANG** without a window-server + Screen-Recording TCC session — measured in [`research/spikes/vtbench/RESULTS.md`](research/spikes/vtbench/RESULTS.md) ("the HW VideoToolbox encode harness **HANGS when run over SSH**"). Only the host **pure logic** (session state machine, datagram routers, send scheduler) is tested; no test imports ScreenCaptureKit/VideoToolbox/Metal (verified by grep of `Tests/`). |
| GUI **decode + Metal render** + client cursor + UDP transport + display-link pacing | `SlopDeskVideoClient` (`VideoDecoder`, `MetalVideoRenderer`, `ClientCursorCompositor`, `FramePacer` CVDisplayLink/CADisplayLink, `NWVideoClientTransport`, `SlopDeskVideoClientSession` orchestrator) | Decode is MEASURED-safe (~0.9–1.1 ms synchronous), but to honour the same hang-safety rule no `VTDecompressionSession`/Metal device/display link is instantiated in tests — only the client pure logic (frame-pacer newest-wins, reassembly pacing, scale math, parameter-set parse) is. |
...
| iOS **input responder host** + UIKit table-stakes wrappers | `SlopDeskClientUI` (`TerminalInputHost` — the `UIResponder`/`UIViewRepresentable` that assembles `KeyRepeater` + `KeyboardAccessoryBar` + `IMEProxyTextView` + `FloatingCursorController` and routes to `SlopDeskClient.sendInput`; `InputBarView` uses it on iOS, macOS path unchanged) | The responder host **compiles for iOS via `scripts/check-ios.sh`** and is code-reviewed, but its on-device interaction — key-repeat cadence under real `pressesBegan`/`pressesEnded`, IME multi-stage composition, the floating-cursor gesture — is **unverified**: it is iOS-only `UIResponder`/`UIView` glue, not unit-testable on macOS, and has not been run on a simulator/device. The underlying logic (repeater cadence, cursor delta→arrow, accessory decision, IME routing) IS pure + macOS-unit-tested. |
...
> (`Sources/SlopDeskVideoClient/VideoWindowView.swift`). With a `nil` connection the backing
> `MetalVideoLayerView` builds the Metal chrome but **does not bring up the
> `SlopDeskVideoClientSession` orchestrator** — no UDP sockets open, no decoder, no display link,
...
5. **Remaining wiring seam (small, documented honest gap).** The renderer's OUT path —
   ...
   `SlopDeskClient.sendInput(_:)`. The `GhosttyTerminalView` is handed only the `TerminalViewModel`
   by the factory closure (the model has no input sink and does not hold the live client), so
   the connection layer that owns the `SlopDeskClient` sets `model.surface?.onWrite = { bytes in
...
2. **Two-machine terminal test over NetBird.** Run `slopdesk-hostd --port 7420` on the host
   machine; run `slopdesk-client --host <netbird-ip> --port 7420` on the M2 Pro client. Confirm
...
3. **Wire `GhosttySurface` into the macOS client app** via
   `SlopDeskClientUI.TerminalRendererFactory.shared` (see `Apps/Shared/AppMain.swift`) so the
...
4. **Wire a `VideoWindowConnection` host endpoint into the app** so the live PATH 2 pipeline
   ...
   `SlopDeskVideoClientSession` orchestrator actually comes up. Until this is done the live decode
   pipeline never starts in the app.
5. **Grant TCC for the GUI video path** on the host: **Screen Recording** (capture),
   **Accessibility** + **Post Event** (input injection). Then exercise `SlopDeskVideoHost`
   capture/encode → `SlopDeskVideoClient` decode/render for a GUI window. (Run capture/encode
...
3. **Real 2-machine terminal test over NetBird** (host ↔ M2 Pro client): interactive shell +
   `slopdesk-hostd --claude` (Claude Code) + reconnect — this validates the entire PATH 1 on
   hardware.
...
- **iOS input responder host is wired but on-device-unverified.** `TerminalInputHost` (the
  ...
  `SlopDeskClient.sendInput`; `InputBarView` uses it on iOS (macOS path unchanged). It
...
- **GUI video path is built to MEASURED spike configs, never executed — and not even started
  ...
  UDP orchestrators (`SlopDeskVideoHostSession` / `SlopDeskVideoClientSession`) + display-link pacing
...
- **One latent package-graph note (non-blocking):** addressed in WF-8 review by removing the
  inert `import SlopDeskTerminal` from `SlopDeskClient.swift` (the terminal surface is a closure
