...
pipeline, and inspector channel are PROVEN — this document wraps them, never reinvents them.

Base design: **domain-model-first** (tree-of-intent vs table-of-liveness — the test story that
survives the concrete-actor reality), grafted with a SwiftUI-native shell, pure
`LayoutSolver`/`FocusResolver` + virtual-clock `CommandInterpreter`, and a single-switch responsive
projection (`CompactLayoutResolver`).
...
1. **Pure domain (~85% of new logic) has no client at all.** Pane tree, layout solver, focus
   resolver, compact projection, command interpreter, persistence codec — all value types and free
   functions. Plain synchronous `XCTest`: no async, no network, no stub.
...
- **The tree of intent** — a pure, value-typed, `Codable` `Workspace { tabs: [Tab] }`; each `Tab`
  owns a recursive `PaneNode` tree whose leaves carry a `PaneID` + a value-typed `PaneSpec` (kind +
  endpoint, NEVER a live object). Every layout mutation (split, close, resize, zoom, focus, reorder,
  rename, add/close tab) is a PURE function returning a new tree. This is the serialization format and
  ~85% of the test surface.
...
- **The table of liveness** — a `[PaneID: any PaneSessionHandle]` registry inside the one
  ...
  `InspectorViewModel/Client` for claudeCode, or a `RemoteWindowModel` for remoteGUI).

The store **reconciles** the two: after every tree mutation, `reconcile()` diffs `tree.allLeafIDs()`
against `registry.keys`, materializes missing leaves (via `makeSession`) and tears down orphans (via
the proven teardown order). The tree never holds a session; the registry never holds layout.
...
- The four byte-pipeline invariants hold **by construction**: keying the registry by identity
  guarantees one `ConnectionViewModel` (hence one ordered-OUT stream, one events consumer, one
  `ReconnectManager`) per pane, never shared.
- Persistence is free — the tree is already `Codable`.
- Responsive is a pure VIEW-time projection of the SAME tree: compact = render only the focused leaf
  (an always-on zoom). One model, two renderings.
- It survives the concrete-actor reality: the testable layers don't depend on a fakeable client; the
  one layer that touches the client is faked at the `PaneSession` boundary.

### 1.3 Grafts

- **Native shell:** `NavigationSplitView` is the responsive spine — free sidebar+detail, native macOS
  source-list, compact collapse-to-stack. The ONLY size-class branch is in `WorkspaceRootView.detail`.
  `.draggable`/`.dropDestination` for tab reorder. `.geometryGroup()` + a resize-forwarding throttle
  protect the proven `sendResize` path.
- **One geometry source of truth:** a pure `LayoutSolver` produces a single `[PaneID: CGRect]` map
  consumed by BOTH the rendered layout AND `FocusResolver.neighbor`, so "move focus left" can never
  disagree with what the user sees. A pure `CommandInterpreter` on a virtual clock (the
  `ManualRepeatScheduler` pattern) unit-tests keyboard intent.
- **One responsive switch + pure projection:** compact layout is the pure `CompactLayoutResolver`
  (tree + focus + swipe -> next focus / page index), unit-tested on macOS with zero UIKit. Compact =
  exactly one mounted `TerminalInputHost`, sidestepping the iOS two-first-responder race on phone.
- **One fan-out site:** `pause/resume` route through `PaneSessionHandle.pause()/resume()` (covering
  connection + inspector channel), iterated in a `TaskGroup` AWAITED before suspend.
...
`reconcile()` is the single audited seam. Contract (tested): after any op,
`Set(registry.keys) == Set(workspace.tabs.flatMap { $0.root.allLeafIDs() })`; idempotent; a
...
No native cross-platform N-way resizable splitter exists (`NSSplitView`/`HSplitView` is AppKit-only
and forbidden), so this is the ONE hand-rolled view — minimal, with all math in the pure
`LayoutSolver`/`PaneNode.settingFractions`.
...
- **Render:** `SplitContainer` is one `GeometryReader`; children laid out along `axis` with
  `.frame(width/height: fraction × total)`, native `Divider()` between each pair, overlaid with an 8pt
  `DividerHandle` (`.onHover { NSCursor.resizeLeftRight }` on macOS; wider `contentShape` hit area on
  iOS-regular touch).
- **Resize:** the handle's `DragGesture` converts translation to a fraction delta and calls pure
  `PaneNode.settingFractions` via the store, clamped to `minFraction = 160pt / total`. Ratios live in
  the model (resolution-independent, persists, round-trips). The OUT path is protected: `sendResize`
  forwarding is **throttled** (write the model live for layout, forward only on drag-ended + a
  low-rate sample) so a drag never floods `TIOCSWINSZ`; `lastSentSize` coalescing absorbs duplicates.
  `reset()` is NEVER called on a transient hide/zoom.
- **Zoom/maximize:** `Tab.zoomedPane` is a presentation flag. `PaneTreeView` checks it at the top and
  renders only that leaf full-bleed; tree and registry are untouched (no materialize/teardown on
  zoom). Zoom is presentation, not tree surgery.
...
detail resolves regular). Computed once in `WorkspaceRootView` — the ONLY adaptation switch in the app.
...
- **Regular (macOS, iPad full-screen/regular split):** `NavigationSplitView` sidebar (tab rail) + full
  recursive `PaneTreeView` with draggable dividers, zoom, multi-pane visible. `minWidth: 720` on the shell.
- **Compact (iPhone, iPad slide-over):** the SAME tree is flattened by `CompactLayoutResolver.pages`
  into a `TabView(.page)` carousel — exactly ONE leaf visible (the focused one, an always-on zoom).
  Page selection is BOUND to `tab.focusedPane`; first-responder is driven EXPLICITLY on focus change
  (not `.onAppear`) via `PaneFocusCoordinator`. The sidebar collapses into the `NavigationSplitView`
  stack. No dividers; `split` still mutates the tree (round-trips to desktop) but compact gains a swipe page.
...
- **Structural win:** compact = one mounted `TerminalInputHost`, sidestepping the iOS
  two-first-responder race on phone. Regular-iPad is the only path needing the focus coordinator (below).

---

## 5. Keyboard / command model

A pure command-intent layer drives both platforms; SwiftUI `Commands` (macOS menu bar + iPad hardware
`UIKeyCommand`) and `.keyboardShortcut` are thin adapters over a tested core.
...
- **macOS:** `WorkspaceCommands: Commands` adds native "Pane" + "Tab" menus with these shortcuts; each
  item calls `apply(_:to:)`.
- **iPad hardware keyboard:** the same `Commands` surface as `UIKeyCommand` in the ⌘-hold HUD; focus is
  published via `FocusState<PaneID?>` + `.focusedSceneValue` so commands act on the focused pane.
- **Compact iPhone (no hardware keyboard):** on-screen affordances — `.contextMenu` on the pane header
  (split/close/zoom), swipe between leaves, tab drawer. The command model degrades gracefully.
- **CONFLICT RULE (load-bearing):** the terminal must keep receiving raw bytes. All workspace
  ...
  Ctrl-letters flow to the focused terminal untouched. Focus-move uses ⌥⌘+arrows because plain arrows
  belong to the shell. No bare-key binding ever.
...
- **Versioning + safety:** top-level `schemaVersion: Int`; decode failure falls back to a single
  default terminal tab rather than crashing. Byte-stable round-trip tests guard the hand-rolled Codable.
- **RESTORED vs RECONNECTED (the discipline):** persistence restores SHAPE and INTENT, never live
  ...
  compact: the focused page) — restoring a 12-pane workspace does not slam 12 sockets at launch. Video
  panes restore with the endpoint pre-filled but NOT auto-opened (UDP is user-initiated). Stable
  `PaneID`/`TabID` survive the round-trip so focus/zoom references stay valid. NOT persisted: live
  objects, byte buffers, sessionIDs (a relaunch is a fresh session — the host's tail-retention is for
  live pause/resume, not cold restart).
...
- **TerminalRendererFactory** (`Terminal/TerminalRenderingView.swift:76`, registered once in
  ...
  `TerminalScreenView(model: handle.terminalModel)` → `TerminalRendererFactory.make`. The factory is
  stateless; N leaves = N `make` calls, each with its own `TerminalViewModel`. `.id(PaneID)` forces
  SwiftUI to give each its own `GhosttySurface` and never reuse one across panes (the
  `TerminalScreenView` `@State` capture hazard). **PaneID is stable for the session's lifetime — only a
  true session swap changes it; resize/focus/zoom re-renders never do.**
- **VideoWindowFactory** (`Video/VideoWindowSeam.swift:69`, registered once in `AppMain.swift`):
  ...
  (the one genuine edit — add `showCloseButton: Bool = false` so pane chrome owns close; current init
  is `(model:)` only at `RemoteWindowPanel.swift:85`, Close hardcoded at line 99) → `VideoWindowFactory.make`.
  Each leaf's `MetalLayerBackedView` owns its own `VideoWindowPipeline`; SwiftUI `dismantle` fires
  `deactivate()` on removal (backstop preserved). `activate`/`deactivate` is gated on
  `.onAppear/.onDisappear` (decode only on-screen/focused panes), and the store enforces `liveVideoCap`
  to bound the 2N-UDP-sockets / N-VTDecompression / N-CVDisplayLink ceiling.
- **Inspector** (`Inspector/InspectorPanel.swift`, `InspectorChannel.swift`): a `.claudeCode` leaf is a
  per-pane composite — `TerminalScreenView` + `InspectorPanel(model: handle.inspector.model,
  client: handle.inspector.client)`, ratio in the pane's own state. The store opens NWConnection #2
  per claudeCode pane via `makeInspector` (the genuinely missing app-glue, wired here). Single-consumer
  rule holds: the panel's own `.task` is the sole `events()` drain; read-only — no path back to OUT.
...
`TerminalInputHost.swift:52` does a bare `DispatchQueue.main.async { becomeFirstResponder() }` with NO
generation counter and NO resign-before-become. For regular-iPad multi-visible-pane we add:
...
The async `becomeFirstResponder` captures the generation at dispatch and no-ops if stale. Compact mode
(one mounted host) needs none of this. This surface is only typecheck-covered by `check-ios.sh` — **a
real-device pass on iPad-regular is required** and is NOT assumed free.
...
- `PaneNodeTests`: split (each axis, root/nested/deep), close (collapse singleton, empty→nil, refocus
  neighbor, renormalize fractions), `allLeafIDs` ordering, `updatingSpec`.
...
- `CompactLayoutResolverTests`: pages order = pre-order leaves; selectedIndex; swipe→next focus —
  the phone layout, tested on macOS with zero UIKit.
...
- `WorkspaceStoreReconcileTests`: inject `makeSession: { spec in FakePaneSession(spec) }`. Assert split
  adds exactly one handle for the new PaneID; closePane tears it down (FakePaneSession records
  `teardown()`); `Set(registry.keys) == Set(allLeafIDs)` after every op; reconcile idempotent; a
  compact↔regular flag flip does NOT mutate the registry.
- `ScenePhaseFanOutTests`: N FakePaneSessions; `pauseAll()`/`resumeAll()` calls `pause()`/`resume()`
  on EVERY handle, AWAITED (TaskGroup), inspector-bearing ones included.
...
- `PaneTreeView`/`SplitContainer`/`WorkspaceRootView`/`PaneCarouselView` bodies are thin (all math in
  pure ops). Covered by headless `swift build` + `scripts/check-ios.sh` (iOS triple typecheck) + the
  runtime `scripts/check-macos.sh`/`scripts/check-video.sh` (env vars preserved; run from a real
  unlocked GUI session).

Net: `swift build` + `swift test` (the existing suite + new pure/fake tests) + `check-ios.sh` stay
green. No new `HostServer` instance is created.
...
- `PaneNodeTests.swift`, `LayoutSolverTests.swift`, `FocusResolverTests.swift`,
  ...
  `WorkspaceStoreReconcileTests.swift`, `ScenePhaseFanOutTests.swift`, `LiveVideoCapTests.swift`,
  `Support/FakePaneSession.swift`.
...
Tree, ops, geometry, compact projection, command interpreter, persistence codec. 100% pure-unit-tested.
`swift build`/`swift test` green. Depends on: nothing.

### WF3 — Multi-session store + registry
`PaneSessionHandle`, `LivePaneSession` (wrapping the proven objects), `WorkspaceStore` + `reconcile()` +
fan-out + video cap. Tested with `FakePaneSession` via `makeSession`. NO HostServer. Depends on: WF2.
...
`check-macos.sh`/`check-video.sh` from a real GUI session. Depends on: WF4.

### WF6 — Keyboard + responsive + polish
`WorkspaceCommands` + `.keyboardShortcut`; `CommandInterpreter` wiring; `PaneCarouselView` (compact)
bound to focus; `PaneFocusCoordinator` + `FocusGenerationGuard` for iPad-regular focus; tab reorder
(`.draggable`/`.dropDestination`); persistence debounce; `check-ios.sh` + real-device iPad pass.
Depends on: WF5 (real content panes), WF2 (CommandInterpreter, CompactLayoutResolver).
...
3. **iOS first-responder race (built net-new)** → `FocusGenerationGuard` + `PaneFocusCoordinator`
   (resign-before-become); compact sidesteps it; required real-device pass.
...
6. **Persistence corruption** → hand-written discriminated Codable + schemaVersion + byte-stable
   round-trip tests + decode-failure fallback; persist value tree + endpoints only; lazy connect-on-view.
...
9. **macOS has no size class** → width fallback in the one responsive switch; a size-class flip must not
   call reconcile / drop focus / tear down sessions (idempotent projection).
10. **Automation-seam migration** → env var names unchanged; moved to store pane-0; re-run both runtime
    scripts from a real unlocked GUI session.
