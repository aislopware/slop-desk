...
- **RESTORED vs RECONNECTED (the discipline):** persistence restores SHAPE and INTENT, never live connections. On launch `WorkspaceStore(restoring:)` decodes the tree; the registry starts empty; `reconcile()` materializes an *idle* `LivePaneSession` per leaf but does NOT auto-connect. Each leaf view's `.task` triggers `connect()` lazily only when first projected (desktop: visible panes; compact: the focused page) — restoring a 12-pane workspace does not slam 12 sockets at launch. Video panes restore with the endpoint pre-filled but NOT auto-opened (UDP is user-initiated). Stable `PaneID`/`TabID` survive the round-trip so focus/zoom references stay valid. NOT persisted: live objects, byte buffers.
  <br>**SUPERSEDED by [45 — Multi-client state sync](45-multi-client-state-sync.md).** Two corrections, both live: (1) **a relaunch reattaches to the SAME live shell.** A pane's `PaneID` IS its mux `sessionID` and its document objectID — one namespace, verbatim (docs/45 §5.7) — so presenting it on `channelOpen` is what resumes the session, and Stage-2 resume is default-ON. There is no separate persisted session id and no "a relaunch is a fresh session" rule. (2) The `SlopDeskClientUI/…` paths named elsewhere in this document resolve to `Sources/SlopDeskWorkspaceCore/Workspace/`, where the store, persistence and tree ops live. The whole persistence section above describes the CANVAS-era client file; the layout itself is the host's `workspace-state.json`, and what the client keeps is `workspace-cache.json` (raw snapshot bytes) plus `device-prefs.json` (docs/45 §7.3).
...
- **TerminalRendererFactory** (`Terminal/TerminalRenderingView.swift:76`, registered once in `AppMain.swift`): `PaneLeafView` for `.terminal`/`.claudeCode` renders `TerminalScreenView(model: handle.terminalModel)` → `TerminalRendererFactory.make`. The factory is stateless; N leaves = N `just` calls, each with its own `TerminalViewModel`. `.id(PaneID)` forces SwiftUI to give each its own renderer instance and never reuse one across panes (the `TerminalScreenView` `@State` capture hazard). The seam is unchanged by `docs/68-terminal-surface-in-rust.md` — same three members, same one conformer; only what the conformer draws with moved. **PaneID is stable for the session's lifetime — only a true session swap changes it; resize/focus/zoom re-renders never do.**
...
- **App shell:** `Apps/Shared/AppMain.swift` is UNCHANGED — factory registration stays the single launch-time site and the ONLY importer of the renderer and `SlopDeskVideoClient`. (`docs/68-terminal-surface-in-rust.md` ended the clang-module half of that: there is no `CGhostty` to import, the conformer compiles inside the SwiftPM package, and registration is a `public func` the app calls.) `SlopDeskClientApp.swift` swaps its two `@State` vars for one `@State private var store: WorkspaceStore`, renders `WorkspaceRootView(store:)`, fans `handleScenePhase` over `store.allSessions` in an AWAITED `TaskGroup`, and migrates the automation seams (below).
