...
de-duplicated optimization backlog for SlopDesk.

**SlopDesk recap.** Swift 6 self-hosted remote workspace = remote TERMINAL multiplexer
...
claude-agent, remote-desktop-video}). Host = headless CLI daemons (`slopdesk-hostd`,
`slopdesk-videohostd`); a menu-bar host GUI is wanted.
...
2. **cmux and muxy are the same SwiftUI + libghostty stack as SlopDesk.** Treat their open
   ...
   directly onto SlopDesk's `WorkspaceStore` / `ConnectionViewModel` / `reconcile`.
...
a `SlopDeskStatusEntry`. The host shell emits a tiny text protocol on the existing control mux
...
**Structural insight:** SlopDesk's daemon split already matches RustDesk's "Connection
...
(`slopdesk-videohostd`) + **Accessibility** (host CGEvent keyboard/mouse injection) granted —
...
`...?Privacy_Accessibility`. Gotchas to honor: **TCC is keyed by bundle ID** (SlopDesk's
...
SlopDesk's daemons are already stream-shaped, it's headlessly testable like the existing E2E
...
only the data planes. The only new daemon surface is a place in `slopdesk-hostd` to gate
...
host password, legitimately default-on because SlopDesk is overlay-only 100.x) vs **Share now**
...
aligns with SlopDesk's per-pane model.

---

## (D) connection resilience / connect-once / reconnect

SlopDesk today has connect-once endpoint inheritance but **no dead-host timeout, no auto-
...
each with a seqnum, plus reconnect) is ~1:1 with SlopDesk's mux.
...
bounded **host-side scrollback ring per pane** (tmux model — SlopDesk ships bytes, so do NOT copy
...
zellij-without-resurrection). SlopDesk is architecturally positioned to *win* because PTYs (and
...
- **120/60 fps game-stream assumptions.** SlopDesk's GUI path is intentionally ~24–30 fps; do
...
- **mosh-style drop-scrollback state-diff for the terminal.** SlopDesk ships *bytes* and renders
...
- **Building your own NAT traversal / rendezvous + relay (RustDesk/DERP).** NetBird/Tailscale
  already solve hole-punching and relay underneath SlopDesk. Only *surface* the path/latency
