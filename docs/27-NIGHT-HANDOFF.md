...
| 7 | Resize reported as garbled — torn chars / misalignment | ✅ fixed | resize coalescing + **ctty fix** (below) |
...
- ⚠️ cua **cannot actuate `MenuBarExtra .window` popover buttons** (not in an enumerable AX window) — verify host-start via the harness/CLI, not a cua GUI click. Host default port is **7420** everywhere — one constant, in `rust/slopdesk-hostlaunch`'s `args`, ASKED for through `slopdesk_hostd_default_port` by `HostdArguments.defaultPort`, the menu-bar app's `@AppStorage` seed and the client's `ConnectionTarget.default` prefill alike. The menu-bar app used to seed 7779 while the client dialled 7420, so the two halves could not meet out of the box; an app that already stored 7779 keeps it.

### #7 Resize — size convergence + the deep repaint fix
- **Size convergence (the garbled/torn-character misalignment complaint):** latest-wins resize **coalescing** on the client OUT-path + a host-side micro-debounce (`MuxChannelSession`). HW-validated: after a fast drag, `stty size` matches the final window grid, scrollback reflows cleanly, no misalignment. Disproves the "pty uses old console size" hypothesis — the PTY now converges to the final size.
