...
`SlopDeskClientUITests` 306/0. The fix restores known-working code + repaint-safety reviewed → HIGH
...
- 🟥 **HIGH regression (caught + fixed)**: enabling `supports_selection_clipboard = true` + a
  ...
  `location` — SELECTION → a private `NSPasteboard(name: "com.slopdesk.terminal.selection")` (preserves
...
- 🟧 **DOCUMENTED (fix needs iOS runtime verification — touches the just-restored keyboard path)**: on iOS a
  pane has **TWO independent OUT drains** to the same `SlopDeskClient` actor — the keyboard
...
1. **iOS keyboard runtime verification** — the headless verification is strong; the SIM runtime was
   ...
   - The sim app did NOT connect to a loopback `slopdesk-hostd`, and screenshots showed it not frontmost
...
Ran a 6-dimension ultracode audit comparing slopdesk's terminal subsystems against canonical OSS
...
- **#1/#2 (HIGH) clipboard recursion → stack-overflow crash.** `confirm_read_clipboard_cb` re-completed
  ...
  AUTO-APPROVES (`confirmed:true`) since slopdesk has no dialog (the canonical embedder pattern — the confirm
...
- **#3 (HIGH) reconnect silently drops the fresh shell's output.** `MuxClientTransport.returningClient`
  ...
  "duplicate". FIX: `SlopDeskClient.connect` now resets `highestSeqFed`/`highestContiguousSeq`/`ackPending`
...
Code/tmux), grounded in the SwiftUI code + live screenshots (`/tmp/slopdesk-hw/ux-*.png`): **23 confirmed (2
...
- **Every pane titled "Terminal" (navigation).** FIX: `PaneChromeView` now prefers the LIVE OSC 0/2 title
  ...
  header showed the shell's cwd-based title ("/V/L/W/o/slopdesk"), not "Terminal".
...
- **Two HIGH `ConnectionRegistry` reconnect-storm races** (race; user-visible). On a shared-link drop with
  ...
  SECOND one and orphan it. (`Sources/SlopDeskTransport/Mux/ConnectionRegistry.swift`.) Test:
...
- SlopDeskClient: `SlopDeskClient.swift` (round-2 #3 reconnect dedup reset + doc), `ReconnectManager.swift`
  (round-2 #16 doc).
- SlopDeskTransport: `Mux/ConnectionRegistry.swift` (round-4 identity-gated eviction/release races).
- SlopDeskHost: `PTYProcess.swift` (round-4 reaper exit-0 sentinel).
- SlopDeskClientUI: `Terminal/TerminalScreenView.swift`, `Terminal/TerminalViewModel.swift` (round-2 #9
...
- SlopDeskHost: `HostServer.swift`, **new** `TerminfoResolver.swift`, **new**
  `Tests/SlopDeskHostTests/TerminfoResolverTests.swift`.
- ThirdParty/ghostty: `build-libghostty.sh`, `integration/GhosttySurface/GhosttySurface.swift` (round-2 #1/#2
  ...
  #4 iOS pan cursor-pos), **new** `patches/0002-slopdesk-fix-sizereport-recursive-lock.patch`.
- Tests: `SlopDeskClientTests/SlopDeskClientDedupTests.swift` (round-2 #3 reconnect-reset regression),
  `SlopDeskClientUITests/TerminalViewModelTests.swift` (round-2 #9 fresh-session wipe + round-3 exit/disconnect
  clears-running), `SlopDeskTransportTests/ConnectionRegistryTests.swift` (round-4 reconnect-storm).
- `README.md`.
...
slopdesk-hostd/slopdesk-client build clean; iOS-triple build clean. HW evidence: `/tmp/slopdesk-hw/cb-*.png`,
`recon-*.png`.
...
1. **HIGH zombie-transport reconnect race** (`SlopDeskClient.connect`) — actor reentrant across `await
   ...
   throws — a throw makes `ReconnectManager` retry & fight the winner). Test: `SlopDeskClientReconnectRaceTests`
   (3 tests × 200-loop).
...
R5 verification totals (direct binary): `SlopDeskClientReconnectRaceTests` 3/0, `FrameDecoderCursorTests` +
...
R5 files changed: `Sources/SlopDeskClient/SlopDeskClient.swift`, `Sources/SlopDeskProtocol/{FrameDecoder,
...
- **(#1, HIGH) `ConnectionViewModel.connect()`/`resume()` had no supersede guard** — the VM analogue of
  R5 rank-1. Since `SlopDeskClient.connect` now RETURNS (not throws) on supersede, the VM's success branch
...
`Sources/SlopDeskInspector/{InspectorReplayLog,NWByteChannel}.swift`. New test:
...
`Sources/SlopDeskInspector/InspectorReplayLog.swift`, `Sources/SlopDeskTransport/Mux/MuxNWConnection.swift`,
`Sources/SlopDeskProtocol/Mux/ChannelTable.swift`, `Sources/SlopDeskClientUI/SlopDeskClientApp.swift`,
`Sources/SlopDeskVideoProtocol/FrameReassembler.swift`, `Sources/SlopDeskVideoClient/SlopDeskVideoClientSession.swift`.
...
- **(#3, HIGH) per-session ZDOTDIR shim dir never deleted** — one `slopdesk-zdotdir-*` dir + 4 files leaked
...
UI 311/0; iOS build SUCCEEDED. R8 files changed: `Sources/SlopDeskTransport/Mux/{ConnectionRegistry,
MuxNWConnection}.swift`, `Sources/SlopDeskHost/{HostEnvironment,ClaudeCodeProfile,MuxChannelSession,HostServer}.swift`,
...
Also fixed (R9 #4 completion): **`SlopDeskClaudeCode/TerminalModeTracker`** (the 3rd parser, input-box mode
...
real-binary E2E 1/0; iOS build SUCCEEDED. R9 files changed: `Sources/SlopDeskHost/{HostServer,
HostTitleBellSniffer,HostCommandStatusSniffer}.swift`, `Sources/SlopDeskClaudeCode/TerminalModeTracker.swift`,
...
`pendingResultCap=4_096` eviction, and `clearOpenCard` on completion (45 SlopDeskInspectorTests green). R10's
...
build SUCCEEDED. R10 files changed: `Sources/SlopDeskTransport/HostTransport.swift`,
...
- **(LOW dead-code) removed `SlopDeskVideoHostSession.handleReap()`.** Documented as wired via
  ...
  (verified); removed rather than left as a confusing public no-op. (160 SlopDeskVideoHostTests still green.)
...
SUCCEEDED. R11 files changed: `Sources/SlopDeskInspector/InspectorViewModel.swift`,
`Sources/SlopDeskTransport/Mux/{NWMuxByteLink,MuxNWConnection}.swift`,
`Sources/SlopDeskClientUI/Connection/ConnectionViewModel.swift`,
...
- **(highest-value win) Humanized connect-failure errors.** `SlopDeskTransportError` had no `LocalizedError`,
  ...
  failed — is this an slopdesk host?", …) + swapped the two `String(describing: error)` sites in
  `ConnectionViewModel` (connect + resume catch) for `error.localizedDescription`. Test:
  `SlopDeskTransportErrorTests` (clean line + no detail-payload leak). Lowest blast radius, highest visibility.
...
SUCCEEDED; HEAD still `b49ec37`. Files changed: `SlopDeskTransportError.swift`, `ConnectionViewModel.swift`,
...
- **(LEAD, HIGH) First-run onboarding caption.** The connect form prefills a default host, misleading a
  ...
  slopdesk-hostd." under the form headline (`PaneLeafView.connectForm`). Biggest comprehension gain, zero risk.
...
`Sources/SlopDeskClientUI/iOS/KeyEncoding.swift` (mirroring the `FloatingCursorMapping` / `KeyboardAccessoryDecision`
...
- **(MED #2) Reconnect ack-ticker leak** — `SlopDeskClient.connect()` re-checked liveness only BEFORE four
...
1. **(HIGH) reconnect supervisor never checked `closed`** — `ReconnectManager.start()` + `reconnectLoop()`
   ...
   (reachable in the headless CLI, which never cancels the supervisor). Fix = new `SlopDeskClient.isClosed` +
...
2. **(MED) HOSTVIEW-1 — host port field accepts negative / out-of-range**, silently coerced
   ...
   Fix = pure `SlopDeskTransport.PortValidation` (`isValid`/`port`/`clamped`) + disable Start when invalid + an
...
**R16-deferred completion (shipped):** the slopdesk-hostd inspector-bind-failure `exit(1)` now routes through the
...
`SlopDeskTransportError.waitingErrnoIsFatalBindConflict(_:)` (codebase idiom: pure decision + thin NW glue, since
