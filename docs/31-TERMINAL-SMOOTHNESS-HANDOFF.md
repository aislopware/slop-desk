...
Made the TERMINAL path (host PTY → TCP mux → libghostty) as smooth as the architecture allows. 9 commits on `main` (`1021def..58fa8ed`), suite 1726/0, macOS + iOS build, `check-macos.sh --connect` PASS, cua loopback rig verified (flood + instant Ctrl-C + live RTT badge).
...
- **Ctrl-C is instant**: credit-at-consumption bounds client-side un-rendered bytes to ~2 windows (~128 KiB). Prompt is back in the same screenshot as the press.
- **Typing under flood**: OUT drain runs off-main (keystroke sends no longer queue behind ingest/render main-actor work).
...
27-agent adversarial review caught a HIGH the 1726-test suite couldn't: frame caps bounded **payload** while the sender debits **wire bytes** (payload + 13-byte `.output` header) → max frame 32 781 > window/2 32 768 → a credit park whose partial-frame prefix landed in that 13-byte dead zone could never re-grant (receiver credits only COMPLETE decoded frames). Now `MuxFlowControl.maxOutputFramePayloadBytes = min(mergeCap, window/2 − 16)` + head-chunk splitting. Rule: **with credit-at-consumption, always reason in wire bytes, and only complete-frame bytes are creditable.**
...
- **Type-under-flood**: keys land instantly during a 41 s `yes` flood; RTT badge <1 ms; Ctrl-C → `^C` + fresh prompt in the very next frame.
- **TUI (Neovim)**: dashboard (25 plugins), insert-mode typing, statusline, alt-screen enter/exit render clean; scrollback restored exactly on exit.
...
1. **Deploy both ends + WAN feel-test** — PARKED: MacBook Pro stays offline; loopback portion done. When it returns, deploy host + client together (see deploy section), feel-test WAN typing, consider defaulting `SLOPDESK_GLITCH_CARET=1` on.
2. **iOS real-device check** of the gated tick (type/echo, pan-scrollback, cursor blink) — simulator keeps free-run deliberately; blink rides the renderer's own present path in theory, unverified on device.
