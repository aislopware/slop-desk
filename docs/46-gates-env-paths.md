...
| `scripts/herdr-sync.sh` | `SlopDeskAgentDetect` engine/manifest changes, or herdr upstream sync — builds the real herdr binary, diffs both engines on ~10k screens (`scripts/herdr-differential.py`); pin = `scripts/herdr.pin`. ⚠️ Parity is NOT the goal for every RULE — `DIVERGED_RULES` in that script names the ones we deliberately made better (today claude's `live_prompt_box` + `legacy_no_prompt_blocker`); every other rule of those agents stays under test, and a mismatch is excused only when a diverged rule explains it. Adding one needs a written reason + a test (`docs/50` §9) |
