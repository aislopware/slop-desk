...
| `scripts/check-web.sh` | ONLY gate on the Web panel's BROWSER — needs a Chrome-family browser (`brew install --cask google-chrome`); sets `SLOPDESK_WEB_HW=1`, without which every case in `WebBrowserHardwareTests` is a no-op and a clean checkout stays green. Everything PURE about the panel (launch flags, announce parse, locator, profile resolution, manager lifecycle, verb-23 routing, target selection, address normalisation, `/json` decode) is already in `make test-touched`; this proves the flags still produce a debugging port and that the relay carries the browser's bytes untouched. The browser it starts is headless on a throwaway profile and terminated at the end — a Chrome the user has open is never touched. Dialect + traps: `docs/49-web-panel.md` |
...
**Nor is the Web panel:** a THIRD foreign wire — Chrome's DevTools Protocol plus its `/json/*` endpoints, spoken to a browser the user installed. TWO relays, neither optional: hostd's (`WebDebugRelay`) because Chrome binds its debugging port to 127.0.0.1 and `--remote-debugging-address=0.0.0.0` is accepted and IGNORED, and the client's own loopback proxy because the DevTools frontend opens its websocket back to `ws://127.0.0.1:*` and admits nothing else. Only discovery (metadata verb 23) rides a SlopDesk wire. Note the authority that port carries: a browser runs whatever it is pointed at, so it is host-user code execution on the mesh, like verb 22's `adb` bridge. Dialect, measurements and traps: **`docs/49-web-panel.md`** — read it before touching anything under `Sources/SlopDeskClientUI/Web` or `Sources/SlopDeskHost/Web`. Gates: `make test-touched` for everything pure, `scripts/check-web.sh` for the browser.
...
| `SLOPDESK_WEB_BROWSER_BIN` / `_WEB_PROFILE_DIR` | Web panel browser override + `--user-data-dir` location. Set-but-not-executable resolves to nil rather than falling through to the search. The profile is NEVER the OS-default one: Chrome 136+ refuses remote debugging there, and a Chrome the user is running holds its lock. Detail: `docs/49-web-panel.md` |
| `SLOPDESK_WEB_HW` | `=1` arms `WebBrowserHardwareTests` (needs a Chrome-family browser). Set by `scripts/check-web.sh`; off ⇒ every case is a no-op |
