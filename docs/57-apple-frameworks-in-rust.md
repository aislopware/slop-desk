...
| `slopdesk-apple-cgvirtualdisplay` | CoreGraphics' virtual-display area — `CGVirtualDisplay` and its descriptor, settings and mode classes | the Objective-C shim that reached them, with its `weak_import` linkage attribute and its `NSClassFromString` gate | **landed** — the four classes live in the PUBLIC framework and only their headers are private, so `AnyClass::get` plus `msg_send!` is the whole mechanism: no `dlsym`, no hand-declared `extern`, no `transmute`. `AnyClass::get` answers `Option`, which collapses the shim's two devices into one lookup — a macOS that renames one of the four answers "no display" instead of crashing the daemon |
| `slopdesk-apple-text` | Core Text | the family-name answer `slopdesk font import` needs, and the font engine `slopdesk-termrender` says it does not have | **landed** — `FontStack` resolves a family at a size into faces and metrics, `Shaper` turns a run of cells into positioned glyph ids, `Rasterizer` turns a glyph id into a bitmap (`docs/68` §5.1 puts the renderer in Rust and names this crate as the one that extends). The gate is `any(macos, ios)` rather than `macos`: Core Text is on both slices and the terminal draws the same way on each |
