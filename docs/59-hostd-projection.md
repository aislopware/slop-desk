...
**Step 9 — the test seams STAY, and this is the argument.** *(0 Swift lines)* This step was written
as "~400 lines of `…ForTesting` fall out once that state is reachable in Rust". Counted seam by seam
against the tests that CALL them, that is wrong, and the honest version is worth more than the line
count was.

`MuxChannelSession.swift` carries 58 seam mentions and `HostServer.swift` 20. Sorted by what their
callers assert, they are three kinds, and only one kind was ever a mirror:

- **Drivers** — the large majority (`ingestPTYChunkForTesting`, `enqueueChunkForTesting`,
  `enterBareSubscriberForTesting`, `foldTitleSampleForTesting`, `serveMetadataForTesting`, …). The
  production entry point is `private` and wants a live PTY, a mux link and a peer; `@testable import`
  reaches `internal`, not `private`. The seam is the headless RIG that lets a test reach the ladder
  at all, and Rust owning the decision does not give a Swift test a way to drive the Swift ladder.
  Deleting these deletes the tests.
- **Ordering probes** — `hasExitTaskForTesting`, `awaitEOFForTesting`, `awaitExitSentForTesting` and
  the `signal…`/`is…` pairs beside them. Step 5b moved the LATCHES to `lifecycle.rs` and unit-tests
  them there, but what these pin is hostd's exit ORDER: the drain flushes the tail before `.exit`
  goes out, and `onExit` cannot cancel the drain before it does. The ladder enforcing that order is
  still Swift `Task` work (§6), so the Rust test is the other half of the claim, not a replacement.
- **Count readouts** — `subscriberCountForTesting`, `teardownCompletionsForTesting`, and
  `HostServer`'s registry counts. The closest to redundant, and still not: `registry.rs` proves the
  table's arithmetic; these prove hostd's paths CALL the right table operation on the right edge,
  which is integration coverage no Rust unit test reaches.

A seam is deletable only when the tests calling it are fully redundant with a Rust test. Enumerated
...
Each rule carries a BREAK TEST — a `#[test]` that writes the drifted file into a fixture tree and
asserts the rule fires on it, beside a green-tree case. That is the crate's actual convention (a
`/// BREAK-TESTED <date>:` prose line appears on seven rules out of 314 and is not one), and it is
the stronger half: the prose records that somebody once broke it, the test re-proves it every run.
