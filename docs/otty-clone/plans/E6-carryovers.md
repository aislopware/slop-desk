...
⏸️ The otty sidebar sort/group hamburger (`OttySortMenuButton`, `Chrome/OttyTabRow.swift`) documents a THIRD menu section — **DIVIDER: "Insert Divider" + "Remove All Dividers"** (`user-interface__window-tab-split.md:191-194`, `group-tabs.png`) — that lets the user drop a visual section separator between sidebar tab rows. The clone currently ships only the **GROUP** and **ORDER** sections.
