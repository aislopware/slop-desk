...
On this hardware, **the HEVC encode is the entire processing cost, and it scales ~linearly with the
pixel count.** Decode (~1.3 ms), packetize + Reed–Solomon FEC (~0.05 ms) and the pacer are noise.
So "remote window feels bad" is governed by **capture resolution (= encode time)** and **the
rate-control budget (= motion blur)** — not by the wire path.
...
**The 60 fps encode wall on M1 Max HEVC low-latency is ≈ 2400×1350 (capture scale 1.25).** Anything
larger overruns the 16.7 ms frame budget → VideoToolbox drops frames → the "giật" (stutter). The
default virtual display captures at **2× → 3840×2160 → 23 ms/frame → cannot do 60 fps and is
marginal even at 30 fps** (a 24 ms synchronous encode on a 60 Hz capture queue leaves ~8 ms of slack,
so a heavier frame stalls the next capture = periodic hitch).
...
coarser** than a 150 Mbps near-lossless reference of the same content. That is the "nhoè" during
...
on the client decode or the wire path for latency.

## What this means for the two symptoms

- **giật (stutter):** you overran the encode budget. Either (a) you launched the plain default
  (virtual display 2× → 4K → 23 ms → can't hold 30/60 fps), or (b) `SCROLL_FPS=40` is decimating a
  60 fps stream that the encoder can actually sustain at scale ≤1.25 (choppier for no reason).
...
| `AQP_MAX` | 44 | **36** | less motion blur; the link is fat, encode has headroom |
| `CONST_QP` / `AQP_SHARP` | 24 | 24 | keep the crisp static floor |

**If you are running the plain `videohostd` (no profile) — which is what is running now — you get
none of the above:** virtual display 2× (4K encode = stutter), no const-QP (VBR clawback = blur),
30 fps. Launch a profile instead. The plainest fix for a naive launch is to lower the default
capture scale off 2× (a code change worth landing + HW-verifying: bias
`resolveCaptureScaleOverride`'s default toward the 60 fps-sustainable ~1.25× instead of the VD's full
2×).
