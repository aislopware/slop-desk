...
**Scope.** Map the reference subcommands onto `slopdesk-ctl`/store ops: `view/edit`, `config get/show/path/edit/validate/schema`, `font/keybind list`, `tab/pane/window` (`send-keys` already exists), `tab badge --kind`, `pane capture`, `watch <cmd>` (OSC 9;4 wrapper), `watch:claude <id>` (exit 0/4/9), `jump/learn/ignore` (frecency), `version`, `completions`, `sidecars`, `help`. Install-CLI flow + omit-prefix (O3). No first-launch flow, and the CLI only READS the file: settings are a TOML document the app never writes, every choice is best-by-default, and the hook is installed unconditionally (2026-08-24).
