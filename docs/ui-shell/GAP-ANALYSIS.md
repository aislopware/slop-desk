...
| A20 | Tab badges (spinner/check/dot/error/hand/coffee/shield/SSH) | **partial** — `RailRow.status: ClaudeStatus` computed but **never rendered**; agent states only | render badge dot on `SlateTabRow`; add command/caffeinate/sudo badge sources | 2 |
...
| M11 | Line height (Default/Compact/Loose/Custom) | **partial** — density setting exists; not the documented line-height modes | add line-height modes via config builder | 4 |
...
| M13 | Font install (local font directory, CoreText register) | **na-remote** — render is host-side libghostty; fonts must be on host | client text-field only; host-side install note | 5 |
