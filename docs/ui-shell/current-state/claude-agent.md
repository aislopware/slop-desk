...
| **Status badge in inspector** | done | `InspectorColumn` reads `activeLive?.claudeStatus`, renders SF Symbol + tint via `StatusPresentation.agentSymbol/agentTint` at `Sources/SlopDeskPhoneUI/Columns/InspectorColumn.swift:150` |
| **Status indicator in iOS toolbar** | done | `WorkspaceRootView.iosToolbar` renders `StatusPresentation.agentSymbol(activeAgentStatus)` at `Sources/SlopDeskPhoneUI/WorkspaceRootView.swift:70` |
| **Status dot in sidebar tab rows** | partial | `RailRow.status: ClaudeStatus` populated by `RailRowsBuilder.rows(for:)` at `Sources/SlopDeskPhoneUI/Rail/RailRowsBuilder.swift:38`; but `SlateTabRow` (macOS row view at `Sources/SlopDeskPhoneUI/Chrome/SlateTabRow.swift`) does **not render the status** — accepts only `title`/`active`/`onSelect`/`onClose`. Dot referenced in doc comments but the view prop was never added. |
| **Attention edge (needsPermission / done notification)** | done | `applyAttentionEdge` at `Sources/SlopDeskWorkspaceCore/Workspace/Store/WorkspaceStore+Attention.swift:77`; `onAgentAttention` wired in `SlopDeskClientApp` at `Sources/SlopDeskPhoneUI/SlopDeskClientApp.swift:155`; delivers macOS `UNUserNotificationCenter` local notification |
...
| **Composer (multi-line input bar, ⌘⇧E)** | partial | `InputBarModel` at `Sources/SlopDeskWorkspaceCore/Input/InputBarModel.swift`; `InputBoxModel` at `Sources/SlopDeskClaudeCode/InputBoxModel.swift` (mode detection + dedup ring); `InputBar` view at `Sources/SlopDeskPhoneUI/Pane/InputBar.swift`; **not mounted** in `TerminalLeafView` (TODO(L5) at `:42`); rich/multi-line toggle exists; no draft persistence, no pin mode, no float-panel mode, no ⌘⇧E keybinding registered |
| **AgentInputFooter bottom bar** | partial | `AgentInputFooterCoordinator` at `Sources/SlopDeskPhoneUI/Footer/AgentInputFooterCoordinator.swift`; `AgentInputFooterAction` enum at `Sources/SlopDeskPhoneUI/Footer/AgentInputFooterAction.swift` (8 action cases); coordinator handles notifications chip (W4), rich-input toggle (W3), file-explorer toggle (W2), start-remote-control (W1), settings (stub), add-context (stub), file-select; **no `AgentInputFooter` SwiftUI view exists** — the rendered bar is a TODO(L5) stub in `TerminalLeafView` |
| **History viewer (JSONL transcript rendering)** | missing | Spec at `docs/ui-shell/spec/agents__history.md`; no impl code. `BlockHistoryView` at `Sources/SlopDeskPhoneUI/Inspector/BlockHistoryView.swift` is **command-block** (shell) history, not Claude session JSONL. |
...
- `Sources/SlopDeskPhoneUI/App/StatusPresentation.swift` — view-layer SF Symbol + colour mapping
- `Sources/SlopDeskPhoneUI/Rail/RailRowsBuilder.swift` — `RailRow.status` population
- `Sources/SlopDeskPhoneUI/Chrome/SlateTabRow.swift` — sidebar row view (status NOT rendered)
- `Sources/SlopDeskPhoneUI/Columns/InspectorColumn.swift` — "Agent" row in inspector
- `Sources/SlopDeskPhoneUI/WorkspaceRootView.swift` — iOS toolbar agent indicator
- `Sources/SlopDeskPhoneUI/Footer/AgentInputFooterCoordinator.swift` — footer coordinator
- `Sources/SlopDeskPhoneUI/Footer/AgentInputFooterAction.swift` — footer action enum
- `Sources/SlopDeskPhoneUI/Pane/TerminalLeafView.swift` — TODO(L5): footer not mounted
...
- `Sources/SlopDeskPhoneUI/Inspector/BlockHistoryView.swift` — command-block history (NOT Claude session history)
- `Sources/SlopDeskPhoneUI/SlopDeskClientApp.swift:155` — `onAgentAttention` → `UNUserNotificationCenter`
