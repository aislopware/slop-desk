...
| **Attention edge (needsPermission / done notification)** | done | the ladder is `slopdesk-workspace::pane_facts::commit` (`PaneFacts.commit`, applied verbatim by `setAgentStatus` at `WorkspaceStore+Attention.swift:44`), `scheduleAgentAttention` `:395`, `fireAgentAttention` `:421`; the store slot is `WorkspaceStore.onAgentAttention` at `WorkspaceStore.swift:2132`, bound **once, cross-platform**, at `Sources/SlopDeskClientCore/App/ClientComposition.swift:364` (NOT in a per-platform app file). The Code-Agent notification sound rides the same edge (`Sources/SlopDeskWorkspaceCore/Connection/NotificationSound.swift:42`) |
