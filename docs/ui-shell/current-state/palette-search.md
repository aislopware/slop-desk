...
- **The palette chord is ⌘⇧P, not ⌘K.** `docs/DECISIONS.md:236` re-bound `view.palette` to the reference
  default and freed ⌘K. ⌘K is now a PICKER-LOCAL chord *inside* Open-Quickly (the per-row Actions
  popover) and is never registered in `WorkspaceBindingRegistry` (`docs/DECISIONS.md:341`).
- **Open-Quickly shipped, and Jump-To folded into it.** ⌘⇧O opens the picker at the merged `.all` pill,
  ⌘J at `.current` (`docs/DECISIONS.md:338,341`). `JumpToView.swift` was deleted in that fold;
...
| **Command palette — keyboard chord** | done | `WorkspaceBindingRegistry.swift:645–649` — `view.palette`, chord **⌘⇧P**; routed at `WorkspaceBindingRouting.swift:146` (`case .commandPalette: toggles.palette?()`). The survey's `⌘K` is stale by one day (`docs/DECISIONS.md:236`) |
...
| **Palette row platform filter** | done (new since the survey) | `Palette/PaletteRowPlatform.swift` over `rust/slopdesk-workspace/src/palette_rows.rs` (`ROWS`, 33 entries). Five are `Platform::Mac` — `action.detachPane`, `action.reattachAllPanes`, `action.secureKeyboardEntry`, `action.closeWindow`, `action.pinWindow` (:73,74,85,99,106) — the same five features `binding_rows.rs` gates. `registryRows` needs no gate of its own: it reads the already-filtered `bindings` |
| **Find-in-terminal (⌘F) — pure engine** | done | `SlopDeskWorkspaceCore/Terminal/TerminalSearchController.swift` — literal + regex, case toggle, overlapping matches, next/prev/wrap, "N of M"; `TerminalSearchControllerTests` |
...
- `Sources/SlopDeskWorkspaceCore/Terminal/TerminalSearchController.swift` — the pure ⌘F engine
...
- `Sources/slopdesk-fuzzybench` — the ranking-parity bench against real `fzf --filter`
...
capability iOS does not have — the same five `binding_rows.rs` drops, by design rather than by omission.
