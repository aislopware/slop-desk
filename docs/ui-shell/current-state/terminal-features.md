...
a door: they are decided on THIS side, read live from `SettingsKey` at the moment of the gesture. What
is left genuinely unactuated is `shift-arrow-select`, `mouse-shift-capture`, `cursor-click-to-move`
and the TERMINAL half of `line-height` (its code-panel half is live through `CodeFontSync`).
...
in the matrix rows. Nine are listed below, renumbered: the seven that survived that pass, plus §6 and
§9, which this pass FOUND rather than inherited.
