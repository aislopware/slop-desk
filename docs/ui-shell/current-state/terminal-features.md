...
| **Autocomplete** (shell completion overlay) | **built** — this row was stale | Ranked completion is `rust/slopdesk-terminal/src/prompt/complete.rs` (subcommands, flags, paths, directories, variables, history), driven by `CommandEditor::complete` and stepped by `select_next_candidate`/`select_previous_candidate`, accepted by `accept_completion`. It crosses on the prompt handle (`slopdesk_prompt_complete`, `_candidates`, `_candidate_arena`, `_candidate_positions`, `_accept_completion`, `_dismiss_completion`) and is wired on BOTH platforms — `MacTerminalRendererView.swift:586-619`, `TerminalLeafView.swift:1483-1566`. The panel is drawn by `TerminalPromptBand.drawAccessory` (six rows, detail column, matched-scalar underline, selection in `accent`), the inline GHOST by `TerminalPromptBand.ghost`. ⚠️ That ghost has a SECOND source since 2026-09-01: with no list open it prints the **history autosuggestion** instead — `zsh-autosuggestions`' default `history` strategy, in-house. `CommandHistory::suggestion` is a plain function of (store, line) and holds no state, which is why it can be shown unasked; `CommandEditor::suggestion` suppresses it on five conditions (⌃R open, candidates open, a selection, caret not at the end, a newline in the line) but NOT during a history walk. The accept follows `fish`'s rule — it belongs to the input FUNCTION, not to a key — so `prompt::keys::over_suggestion` translates a `Motion`: `→`/`End`/`⌃E`/`⌃F`/`⌘→` take the whole suggestion, ⌥→ takes a word, and a `DefaultKeyBinding.dict` entry the user invented inherits it. The Mac asks in motions (`slopdesk_prompt_suggestion_accept_for_motion`, because AppKit hands it a selector and never a key), the phone in keys (`slopdesk_prompt_key_action`'s new `has_suggestion` flag); the rule itself is Rust's on both. ⚠️ The accept goes through `replace_range`, never `insert_text` — typed insertions coalesce, so an insert-spelled accept merged into the burst that summoned it and one ⌘Z emptied the line. ⚠️ **⌃R IS THE SAME PANEL, since 2026-09-01** — `CommandEditor`'s reverse search fills the SAME candidate list, so it crosses through the same three doors and is drawn by the same `drawCandidates`. `complete::search_history` ranks every history entry with the same `slopdesk_fuzzy::score` (score desc, then RECENCY, which is where it diverges from `complete`'s alphabetical tie-break), so `car t` finds `cargo test` in ⌃R exactly as it does in the list. An empty query opens the panel FULL — the recent-commands list `fzf` and `atuin` open with. `slopdesk_prompt_search_again`/`_back` move the selection (wrapping — the end is visible now), Enter puts the row on the line WITHOUT running it (`fish`'s pager, not `atuin`'s Enter), and `CommandEditor::complete` refuses while a session is up so a redraw cannot stomp the rows. Enter on a query that matched NOTHING still closes the session and reports that it wrote nothing — a key that visibly does nothing reads as a wedged prompt. Deleted by it: `ReverseSearch`, `SearchHit`, `find_smart_case` and its case-fold arithmetic, `slopdesk_prompt_search_hit`, `search_has_hit`. Added by it: `slopdesk_prompt_search_back`, and `search_matches` on the state record — the one search number the shared rows cannot carry, since they carry what FITS and two caps (`complete::LIMIT`, then the band's six) stand between the history and the screen, so the query row's `6 of N` would otherwise print the nearer cap as the total. ⚠️ It also WIRED the matched-scalar underline, which had crossed since the candidate records landed and was drawn by nothing — survivable on a prefix list, not on a fuzzy one. Pinned by `TerminalSearchPanelPixelsOnIOSTests` (the band grows one row per match; the matched run is underlined) and by `testASearchCountsItsQueryRowAndItsPanel`. ⚠️ **Tab now also asks the user's OWN zsh**, since 2026-09-01: verb 23 was built end to end (host performer, wire codec, `ShellProvider`) but no keystroke reached it — `MetadataClient.shellComplete` had zero callers and `CommandPrompt.setShellCandidates` none outside its own file, so the one thing this terminal has over Warp fired never. `TerminalViewModel.completeCommandPrompt(forward:didChange:)` is now the ONE implementation of the Tab rule for both renderers; it asks the local sources and the shell in parallel, SUPPRESSES the accept-on-exactly-one while a shell request is out (a lone local `main.rs` must not beat zsh's 27 refs to `git checkout ma`), withholds that accept from a line the user kept typing through the round trip, latches `.noShell` for the connection and never latches `.notReady`, and drops a stale answer by generation. Wired by `TerminalPaneWiring.wireShellCompletion(live:)`; pinned by `TerminalShellCompletionTests` (15 — the later five pin `CommandPrompt.completionEpoch`, which drops an answer whose question the user ENDED inside the 20–92 ms window: Enter that ran the line, ⌃C, ⎋, the `acceptCompletion()` both views reach on Enter instead of submit, and ⌃R, whose rows ARE candidates so a merge dropped shell candidates into a reverse search). ⚠️ **⌃R reads `fzf`'s EXTENDED-SEARCH syntax**, since 2026-09-01 — `git !push ^g`, with `|` for OR, `'` for a substring and `^`/`$` for the anchors. `rust/slopdesk-fuzzy/src/pattern.rs` ports `pattern.go`'s `parseTerms`/`extendedMatch` and `algo.go`'s `exactMatchNaive`, `PrefixMatch`, `SuffixMatch`, `EqualMatch` and `calculateScore`, attributed like the `FuzzyMatchV2` port beside it. `search_history` parses one `Pattern` per query and `complete` keeps the PLAIN scorer, because a completion's query is shell text where `$HOME`, `!!` and `|` already mean something — a test each side says so. The fzf BINARY stays rejected: wrong side of the wire, a full-screen TUI, a process per keystroke. What is genuinely not built is the **Fig-style bundled spec DB** — the providers are the ones the host can answer from itself. |
