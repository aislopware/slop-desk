...
- `slopdesk_term_surface_set_scrollback(handle, lines)` — the retention depth, in ROWS. The old text
  ...
  `scrollback-limit = 10000` now means ten thousand rows.
...
| **Kitty image protocol (inline images)** | **done** (2026-09-01) | Engine side `rust/slopdesk-vterm/src/graphics.rs` (images, placements, the PNG hook the bindings publish); placement and pixel cache `rust/slopdesk-termrender/src/image.rs`; textures and shaders `rust/slopdesk-apple-metal/src/images.rs` + `image_vertex`/`image_fragment`; wired at `Surface::place_images` in `rust/slopdesk-ffi/src/terminal_surface.rs`. Three z bands interleaved into the existing pass, clipped to the BLOCK. Row `terminal.images` (default ON) gates drawing only. ⚠️ The `t=f`/`t=t`/`t=s` file and shared-memory transmission mediums are CLOSED and no setting reopens them — see `docs/68` §5.7 and `docs/DECISIONS.md`. The one shape declined is the VIRTUAL (`U=1`, unicode-placeholder) placement, which reports no viewport position; it is skipped rather than guessed. |
| **iTerm2 inline images** | **gap** — decoder only | No `OSC 1337 File=` handling anywhere. The vendored `libghostty-vt` bindings implement only two other OSC 1337 subcommands (`CurrentDir`, treated like OSC 7; `Copy`, treated like OSC 52) — the image-carrying subcommand is absent. The RENDER half is no longer missing: an image that reached `ImageStore` would draw through the kitty pass unchanged, so what is left is a transmission format the engine does not parse. |
| **Sixel graphics** | **gap** — decoder only | Only a Device-Attributes capability-flag constant exists in the vendored crate (self-identification only) — no sixel decoding/parsing code exists anywhere. Like iTerm2 above, the render half now exists; a sixel decoder would land in the same store and draw through the same pass. |
...
- `rust/slopdesk-ffi/src/find_matches.rs` — the ⇧⌘F cross-pane scan door, over `slopdesk-rowscan`. The ⌘F doors are `slopdesk_term_surface_find` / `_find_position` in `terminal_surface.rs`.
