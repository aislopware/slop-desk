...
| **Ownership: the host owns the document** | **done — the biggest change since the survey** | `WorkspaceStore.tree` is a projection over the replica, memoized against `workspaceMirrorRevision`, with this device's own focus and the live divider-drag preview layered on top (`WorkspaceStore.swift:40-70`). The mirror reads in precedence `pending` → `entries` → `fastPath`; `entries` is provably `apply(diffs, base)` and a fast-path write is ERASED the moment a snapshot supplies the same key (`rust/slopdesk-wire/src/document/mirror.rs`, behind `WorkspaceMirrorBox`). Optimistic patches run the SAME `WorkspaceIntentApplier` the host will run. |
...
- `Sync/{WorkspaceMirrorBox,WorkspaceChannelClient,LoopbackWorkspaceDocument}.swift` — the replica's face and its transport
...
supplies the same key (`rust/slopdesk-wire/src/document/mirror.rs`). Letting a fast-path value survive a document
