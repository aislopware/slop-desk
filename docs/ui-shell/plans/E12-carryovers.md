...
exclusions, since E12's composer/prompt-queue falls under the "agents" surface where the
...
- **Agents = Claude Code only initially (the one that bites E12).** The composer and prompt queue
  ...
  per-agent backend selection, or codex/opencode-specific composer affordance even if the spec's
  `agents__composer.md` / `agents__prompt-queue.md` docs show one. Claude Code is the only
