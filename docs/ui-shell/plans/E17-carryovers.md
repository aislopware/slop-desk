...
(Theming + fonts) closed all of its review findings (base `e87ae9d` → polish `3361360d` → finish
`80c2a1a`); its only open item is a Phase-3 HW GUI-fidelity check, which does not constrain E17.
This file carries forward the **binding scope reductions** plus the **platform/wire/balance traps**
that specifically bite read-only-gate + secure-input work.
...
- **Vertical-tabs-only.** slopdesk is vertical-tabs-only (committed product decision in
  ...
  **NOT** introduce any top/horizontal tab-bar region or a pill whose only purpose is decorating one.
  Place pills in the pane title/overlay region and the sidebar rail row, never a horizontal strip.
- **No SSH-host filter.** Standing exclusion (primary impact E11). Not relevant to E17; noted so no
  settings cross-reference reintroduces it.
- **Agents = Claude Code only initially.** Standing exclusion (primary impact E13). Not relevant to
  E17's terminal gating; noted so nothing pulls in multi-agent UI.

## DO-NOT-CONFLATE (important — read-only mode IS the feature)

- The standing **"never add an approval gate"** directive is about *agent supervision* (E13) — never
  force the user to approve each agent action. E17's **read-only mode is a different, legitimate,
  explicitly-in-scope feature**: a *user-toggled* per-pane input gate that the user turns on/off at
  will. It is NOT an agent approval gate. Build it fully (gate keys/paste/click-to-move/mouse-report/
  drop, beep-on-blocked, `🔒 READ ONLY ×` pill, Shell→Read-Only menu/palette terms per ES-E17-1).
  Do not water it down out of misplaced deference to the agent-gate rule.
...
- **`EnableSecureEventInput` is PROCESS-GLOBAL and MUST be balanced.** Every `Enable` needs exactly
  ...
  until the process exits (a real, user-hostile bug). Reference-count or strictly pair it, and ensure
  it is disabled on pane close / app background / window resign / no-echo-cleared. Add a test (or at
  minimum an asserted invariant) that the enable/disable calls balance. Do not enable it
  unconditionally — only while the host signals canonical no-echo AND the Auto/Indicator setting
  allows it.
- **"Host signals canonical no-echo" may be a WIRE concern — treat it carefully.** The PTY echo/no-echo
  (termios `ECHO`) state lives on the HOST. If E17 needs the host to *tell* the client "child has
  ...
  then: it is a WIRE EXTENSION → the host accepts only version `1` (no negotiation), so host+client
  **must redeploy together** (no-backcompat), update `docs/20-wire-protocol.md`, manual big-endian
  ...
  keys survive + run `bash scripts/golden-check.sh`. Default to the no-wire client-side derivation;
