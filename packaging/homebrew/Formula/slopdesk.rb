...
class Slopdesk < Formula
  ...
  # agents — this one and a checkout's `com.slopdesk.superd` from `scripts/install-superd.sh` —
  ...
  def caveats
    <<~EOS
      ...
      `scripts/install-superd.sh` agent and this one coexist, and whichever booted first keeps
      the panes.
