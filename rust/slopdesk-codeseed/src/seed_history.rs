//! Every settings seed this repo EVER shipped before the current one.
//!
//! A settings file byte-identical to a former seed is still PRISTINE — the user never touched it
//! (the workbench rewrites the file on any settings edit) — so it may be upgraded to the current
//! seed. Anything else is the user's and stays untouchable.
//!
//! A separate module for the same reason it was a separate Swift file: it is retired JSON that no
//! reader of the seeding logic ever needs to scroll past, and it only ever grows.

/// The retired seeds, oldest first. Order is documentation only — the check is set membership.
pub const OBSOLETE_SEEDS: &[&str] = &[
    ...
    // v30 — every tab wore a chip of rising ink (3.5 / 7 / 11%) on a bed painted in the
    // ground cream, so the strip had no floor and the OPEN tab was the darkest thing in it.
    r##"{
    "chat.disableAIFeatures": true,
    "workbench.shadows": false,
    "workbench.colorTheme": "Alucard",
    ...
    // v31 — the tab strip carried a real 9% bed so the cream open tab could read as the
    // canvas climbing into it. The bed does not stop at the last tab: it ran on under the
    // action icons and dead-ended at the sidebar, a grey rectangle across a cream panel.
    r##"{
    "chat.disableAIFeatures": true,
    "workbench.shadows": false,
    "workbench.colorTheme": "Alucard",
