...
/// to overrule it with `?? .h264`, which configured an H.264 NAL-type reading for AV1 parameter sets
/// and handed `VTDecompressionSession` a mis-typed format description — the black rectangle
    ...
    check_all(
        ...
        &[
            Claim::Lacks {
                ...
                view: View::Raw,
                message: "the Android panel defaults an unrecognised codec again — the door already \
                          refused it",
            },
            Claim::Matches {
                ...
                view: View::Raw,
                message: "AndroidStreamProtocol stopped asking which codecs decode — that set is \
                          Rust's",
            },
        ],
    )
...
/// in for it — `M_MIN` is 1 — so a reader who does not know the door exists reaches for the literal,
/// which is how it came to be spelled twice in Swift and a third time inside the crate's own tier
/// table. A host and a client that disagree about it emit and expect different parity counts per
/// group, and neither end logs anything: the client simply stops repairing.
#[must_use]
...
pub fn the_level_bytes_are_read_through_doors(tree: &Tree) -> Report {
    ...
    let mut claims = vec![Claim::Mentions {
        ...
        message: "MetadataCodec no longer calls {entry} — the level readings are \
...
/// already drifted on: `TemplatePane::keystrokes` hardcoded `None` for the cwd and so could not emit
/// a `cd` line.
#[must_use]
pub fn the_dead_rust_expansion_stays_deleted(tree: &Tree) -> Report {
    ...
    let claims: Vec<Claim> = REVIVED
        .iter()
        .map(|revived| Claim::Lacks {
            path: TEMPLATES,
            pattern: revived,
            view: View::Raw,
            message: "templates.rs grew the preset expansion back — it is \
                      LaunchPresetEngine.plan's, and a Rust copy nothing calls cannot be caught \
                      disagreeing",
        })
        .collect();
    check_all(tree, &claims)
...
/// actually paced a recovery IDR off a post-backoff ABR, serializing for hundreds of ms the one
/// frame whose delivery time IS the client's recovery time. Nothing could fail on it: the inline
/// path has no test, and the two paths are never both live.
///
    ...
    check_all(
        ...
        &[
            Claim::Matches {
                ...
                view: View::Raw,
                message: "the host session stopped asking VideoSendLane.plan — the paced-send \
                          schedule is slopdesk_send_pace_plan's",
            },
            Claim::Exactly {
                path: VH_SESSION,
                ...
                count: 1,
                view: View::Raw,
                message: "the host session computes the pacing gap in {found} places, not 1 — the \
                          two copies had already parted on keyframes",
            },
            Claim::Matches {
                path: VH_LANE,
                pattern: r"slopdesk_send_pace_plan\(",
                view: View::Raw,
                message: "the send lane stopped calling slopdesk_send_pace_plan — the chunk \
                          boundaries would be hand-rolled again",
            },
        ],
    )
}
...
/// crate's `built_in_*` tables is the arrangement `CLAUDE.md` bans by name, and the cost is specific
/// rather than stylistic: a built-in's UUID is FIXED so that re-seeding a workspace MATCHES its row
/// instead of appending a second one, so a fourth row added to one side only hands every device a
/// different set depending on which side seeded it — surfacing weeks later as a duplicated menu row
/// with nothing in any log. `compare_abi_enum` cannot see it (it pins names and numbers it was told
/// about), and the differential that used to see it is gone precisely because there is now one
/// table.
///
/// The `builtInID` helper is banned too: it existed only to spell a literal UUID for a table, so its
/// return is the shape of the mirror coming back.
#[must_use]
    ...
    check_all(
        ...
        &[
            Claim::Matches {
                ...
                view: View::Raw,
                message: "SessionTemplate.swift stopped seeding from the crate — the shipped table \
                          is templates.rs's",
            },
            Claim::Matches {
                path: MODEL_PRESET,
            ...
            Claim::NoneOf {
                ...
                pattern: r"builtInID\(",
                view: View::Raw,
                message: "a model file spells a built-in UUID again ({files}) — the rows come from \
                          the crate",
            },
            Claim::Mentions {
                path: TEMPLATES,
...
mod tests {
    ...
    fn a_raw_level_byte_read_directly_is_red() {
        ...
        fixture.write(
            ...
            "let pressure = slopdesk_metadata_memory_pressure(pressureByte)\n\
             let state = slopdesk_metadata_service_state(stateByte)\n",
        );
    ...
    fn a_revived_rust_expansion_is_red() {
        ...
        fixture.write(
            ...
            "pub fn built_in_session_templates() -> Vec<Template> { vec![] }\n\
             pub fn built_in_launch_presets() -> Vec<Preset> { vec![] }\n",
        );
        ...
        fixture.append(
            ...
            "pub struct LaunchPlan { pub panes: Vec<PaneLaunch> }\n\
             pub fn plan(preset: &Preset) -> LaunchPlan { LaunchPlan { panes: vec![] } }\n",
        );
        assert!(!super::the_dead_rust_expansion_stays_deleted(&fixture.tree()).is_clean());
    }
    ...
    fn a_second_pacing_gap_is_red() {
        ...
        fixture.write(
            ...
            "let gap = Self.adaptivePaceGapNanos(for: job, keyframe: job.keyframe)\n\
             let plan = VideoSendLane.plan(for: job)\n\
             let inlineGap = Self.adaptivePaceGapNanos(for: job, keyframe: false)\n",
        );
        assert!(!super::one_pacing_schedule_and_one_gap(&fixture.tree()).is_clean());

        // And the lane hand-rolling the chunk boundaries again.
        pacing(&fixture);
        fixture.write(super::VH_LANE, "static func plan(for job: Job) -> Plan { chunked(job) }\n");
        assert!(!super::one_pacing_schedule_and_one_gap(&fixture.tree()).is_clean());
    }
