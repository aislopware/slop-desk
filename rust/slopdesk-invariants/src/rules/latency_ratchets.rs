...
///
/// `HostWorkspaceMirror.topology` is a computed property: every read copies the entire entry map
/// and re-runs `WorkspaceTopology.init(entries:)` over every cell in the document. Measured in a
/// scratch harness (`swiftc -O`), the dictionary copy alone is 6.4µs at 12 panes and 23.9µs at 48,
/// and the per-cell decode walk on top of it takes those to 10.3µs and 37.9µs — a FLOOR, since the
/// real projection also rebuilds every split tree, spec, MRU and closed tab.
///
...
/// memoized against `workspaceMirrorRevision`, the key `tree` already trusted, and the ONE
/// remaining direct read is the memo's own miss path. A second direct read anywhere puts the whole
/// projection back on that caller's path with green tests and no compile error — `docs/55` §8's
/// drift class.
///
...
/// `WorkspaceStore+Intents.swift` failed the ban; removing it passed. Deleting one of the memo's
/// two reads failed the ceiling.
#[must_use]
pub fn the_mirror_topology_is_projected_once(tree: &Tree) -> Report {
    check_all(tree, &[
        ...
        Claim::Populated {
            ...
            extensions: SWIFT,
            minimum: 200,
            message: "only {found} Swift sources — the topology-memo ban would pass on an empty haystack",
        },
        Claim::NoneUnder {
            roots: &["Sources"],
            extensions: SWIFT,
            ...
            view: View::Code,
            exempt: &[MIRROR_MEMO],
            message: "{files} re-derives the WHOLE topology — the entry-map copy plus a walk of every cell, \
                      10µs at 12 panes and 38µs at 48. Read `mirroredTopology` instead; it answers from the \
                      memo keyed on `workspaceMirrorRevision`",
        },
        Claim::AtMost {
            path: MIRROR_MEMO,
            pattern: r"workspaceMirror\.topology",
            maximum: 2,
            view: View::Code,
            message: "the store reads workspaceMirror.topology {found} times; the memo has ONE miss path. A \
                      further read belongs inside `mirroredTopology` or it is not memoized",
        },
    ])
}
...
///
/// 1. THE DEVICE CONSOLES. `visible` is a `localizedCaseInsensitiveContains` over every retained
///    log line, and `logCapacity` is 600 on both models. Measured in a scratch `swiftc -O` harness
///    (NOT in the tree, two runs agreeing to 1%): 0.78 ms per derivation when the needle hits, 1.50
...
pub fn three_projections_read_once_per_pass(tree: &Tree) -> Report {
    ...
    let mut claims = vec![
        ...
        Claim::Lacks {
            ...
            view: View::Code,
            message: "OpenQuicklyView: `displayEntries` is back as a computed var — it is a second whole \
...
/// `slopdesk_folder_ranked`, `slopdesk_folder_sanitized` and `slopdesk_sync_input_keyboard_only`
/// all answer a SUBSET of what was lent, so the caller holds the exact ceiling of the answer before
/// it asks. Both faces used to call with a null output first to learn a size they could already
/// compute — and a null-output call on these doors is not free, it rebuilds the whole folder
/// database and sorts it, or runs the whole `vtscan` pass, and throws the answer away. Measured:
/// `ranked` at the shipped 200-entry cap 30.9 µs → 15.6 µs through the door (54.3 µs → 39.0 µs for
/// the whole Swift face), and the sync-input strip 14.4 µs → 7.5 µs on an 8 KiB mirrored paste. The
/// overlay asks for the ranking twice per keystroke. The GUESS is what makes the probe unnecessary,
/// so its absence is the same bug wearing a different face.
///
...
pub fn the_index_doors_guess_they_do_not_probe(tree: &Tree) -> Report {
    ...
    check_all(tree, &[
        ...
        Claim::Lacks {
            ...
            view: View::Code,
            message: "W1a: the folder ranking probes with a NULL output again — that call rebuilds the \
                      whole folder database and sorts it, to learn a size the caller already has (docs/55 \
                      §4)",
        },
        Claim::Lacks {
            path: SYNCINPUT,
            pattern: r"\bnil, *0\b|\bnull, *0\b",
            view: View::Code,
            message: "W1b: the sync-input strip probes with a NULL output again — that call runs the whole \
                      vtscan pass and keeps nothing (docs/55 §4)",
        },
        Claim::Matches {
            path: FRECENCY,
...
/// quadratic in a row with many hits. Measured against this module at the commit before, linked
/// into the same binary so both sides run the same `regex` build: 10 000 rows / 736 KB,
/// case-insensitive literal 6.3 ms → 1.2 ms; with whole-word 8.2 ms → 2.0 ms; and on ONE 160 KB row
/// with 20 000 regex hits — the shape a program prints whenever it emits an unwrapped line — 782 ms
/// → 1.9 ms, per keystroke.
///
...
/// `HostWorkspaceMirror.resolved` copies the whole entry map and folds the overlay and every
/// pending patch over it, and `WorkspaceIntentApplier.apply` asks its `projectKey:` closure once
/// per pane the document names — live specs UNION the reopen ring. Built inside the closure that is
/// one whole-map copy per pane, which is quadratic in the workspace.
/// `WorkspaceMirrorBox.stageIntent` hoists it and says why at its own call site; this was the
/// sibling call site that did not.
///
/// ## W5. The launch-bytes emptiness rule has ONE author
/// Not latency: drift, of `docs/55` §8's class. `SessionTemplateEngine.launchBytes` used to trim
/// the cwd and the command itself, ahead of calling `templates::keystrokes`, which decides the same
...
pub fn the_scan_and_the_mirror_derive_once(tree: &Tree) -> Report {
    ...
    check_all(tree, &[
        ...
        Claim::Lacks {
            ...
            view: View::Code,
            message: "W3b: the row scan allocates a fresh Vec<u16> per row again — a malloc and a free per \
                      row of the scrollback, per keystroke in the find bar",
        },
        Claim::Lacks {
            path: ROWFIND,
            pattern: r"utf16_units\(line\.get\(\.\.",
            view: View::Code,
            message: "W3c: the row scan measures each regex column from the start of its row again — \
                      quadratic in a row with many hits, and one long unwrapped line freezes the find bar \
        ...
        Claim::Lacks {
            ...
            view: View::Code,
            message: "W5: the template engine decides emptiness itself again — `templates::keystrokes` owns \
                      that rule, and the last time both wrote it they disagreed about a whitespace-only cwd \
                      (docs/55 §8)",
        },
    ])
}
...
mod tests {
    ...
    fn the_removed_re_derivations_stay_removed() {
        ...
        sweep(&fixture);
        assert!(
            super::the_index_doors_guess_they_do_not_probe(&fixture.tree()).is_clean()
        ...
        fixture.write(
            "rust/slopdesk-rowscan/src/find.rs",
            "fn stands_alone(units: &[u16], at: usize) -> bool { true }\nfn folded(text: &str, \
             case_sensitive: bool) -> Vec<u16> { vec![] }\nlet prefix = \
