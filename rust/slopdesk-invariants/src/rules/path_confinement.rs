...
///
/// Two bans and a pair of tombstones, and they are one rule because they are one predicate seen from
/// four sides.
///
...
///
/// `CodeBridgeServer.contains` was `path.hasPrefix(root)`, which treats `/a/repo-evil` as a child of
/// `/a/repo` unless a separator guard is bolted on beside it, and which says nothing at all about
/// `..`. A containment answer that is a string comparison is a bug whose next reader will assume the
/// guard is somewhere.
///
/// `pathComponents`/`isWithin([String],root:)` were the decoder's own splitter and prefix match, and
/// a `contains(root:path:)` with a BODY is the bridge's string test coming back. The last of those
/// is a WINDOW rather than a line — the signature and the `hasPrefix` under it only mean something
/// together — which is why it is a file-level ban with a multi-line pattern.
#[must_use]
    ...
    check_all(
        ...
        &[
            Claim::NoneUnder {
                ...
                view: View::Raw,
                exempt: &[],
                message: "a Swift file is deciding about a '..' component itself ({files}) — path \
                          confinement is slopdesk_path_confine's answer alone \
            ...
            Claim::NoneUnder {
                ...
                view: View::Raw,
                exempt: &[],
                message: "MetadataResponseBuilder's own path splitter/prefix match is back \
                          ({files}) — the rule is rust/slopdesk-probe/src/path_confine.rs",
            },
            Claim::NoFileUnder {
                roots: &["Sources/"],
                extensions: &["swift"],
                ...
                view: View::Raw,
                exempt: &[],
                message: "CodeBridgeServer.contains has a body again ({files}) — it must forward \
                          to PathConfinement.isWithin",
            },
        ],
    )
}
...
///
/// Someone "fixing" the documented symlink residual with `canonicalize` is not a fix, and the module
/// says why at length: it needs the path to EXIST (so a missing file becomes a refusal rather than a
/// clean not-found), it refuses legitimate paths whose ROOT is itself a symlink (`/tmp` on macOS),
/// and it still loses to a symlink swap between the check and the open. Changing this is a design
/// decision, not a patch, and it must not arrive as a one-line diff.
///
    ...
    check_all(
        ...
        &[
            Claim::NoneUnder {
                ...
                view: View::Code,
                exempt: &[],
                message: "slopdesk-probe reached for canonicalize ({files}) — path confinement is \
                          LEXICAL on purpose; see the 'residual' section of \
                          rust/slopdesk-probe/src/path_confine.rs before changing it",
            },
            Claim::NoneUnder {
                roots: &["rust/"],
                extensions: &["rs"],
            ...
            Claim::Matches {
                ...
                pattern: r"fn (confine|is_confinable_absolute)\(",
                view: View::Code,
                message: "the one home no longer declares the rule — the ban beside this would \
                          then be a ban over nothing",
            },
        ],
    )
}
        ...
        &[
            Claim::Mentions {
                ...
                names: &[
                    ...
                    "slopdesk_path_is_confinable_absolute",
                ],
                message: "{entry} is missing from slopdesk_ffi.h — Swift cannot link the \
                          confinement rule",
            },
            Claim::Matches {
                path: FFI_LIB,
                ...
                view: View::Raw,
                message: "the shim does not export path_confine — the header promises a symbol the \
                          library will not carry",
            },
        ],
    )
}
...
/// accepted. It used to answer `.windowAdjust`, which is flow-control CREDIT: had the two type lists
/// stopped agreeing, an unrecognised byte would have granted a peer a send window out of a struct
/// field nothing filled. `unpack` in `mux_envelope.rs` answers `None` for that input, and the face
    ...
    check_all(
        ...
        &[
            Claim::Matches {
                ...
                view: View::Raw,
                message: "the near-side rebuild stopped refusing an unknown mux type — the type \
                          list is Rust's",
            },
            Claim::Matches {
        ...
        fixture
            .write(
                ...
                "    static func contains(root: String, path: String) -> Bool {\n\
                 \x20       PathConfinement.isWithin(root: root, path: path)\n\
                 \x20   }\n",
            )
            .write(
        ...
        fixture
            .write(
                ...
                "// `canonicalize` is NOT used: it needs the path to exist.\n\
                 pub fn confine(root: &Path, path: &Path) -> Option<PathBuf> { None }\n\
        ...
        fixture
            .write(
                ...
                "bool slopdesk_path_confine(const char *root, const char *path);\n\
                 bool slopdesk_path_is_confinable_absolute(const char *path);\n",
            )
