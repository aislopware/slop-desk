...
/// `[UInt8](message.encode())` — re-encoding, into a fresh array, bytes it had just parsed. Measured
/// against the shipped xcframework, two agreeing runs: 147.8/155.2 ns to re-encode against 0.7 ns to
/// lend the `Data`. `docs/55` §8 is why this is a gate and not a nicety: an exported door with no
/// caller is worse than an unported rule, because it reads as covered.
///
...
/// 488.0/509.1 ns per call with 4 cached shapes and 1249.3/1122.3 ns with 12, against 68.5/68.6 ns
/// and 252.2/258.6 ns through the guard. Matched on the LINE's shape rather than on `isKnown` alone,
/// because `isKnown` is also the test/diagnostics seam and would pass this gate while the hot path
/// went back through the step.
///
...
/// fix anything: the implicit `[Data]` → `[Data?]` bridge that then runs is a runtime cast, and it
/// is more than TWICE as dear as the `map` (902.8/905.0 ns at 24 fragments). So the ban is on the
/// eager `map`, and the positive rule is that the lazy overload is still there to bind to.
#[must_use]
    ...
    check_all(
        ...
        &[
            Claim::Matches {
                ...
                view: View::Code,
                message: "the client session no longer lends the control DATAGRAM — it is \
                          re-encoding a message it parsed",
            },
            Claim::Lacks {
                path: CLIENT_SESSION,
                pattern: r"handleControl\(message\)|\[UInt8\]\(message\.encode\(\)\)|handleControl\(routed",
                view: View::Code,
                message: "the client session re-encodes a routed control message — hand the state \
                          machine the bytes it arrived in",
            },
            Claim::Matches {
                path: SESSION_LOGIC,
                pattern: r"if +isKnown\(shapeID\) *\{ *return false *\}",
                view: View::Code,
                message: "the session logic lost the cached-shape guard — 6 allocations per cursor \
                          packet to answer no",
            },
            Claim::NoneOf {
                ...
                view: View::Code,
                message: "the send path eagerly promotes its fragments — hand the encoder the array \
                          and let the lazy overload take it",
            },
            Claim::Matches {
                path: VIDEO_FEC,
                pattern: r"blobs\.lazy\.map",
                view: View::Code,
                message: "FECScheme lost the lazy [Data] overload — every send-path caller pays a \
                          second array per frame",
            },
        ],
    )
}
...
/// and a finger-at-rest at 128; the momentum field is an ordinal, so ITS end is 3. Those ten numbers
/// were spelled in FOUR places across two languages — a private block in `client_gestures`, the
/// reprojector, the touch translation, and the Mac client's view — and two of the four read
...
///
/// On the Rust side the reprojector must keep READING the table rather than matching bare codes. Its
...
pub fn the_scroll_phases_are_one_table(tree: &Tree) -> Report {
    ...
    /// middle 2,514 lines were an `#if os(macOS)` / `#elseif os(iOS)` two-armed conditional, and the
    /// two phase encodings are `NSEvent.Phase`, so they went to the `AppKit` arm and nowhere else.
    ...
    check_all(
        ...
        &[
            Claim::Doors {
                path: WINDOW_VIEW,
                entries: &[
                    "slopdesk_cg_scroll_phase_code",
                    "slopdesk_cg_momentum_phase_code",
                ],
            ...
            Claim::Lacks {
                ...
                view: View::Code,
                message: "the Mac view decodes an NSEvent.Phase mask into a code itself again — \
                          hand the raw bits to the door",
            },
            Claim::Matches {
                path: SCROLL_REPROJECT,
                pattern: r"use crate::client_gestures::\{",
                view: View::Code,
                message: "the reprojector stopped reading the phase table — a bare 3 and a bare 4 \
            ...
            Claim::Lacks {
                ...
                view: View::Code,
                message: "the reprojector matches a bare phase code again — name it from \
                          client_gestures.rs",
            },
            Claim::Lacks {
                ...
                view: View::Code,
                message: "client_gestures grew a second private phase table — the exported \
                          SCROLL_*/MOMENTUM_* constants are it",
            },
        ],
    )
}
...
/// for every shape a knob can take: absent, empty, in-range, and unparseable are all UNCHANGED; only
/// out-of-range moves. Presence still decides whether const-QP engages at all, so an absent knob is
/// still OFF, and text that is not a number at all still leaves it OFF rather than inventing an
/// operating point.
///
/// ALL FIVE [1, 51] knobs in this target are named: MAX, CONST, CRISP and COMPACT in the encoder,
/// and `AQP_MAX` in the capturer. The fourth is here because this very gate found it — it was not in
/// the brief, it sat ten lines from the other three, and it had the same hand-rolled reject. The
/// FIFTH was found the same way, one file over, and is the reason `envQP` is not `private`: there is
/// no version of "one rule" where the fifth caller gets its own copy for living in another file.
///
...
/// over. The claim pins exactly that, so it can neither be deleted as dead nor quietly grown into a
    ...
    check_all(
        ...
        &[
            ...
            Claim::Matches {
                ...
                view: View::Code,
                message: "the encoder parses its quantiser knobs itself again — the parse and the \
                          clamp are the door's",
            },
            ...
            Claim::NoneOf {
                ...
                view: View::Code,
                message: "a [1,51] quantiser knob is parsed by hand again — clamping through the \
                          door is the answer the caller can act on",
            },
            Claim::Matches {
                path: WINDOW_CAPTURER,
                pattern: r"VideoEncoder\.envQP\(",
                view: View::Code,
                message: "the capturer stopped asking VideoEncoder.envQP for SLOPDESK_AQP_MAX — the \
            ...
            Claim::Matches {
                ...
                view: View::Code,
                message: "the message-shaped handleControl stopped delegating — a test-only face \
                          that decides",
            },
        ],
    )
    ...
    check_all(
        ...
        &[
            ...
            Claim::Matches {
                ...
                view: View::Code,
                message: "the settings sheet no longer asks slopdesk_qp_config_default — its \
                          defaults are the encoder's",
            },
            Claim::Matches {
                path: PREFERENCES,
                pattern: r"slopdesk_adaptive_fec_constant\(",
                view: View::Code,
                message: "the settings sheet no longer asks slopdesk_adaptive_fec_constant — its \
                          defaults are the encoder's",
            },
            Claim::Lacks {
                path: PREFERENCES,
                ...
                view: View::Code,
                message: "a QP or FEC default is a literal again — those four numbers are the \
                          encoder's, and \"reset to default\" writes whatever this file shows",
            },
        ],
    )
}
...
/// and fractions, and it had three implementations: a generic pair in `EnvConfig` with no callers at
/// all, and a private copy inside each of two files.
///
...
/// sheet was written, persisted, shown as active — and never read. `EnvConfig.string` is the only
/// legal reader here: real env FIRST, then the settings overlay, then the compile-time default.
///
/// The ban is on the PARSE-THEN-COMPARE shape all three copies had, `Int(s), v >= lo`, rather than
/// on a door's absence — a file can keep calling the door and grow a second private parse beside it,
/// which is how the third copy appeared in the first place. And the generic pair stays deleted
...
pub fn the_reject_reading_of_an_env_knob_is_rusts(tree: &Tree) -> Report {
    ...
    let mut claims = vec![
        Claim::Lacks {
            ...
            message: "the governor reads the process environment directly again — that bypasses the \
                      settings overlay, which is how a governor tunable set in Settings came to do \
                      nothing",
        },
        Claim::NoneOf {
        ...
        Claim::Populated {
            ...
            extensions: SWIFT,
            minimum: 200,
            message: "the tree-wide Swift corpus read as {found} files — the EnvConfig ban is \
                      passing vacuously",
        },
        Claim::NoneUnder {
            roots: &["Sources", "Tests"],
            ...
            view: View::Code,
            exempt: &[],
            message: "{files} calls EnvConfig.int/.double again — that generic reject pair had zero \
                      callers and is deleted; ask a door",
        },
    ];
    for caller in [ABR_CONTROLLER, FPS_GOVERNOR] {
        claims.push(Claim::Matches {
            ...
            view: View::Code,
            message: "a congestion reader no longer asks slopdesk_abr_validated_int — the reject \
                      rule is congestion.rs's",
        });
        claims.push(Claim::Matches {
            path: caller,
            pattern: r"slopdesk_abr_validated_double\(",
            view: View::Code,
            message: "a congestion reader no longer asks slopdesk_abr_validated_double — the reject \
                      rule is congestion.rs's",
        });
    }
        ...
        fixture
            ...
            .write(
                ...
                "if isKnown(shapeID) { return false }\n\
                 func handleControl(_ message: VideoControlMessage) { handleControl(datagram: message.encode()) }\n",
            )
            .write(
        ...
        fixture
            .write(
                ...
                "let code = slopdesk_cg_scroll_phase_code(bits)\n\
                 let m = slopdesk_cg_momentum_phase_code(bits)\n",
            )
            .write(
    ...
    fn a_settings_default_spelled_as_a_literal_is_red() {
        ...
        fixture.write(
            ...
            "static var qpSharpDefault: Int { slopdesk_qp_config_default(0) }\n\
             static var fecMDefault: Int { slopdesk_adaptive_fec_constant(11) }\n",
        );
