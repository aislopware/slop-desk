...
//! no caller — and the two did not merely duplicate, they DISAGREED: for a divider the file does not
//! name, Rust derives the `SplitNodeId` from the seam's position while the Swift decoder minted a
//! fresh UUID. Every launch renamed every seam, and every remembered divider position was lost.
...
/// `VersionMismatch(i64)` from the crate ⇒ FAIL "the rule and its taxonomy are one place". All three
    ...
    check_all(
        ...
        &[
            Claim::Doors {
                ...
                message: "WorkspaceStateFile stopped asking {entry} — one answer to what survives a \
                          restart (docs/55 §6)",
            },
            Claim::Lacks {
                path: STATE_FACE,
                ...
                view: View::Code,
                message: "WorkspaceStateFile decides something again — it marshals, and \
                          rust/slopdesk-wire's state_file rules (docs/55 §6)",
            },
            Claim::Mentions {
                path: STATE_RULE,
                names: &[
                ...
                message: "rust/slopdesk-wire/src/document/state_file.rs lost {entry} — the rule and \
                          its taxonomy are one place, so the door cannot be a shim over a shim \
                          (docs/55 §6)",
            },
        ],
    )
...
/// `SplitAxis` and the device-prefs template values stay `Codable` on purpose: those are vocabulary,
/// `docs/55` §8.)
///
/// The pool probe is the load-bearing door: the crate holds no entropy, so a caller that stopped
/// asking how many ids a file needs would hand it a pool that runs dry, and a dry pool REPEATS — two
/// panes with one id, which the repair then re-mints apart on every single load. That is the divider
/// defect again, wearing the pane's clothes.
///
    ...
    check_all(
        ...
        &[
            Claim::Absent {
                ...
                message: "the workspace file's rule is rust/slopdesk-workspace's persist, and this \
                          file's decoder minted a fresh id for every unnamed divider — every launch \
                          renamed every seam (docs/55 §6)",
            },
            Claim::Exists {
                path: FILE_FACE,
                message: "the workspace file's door has no Swift face, so every ban about it \
                          stopped checking anything (docs/55 §6)",
            },
            Claim::NoneUnder {
                roots: &["Sources"],
                extensions: SWIFT,
                ...
                view: View::Code,
                exempt: &[],
                message: "a Swift workspace-file decoder is back in Sources/ — the tree's JSON \
                          lives in rust/slopdesk-workspace (docs/55 §6): {files}",
            },
            Claim::NoneUnder {
                roots: &["Sources"],
                extensions: SWIFT,
                ...
                message: "a workspace tree type conforms to Codable again — one encoder, and it is \
                          persist::encode_file; a SYNTHESIZED one has no derivation, so it brings \
                          the divider-renaming defect back exactly as it was (docs/55 §6): {files}",
            },
            Claim::Doors {
                path: FILE_FACE,
                entries: DOORS,
                message: "WorkspaceFile stopped asking {entry} — one answer to the saved \
                          arrangement (docs/55 §6)",
            },
            Claim::Mentions {
                path: HEADER,
                names: DOORS,
                message: "slopdesk_ffi.h does not declare {entry} — the header is hand-written and \
                          it is the ABI (docs/55 §2)",
            },
            Claim::Lacks {
                path: FILE_FACE,
            ...
            Claim::Lacks {
                ...
                pattern: r"JSONEncoder|JSONDecoder|CodingKeys|schemaVersion *= *[0-9]",
                view: View::Code,
                message: "WorkspacePersistence decides the file's shape again — the store beneath \
                          the face cannot go back to it either (docs/55 §6)",
            },
            Claim::Mentions {
                path: FILE_RULE,
                names: &[
                ...
                message: "rust/slopdesk-workspace/src/persist.rs lost {entry} — the file's rule and \
                          its taxonomy are one place, and the derivation is the defect's actual fix \
                          (docs/55 §6)",
            },
        ],
    )
...
/// narrowed: the crate's `natural_compare` does not fold diacritics, so a reappearance here would be
/// a second answer to the sidebar's order rather than a refinement of it. The ban reads
...
/// Rust crate". Separately wrote `func depenetrate(` into a file under Domain/ ⇒ FAIL "grew it back".
...
///
/// Four of these cross as a BARE DISCRIMINANT, so a case that means 4 on one side and 5 on the other
/// sends focus the wrong way, aligns to the wrong edge or re-tiles into the wrong layout — with
/// every test green, because each side is self-consistent. The other four are wire type bytes, where
/// the same skew is not a decode error at all: it is a frame that decodes cleanly as the WRONG
/// message.
///
/// This compared the COUNT of cases for a long time, which cannot see either failure. A count is
/// blind to a reorder, and blind to a case added correctly to both enums and forgotten in the shim's
/// decoder. The Rust half is checked by the compiler and by a round-trip test per enum
    ...
    check_all(
        ...
        &[
            Claim::SameByteMap {
                ...
                rust: rust_map("rust/slopdesk-tree/src/focus.rs", r"pub const fn index\(self\) -> u8"),
            },
            Claim::SameByteMap {
                label: "PaneKind",
...
mod tests {
    ...
    const RUST_SWITCH: &str = "impl FocusDirection {\n    pub const fn index(self) -> u8 {\n        \
                               match self {\n            Self::Left => 0,\n            \
    ...
    fn a_renumbered_case_is_red_even_at_the_same_count() {
        ...
        fixture.write(super::BRIDGE, &SWIFT_SWITCH.replace("case .centerHorizontal: 4", "case .centerHorizontal: 5"));
    ...
    fn a_marker_that_lost_its_uniqueness_is_red() {
        ...
        enums(
            &fixture,
            SWIFT_SWITCH,
            &format!("{RUST_SWITCH}impl NewTabPosition {{\n    pub const fn index(self) -> u8 {{\n        \
                      self as u8\n    }}\n}}\n"),
        );
        let claims = [crate::claim::Claim::SameByteMap {
            label: "FocusDirection",
        ...
        fixture
            ...
            .write(
                ...
                "size_t slopdesk_ws_workspace_file_minted_ids(const uint8_t *p);\n\
                 size_t slopdesk_ws_workspace_file_encode(const uint8_t *p);\n\
                 size_t slopdesk_ws_workspace_file_decode(const uint8_t *p);\n\
                 size_t slopdesk_ws_workspace_file_status(const uint8_t *p);\n\
