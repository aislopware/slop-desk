...
/// which still reports its printable base — stays classifiable. DEL is refused with the C0 block
/// rather than after it: it is the base Backspace reports, it is neither whitespace nor `< 0x20`,
/// and `slopdesk_video::key_naming` refuses it on the Mac — so a phone that kept it would resolve
/// ⌘⌫ to a chord no `keybind` line on the other client could ever match.
#[must_use]
pub fn key_chord(press: &KeyPress<'_>) -> Option<Chord> {
    ...
    if first.is_whitespace() || u32::from(first) < 0x20 || u32::from(first) == 0x7F {
        return None;
    }
    Some(Chord {
