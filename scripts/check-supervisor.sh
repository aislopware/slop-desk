...
# PORTED to `rust/slopdesk-invariants` — `rules::cli_config`: cli-folder-frecency, config-file-reader, number-spelled-once,
#   swipe-nav-handle.
...
# client-send-keys and config-name-table.
...
# PORTED to `rust/slopdesk-invariants` — `rules::byte_scanners`: plain-text-vt-grammar, shell-word-quoting, width-table,
#   escape-end-grammar, find-bar-engine, base64-and-secret-notation, escape-decoding.

# PORTED to `rust/slopdesk-invariants`: foreground-process-vocabulary.

# PORTED to `rust/slopdesk-invariants` — `rules::agent_fold`: tab-badge-ladder, hook-body-reading, pane-detector-probes,
#   secret-shape-vocabulary, fresh-install-payload.
...
# PORTED to `rust/slopdesk-invariants` — `rules::transport_lanes`: device-console-grammar, superd-frame-spelling,
#   receive-buffer, arena-reader, nwconnection-channel, write-loop.

# PORTED to `rust/slopdesk-invariants` — `rules::hot_paths`: untrusted-regex-engine.

# PORTED to `rust/slopdesk-invariants` — `rules::split_surfaces`: split-drop-zone,
# split-cheat-sheet, split-toast-card, split-palette. One claim did NOT come across: the
# palette's `MacWorkspaceRootView` `draws:` set no longer exists anywhere in the tree, so the
# `grep -A4 draws:` had been matching nothing and passing for it.
...
# PORTED to `rust/slopdesk-invariants` — `rules::overlay_split`: overlay-host-ambient-layer, split-peek-card,
#   keystroke-and-peek-rules, split-global-search, split-picker. Three claims did NOT come
#   across: the `grep -A4 'draws:'` asks about `.peekReply`, `.globalSearch` and `.palette`
#   matched nothing — `draws:` is gone from the tree and MacWorkspaceRootView no longer names
#   OverlayHostView at all — so all three had been passing vacuously.
