...
case "${1:-}" in
  ...
  "") ;;
  *) echo "usage: check-video.sh [--window-title SUBSTR]" >&2; exit 2 ;;
esac
...
[[ -z "$PID" ]] && { echo "==> FAIL: client app never started" >&2; exit 1; }
...
for _ in $(seq 1 20); do
  if lsof -nP -iUDP:"$MEDIA_PORT" 2>/dev/null | grep -q "127.0.0.1:$MEDIA_PORT->" ; then CONNECTED=1; break; fi
  sleep 0.5
done
if [[ "$CONNECTED" == "1" ]]; then
