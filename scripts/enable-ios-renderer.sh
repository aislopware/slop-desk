...
# (b) dependencies: add the xcframework after the SlopDeskVideoClient product dependency.
dep_anchor = (
    "      - package: SlopDesk\n"
...
if "libghostty.xcframework" not in text:
    if dep_anchor not in text:
        sys.exit("ERROR: could not find the SlopDeskVideoClient dependency anchor in project.yml")
