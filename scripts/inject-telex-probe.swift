...
// composition bug in SlopDesk's remote-keyboard path.
//
// WHY: SlopDesk forwards layout-level KEYCODES to the host and posts them as synthetic
...
//   variants: slopdesk | nouser | private | session | postpid   (see makeVariants)
//
...
struct Variant {
    ...
    let setUserData: Bool // stamp a nonzero (non-sentinel) tag like SlopDesk does
    let post: PostMode
    let delayMs: UInt32
}
...
func makeVariants() -> [Variant] {
    [
        ...
        Variant(
            name: "slopdesk@12  (hid + userData, .hidSystemState)",
            stateID: .hidSystemState,
            setUserData: true,
            post: .hid,
            delayMs: 12,
        ),
        Variant(name: "slopdesk@60", stateID: .hidSystemState, setUserData: true, post: .hid, delayMs: 60),
        Variant(name: "slopdesk@150", stateID: .hidSystemState, setUserData: true, post: .hid, delayMs: 150),
...
if args.count > 1, args[1] == "word" {
    ...
    let name = args.count > 1 ? args[1] : "slopdesk"
