...
def read_ack(s):
    ...
    if len(data) < 2 or data[0] != CH_CONTROL:
        return ("non-control", data)
    p = data[1:]
    if p[0] != 0x02:
        return ("not-ack", p)
    accepted = p[1]
    (stream_id,) = struct.unpack(">I", p[2:6])
    (cw,) = struct.unpack(">H", p[6:8])
    (ch,) = struct.unpack(">H", p[8:10])
...
def down(s, x, y, c=1):
    button_evt(s, 2, x, y, c)


def up(s, x, y, c=1):
    button_evt(s, 3, x, y, c)


def drag(s, x, y, c=1):
    button_evt(s, 7, x, y, c)
...
def main():
    ...
    if cmd == "suite":
        ...
        for i in range(n):
            ...
            y = 0.2
            down(s, x, y)
            up(s, x, y)  # back-to-back: provoke host reorder
            time.sleep(0.05)
